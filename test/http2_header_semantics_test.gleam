...
fn field(name: BitArray, value: BitArray) -> decoder.Header {
  decoder.Header(name, value, False)
}
...
pub fn rfc8441_extended_connect_requires_target_and_safe_fields_test() -> Nil {
  let prefix = [
    field(<<":method">>, <<"CONNECT">>),
    field(<<":protocol">>, <<"websocket">>),
  ]
  assert header_semantics.validate(
    ...
    == Error(header_semantics.MissingPseudoHeader(<<":scheme">>))
  assert header_semantics.validate(
      list.append(prefix, [
        field(<<":scheme">>, <<"https">>),
        field(<<":authority">>, <<"example.com">>),
    ...
    == Error(header_semantics.MissingPseudoHeader(<<":path">>))
  assert header_semantics.validate(
      list.append(prefix, [
        field(<<":scheme">>, <<"https">>),
        field(<<":authority">>, <<"example.com">>),
    ...
    == Error(header_semantics.ConnectionSpecificField(<<"connection">>))
}
...
pub fn content_length_obeys_bodyless_response_status_rules_test() -> Nil {
  assert header_semantics.validate(
      [
        field(<<":status">>, <<"103">>),
        field(<<"content-length">>, <<"0">>),
    ...
    == Error(header_semantics.ContentLengthForbidden(status: 103))
  assert header_semantics.validate(
      [
        field(<<":status">>, <<"204">>),
    ...
    == Error(header_semantics.ContentLengthForbidden(status: 204))

  let assert Ok(header_semantics.Validated(
    header_semantics.ResponseControlData(304),
    [content_length],
    Some(42),
  )) =
    header_semantics.validate(
  ...
  assert content_length == field(<<"content-length">>, <<"42">>)
  Nil
}
