...
-export([
    ...
    start_client_transport_trace/1,
    start_server_transport_trace/1,
    stop_client_transport_trace/2,
    stop_server_transport_trace/2,
    traced_routed_connection_id/2
]).

-define(CLIENT_TRANSPORT,
        'gleam_quic@internal@runtime@client_transport').
-define(CLIENT_WORKER,
        'gleam_quic@internal@runtime@client_worker').
-define(SERVER_TRANSPORT,
        'gleam_quic@internal@runtime@server_transport').
-define(SERVER_WORKER,
        'gleam_quic@internal@runtime@connection_worker').
...
%% Temporarily expose the exact internal transition that ends a pressured
%% connection. This is a test-only diagnostic seam: a dedicated collector
%% drains the potentially large call traces while the test runs, tracing is
%% limited to the two actors under test, and every pattern is removed again.
-spec start_server_transport_trace(pid()) -> pid().
start_server_transport_trace(Pid) when is_pid(Pid) ->
    start_transport_trace(Pid, server_trace_patterns()).

-spec start_client_transport_trace(pid()) -> pid().
start_client_transport_trace(Pid) when is_pid(Pid) ->
    start_transport_trace(Pid, client_trace_patterns()).

-spec stop_server_transport_trace(pid(), pid()) -> binary().
stop_server_transport_trace(Pid, Collector) ->
    stop_transport_trace(Pid, Collector, server_trace_patterns()).

-spec stop_client_transport_trace(pid(), pid()) -> binary().
stop_client_transport_trace(Pid, Collector) ->
    stop_transport_trace(Pid, Collector, client_trace_patterns()).

-spec server_trace_patterns() -> [{module(), atom(), arity(), atom()}].
server_trace_patterns() ->
    [{?SERVER_TRANSPORT, send, 4, returns},
     {?SERVER_TRANSPORT, buffered_send_bytes, 2, returns},
     {?SERVER_TRANSPORT, tick, 2, returns},
     {?SERVER_TRANSPORT, receive_datagram, 5, returns},
     {?SERVER_TRANSPORT, issue_session_ticket_if_ready, 2, returns},
     {?SERVER_TRANSPORT, prepare_datagram, 3, returns},
     {?SERVER_TRANSPORT, commit_datagram, 3, returns},
     {?SERVER_WORKER, fail_connection, 2, calls},
     {?SERVER_WORKER, shutdown, 2, calls}].

-spec client_trace_patterns() -> [{module(), atom(), arity(), atom()}].
client_trace_patterns() ->
    [{?CLIENT_TRANSPORT, send, 4, returns},
     {?CLIENT_TRANSPORT, buffered_send_bytes, 2, returns},
     {?CLIENT_TRANSPORT, drive, 1, returns},
     {?CLIENT_TRANSPORT, receive_active, 2, returns},
     {?CLIENT_TRANSPORT, activate_once, 1, returns},
     {?CLIENT_TRANSPORT, probe_path_mtu, 2, returns},
     {?CLIENT_WORKER, shutdown, 2, calls}].

-spec start_transport_trace(
        pid(), [{module(), atom(), arity(), atom()}]) -> pid().
start_transport_trace(Pid, Patterns) ->
    Collector = spawn(fun() -> trace_collector(Pid, []) end),
    enable_trace_patterns(Patterns),
    _ = erlang:trace(Pid, true, [call, {tracer, Collector}]),
    Collector.

-spec stop_transport_trace(
        pid(), pid(), [{module(), atom(), arity(), atom()}]) -> binary().
stop_transport_trace(Pid, Collector, Patterns) ->
    disable_process_trace(Pid),
    await_trace_delivery(Pid),
    disable_trace_patterns(Patterns),
    Reference = make_ref(),
    Collector ! {stop, self(), Reference},
    receive
        {trace_diagnostics, Reference, Diagnostics} -> Diagnostics
    after 2000 ->
        <<"trace collector timeout">>
    end.

-spec enable_trace_patterns([{module(), atom(), arity(), atom()}]) -> ok.
enable_trace_patterns(Patterns) ->
    lists:foreach(
      fun
          ({Module, Function, Arity, returns}) ->
...
-spec disable_trace_patterns([{module(), atom(), arity(), atom()}]) -> ok.
disable_trace_patterns(Patterns) ->
    lists:foreach(
      fun({Module, Function, Arity, _Kind}) ->
          _ = erlang:trace_pattern(
                {Module, Function, Arity}, false, [local]),
          ok
      end,
      Patterns).
...
-spec trace_collector(pid(), [term()]) -> ok.
trace_collector(Pid, Diagnostics) ->
    receive
        {trace, Pid, return_from,
         {Module, Function, _Arity}, {error, Error}} ->
            trace_collector(
              Pid,
              [{transport_error, Module, Function, Error} | Diagnostics]);
        {trace, Pid, call,
         {Module, Function, [_State, Reason]}}
                when Function =:= fail_connection;
                     Function =:= shutdown ->
            trace_collector(
              Pid,
              [{lifecycle, Module, Function, Reason} | Diagnostics]);
        {trace, Pid, exit, Reason} ->
            trace_collector(Pid, [{exit, Reason} | Diagnostics]);
        {trace, Pid, _Event, _Detail} ->
            trace_collector(Pid, Diagnostics);
        {trace, Pid, _Event, _Detail, _Extra} ->
            trace_collector(Pid, Diagnostics);
        {stop, Sender, Reference} ->
            Formatted = io_lib:format("~p", [lists:reverse(Diagnostics)]),
            Sender ! {trace_diagnostics, Reference,
