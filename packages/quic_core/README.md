...
`quic_core` is a generic QUIC v1/v2 transport for Gleam on the Erlang target.
...
> cross-platform credential attestations, complete standards mapping,
> coverage, external interoperability, performance, and canonical-source
> distribution gates listed below remain open. The narrower pinned live qlog
> profile passes, but does not close the remaining standards-event inventory.
> Do not deploy it as a supported production release.
...
<!-- example: package=quic_core id=client_exchange -->
```gleam
...
<!-- example: package=quic_core id=server_echo -->
```gleam
...
<!-- example: package=quic_core id=mutual_tls -->
```gleam
...
pub fn configure(
  server_configuration: server.Server,
  client_configuration: client.Client,
  ...
  #(server_configuration, client_configuration, fingerprint)
}
...
- `quic_core/config` separates DNS, connect, handshake, operation, idle,
...
- qlog is opt-in, privacy-strict, asynchronous, bounded, and reports
  drop/write/queue counters instead of blocking transport work. Each
  connection owns its writer and may lend an opaque application sink that can
  add only typed payload-free protocol metadata, never close or inspect it.
...
policy and redacted identity access, P-256/HelloRetryRequest, live RSA-PSS,
ECDSA P-256/P-384 and Ed25519 credentials, and accepted or safely rejected
0-RTT. No tag, version change, Hex upload, hosted release, or push has been
...
`build/quic_core-0.1.0.tar`; this is a local release input, not a publication.

## Remaining release blockers

- complete the role-specific standards/errata maps and required live qlog
  event families;
- reach the full and changed-source coverage thresholds;
- collect the six OTP 28/29 platform credential rows and every declared
  independent interoperability peer;
- retain cross-protocol performance/resource and canonical-archive evidence;
  and
- obtain the independent audit and signed commit after every automated source
