...
<!-- example: package=quic_core id=operational_keys -->
```gleam
import quic_core/server

pub fn configure(
  configuration: server.Server,
  ticket_bytes: BitArray,
  address_token_bytes: BitArray,
  stateless_reset_bytes: BitArray,
) {
  let assert Ok(ticket_key) = server.operational_key(ticket_bytes)
  let assert Ok(address_token_key) = server.operational_key(address_token_bytes)
  let assert Ok(stateless_reset_key) =
    server.operational_key(stateless_reset_bytes)
  let assert Ok(keys) =
  ...
  server.with_operational_keys(configuration, keys)
}
