...
pub fn enforces_sender_credit_and_monotonic_max_data_test() -> Nil {
  ...
  let #(sender, first_blocked) = flow_control.take_blocked_limit(sender)
  assert first_blocked == Some(10)
  let #(sender, duplicate_blocked) = flow_control.take_blocked_limit(sender)
  assert duplicate_blocked == None
  let sender = flow_control.update_sender_limit(sender, 9)
  assert flow_control.sender_limit(sender) == 10
  let #(sender, unchanged_blocked) = flow_control.take_blocked_limit(sender)
  assert unchanged_blocked == None
  let sender = flow_control.update_sender_limit(sender, 20)
  let assert Ok(sender) = flow_control.reserve(sender, 10)
  assert flow_control.sent_bytes(sender) == 20
  let #(_sender, increased_blocked) = flow_control.take_blocked_limit(sender)
  assert increased_blocked == Some(20)
}
