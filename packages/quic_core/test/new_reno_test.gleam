...
pub fn grows_reduces_and_bounds_new_reno_window_test() -> Nil {
  ...
  let assert Ok(state) = new_reno.on_packet_sent(state, 12_000, True)
  assert new_reno.bytes_in_flight(state) == 12_000
  let assert Ok(state) = new_reno.on_packet_acked(state, 1200, 10, False)
  assert new_reno.snapshot(state)
    == new_reno.Snapshot(13_200, 10_800, new_reno.SlowStart)
  let assert Ok(state) = new_reno.abandon_in_flight(state, 10_800)
...
pub fn floors_the_window_at_the_new_maximum_datagram_size_test() -> Nil {
  ...
  // The adapter cannot infer that a later decrease was performed solely to
  // finish a handshake, so the documented advisory deviation preserves the
  // accumulated window while moving its future reduction floor.
  let assert Ok(state) = new_reno.set_maximum_datagram_size(state, 1200)
  assert new_reno.congestion_window(state) == 18_000
...
// nolint: unused_exports -- gleeunit discovers public test functions by suffix.
pub fn sparse_flight_cannot_grow_new_reno_without_a_caller_hint_test() -> Nil {
  let assert Ok(state) = new_reno.new(1200)
  let initial = new_reno.congestion_window(state)
  let assert Ok(state) = new_reno.on_packet_sent(state, 1200, True)
  let assert Ok(state) = new_reno.on_packet_acked(state, 1200, 10, False)
  assert new_reno.congestion_window(state) == initial
}
