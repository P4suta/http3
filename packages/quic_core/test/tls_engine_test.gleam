...
// nolint: unused_exports -- gleeunit discovers public test functions by suffix.
pub fn rfc7301_server_alpn_preference_is_authoritative_test() -> Nil {
  let #(client_config, server_config) = configs([<<"h3-29">>, <<"h3">>])
  let server_config =
    engine.ServerConfig(..server_config, application_protocols: [
      <<"h3">>,
      <<"h3-29">>,
    ])
  let assert Ok(#(client, server)) =
    complete_handshake(client_config, server_config)
  assert engine.client_application_protocol(client) == Some(<<"h3">>)
  assert engine.server_application_protocol(server) == Some(<<"h3">>)
}
...
// nolint: unused_exports -- gleeunit discovers public test functions by suffix.
pub fn negotiated_v2_requires_authenticated_version_information_test() -> Nil {
  let #(client, server) = v2_configs()
  let client = engine.ClientConfig(..client, version_negotiated: True)
  let server =
    engine.ServerConfig(
      ..server,
      transport_parameters: remove_version_information(
  ...
  assert receive_server_flight(client, server)
    == Error(engine.VersionNegotiationFailure)
}
...
pub fn validates_version_negotiation_available_versions_test() -> Nil {
  ...
  assert receive_server_flight(client_config, empty_server)
    == Error(engine.VersionNegotiationFailure)
  // RFC 9368 section 8 preserves incompatible negotiation with legacy
  // version-1-only servers by synthesizing {v1, [v1]} when the authenticated
  // server parameters omit version_information.
  let assert Ok(_) = receive_server_flight(client_config, server_config)
  Nil
}
