...
// A PATH_RESPONSE authenticates only the network path on which it arrived.
// During validation, packets from the former path and from unrelated paths
// are intentionally not passed to the shared QUIC state, because that state
// cannot otherwise bind an encrypted response to its receiving socket/tuple.
// nolint: unused_exports -- gleeunit discovers public test functions by suffix.
pub fn path_validation_only_authenticates_candidate_path_test() -> Nil {
  assert path_gate.may_authenticate(False, path_gate.Active)
  assert path_gate.may_authenticate(False, path_gate.Candidate)
  assert path_gate.may_authenticate(False, path_gate.Unrelated)

  assert !path_gate.may_authenticate(True, path_gate.Active)
  assert path_gate.may_authenticate(True, path_gate.Candidate)
  assert !path_gate.may_authenticate(True, path_gate.Unrelated)
}
...
// nolint: unused_exports -- gleeunit discovers public test functions by suffix.
pub fn local_probe_too_large_lowers_search_without_forgetting_confirmed_path_test() -> Nil {
  let assert Ok(state) = pmtu.new(1500)
  let assert Ok(#(state, 1350)) = pmtu.start_probe(state, 1500)
  let assert Ok(state) = pmtu.probe_too_large(state, 1350)
  assert pmtu.current(state) == 1200
  assert !pmtu.probe_available(state, 1200)
  assert pmtu.probe_available(state, 1500)
  let assert Ok(#(_state, next)) = pmtu.start_probe(state, 1500)
  assert next == 1275
}
