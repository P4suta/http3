...
pub fn client_configuration_rejects_ambiguous_inputs_test() -> Nil {
  ...
  let _configured =
    ...
    |> client.with_connect_address(
      quic_core.ip_address(<<127, 0, 0, 1>>) |> should.be_ok,
    )
    |> client.with_deadlines(deadlines)
    |> client.with_limits(limits)
    |> client.with_version(quic_core.QuicV2)
    |> client.with_congestion_control(quic_core.Cubic)
...
pub fn server_configuration_validates_credentials_and_keys_test() -> Nil {
  ...
  assert server.valid_certificate(certificate)
  assert !server.valid_certificate(<<"not pem":utf8>>)
  assert server.valid_private_key(private_key)
  assert !server.valid_private_key(<<"not pem":utf8>>)
  assert server.valid_server_name("api.example.com")
  assert server.valid_server_name("*.example.com")
  assert !server.valid_server_name("")
  assert !server.valid_server_name("127.0.0.1")
  let configured =
  ...
  let _configured =
    ...
    |> server.with_bind_address(
      quic_core.ip_address(<<127, 0, 0, 1>>) |> should.be_ok,
    )
    |> server.with_deadlines(deadlines)
    |> server.with_limits(limits)
    |> server.with_congestion_control(quic_core.Cubic)
...
// nolint: unused_exports -- gleeunit discovers public test functions by suffix.
pub fn ip_address_accepts_only_network_order_literals_test() -> Nil {
  let ipv4 = quic_core.ip_address(<<127, 0, 0, 1>>) |> should.be_ok
  let ipv6 = quic_core.ip_address(<<0:120, 1>>) |> should.be_ok

  assert quic_core.ip_address_bytes(ipv4) == <<127, 0, 0, 1>>
  assert quic_core.ip_address_bytes(ipv6) == <<0:120, 1>>
  assert quic_core.ip_address(<<127, 0, 0>>)
    == Error(quic_core.InvalidIpAddress)
  assert quic_core.ip_address(<<0:128, 1:size(1)>>)
    == Error(quic_core.InvalidIpAddress)
}
