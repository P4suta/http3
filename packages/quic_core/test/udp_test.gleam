...
@external(erlang, "quic_core_test_ffi", "inject_relay_connection_reset")
fn inject_relay_connection_reset(relay: udp.Relay) -> Nil

@external(erlang, "quic_core_test_ffi", "socket_buffer_bytes")
fn socket_buffer_bytes(socket: udp.Socket) -> List(Int)

@external(erlang, "quic_core_test_ffi", "socket_dont_fragment_values")
fn socket_dont_fragment_values(socket: udp.Socket) -> List(Int)

@external(erlang, "quic_core_test_ffi", "linux_platform")
...
// A second listener must never steal datagrams from a live QUIC endpoint.
// nolint: unused_exports -- gleeunit discovers public test functions by suffix.
pub fn refuses_a_second_exact_ipv4_bind_test() -> Nil {
  let assert Ok(loopback) = udp.ipv4(127, 0, 0, 1)
  let assert Ok(ephemeral) = udp.endpoint(loopback, 0)
  let assert Ok(first) = udp.open(ephemeral)
  let assert Ok(bound) = udp.local_endpoint(first)
  let refused = case udp.open(bound) {
    Error(udp.AddressInUse) -> True
    // nolint: thrown_away_error -- only the exact AddressInUse refusal counts.
    Error(_) -> False
    Ok(second) -> {
      let assert Ok(Nil) = udp.close(second)
      False
    }
  }
  let assert Ok(Nil) = udp.close(first)

  assert refused
}

// nolint: unused_exports -- gleeunit discovers public test functions by suffix.
pub fn assigns_unique_ports_to_held_ephemeral_ipv4_sockets_test() -> Nil {
  let assert Ok(loopback) = udp.ipv4(127, 0, 0, 1)
  let assert Ok(ephemeral) = udp.endpoint(loopback, 0)
  let #(sockets, ports) = open_ephemeral_ipv4_sockets(ephemeral, 256, [], [])
  let close_results = list.map(sockets, udp.close)

  assert list.all(close_results, fn(result) { result == Ok(Nil) })
  assert list.length(list.unique(ports)) == 256
}

fn open_ephemeral_ipv4_sockets(
  endpoint: udp.Endpoint,
  remaining: Int,
  sockets: List(udp.Socket),
  ports: List(Int),
) -> #(List(udp.Socket), List(Int)) {
  case remaining {
    0 -> #(sockets, ports)
    _ -> {
      let assert Ok(socket) = udp.open(endpoint)
      let assert Ok(local) = udp.local_endpoint(socket)
      let #(_, port) = udp.endpoint_parts(local)
      open_ephemeral_ipv4_sockets(endpoint, remaining - 1, [socket, ..sockets], [
        port,
        ..ports
      ])
    }
