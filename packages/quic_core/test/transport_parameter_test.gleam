...
// nolint: unused_exports -- gleeunit discovers public test functions by suffix.
pub fn grease_quic_bit_parameter_is_empty_and_role_symmetric_test() -> Nil {
  let encoded = <<0x6a, 0xb2, 0>>
  ...
  assert transport_parameter.decode_all(
    ...
    == Ok([transport_parameter.GreaseQuicBit])
  assert transport_parameter.decode_all(
      <<0x6a, 0xb2, 1, 0>>,
      transport_parameter.Server,
      transport_parameter.default_limits(),
    )
    == Error(transport_parameter.InvalidParameter(0x2ab2))
}
...
// nolint: unused_exports -- gleeunit discovers public test functions by suffix.
pub fn validates_each_version_information_syntax_boundary_test() -> Nil {
  let limits = transport_parameter.default_limits()
  let malformed = [
    // Missing Chosen Version.
    <<0x11, 0>>,
    // Truncated Chosen Version.
    <<0x11, 3, 0, 0, 1>>,
    // Available Versions has a trailing partial identifier.
    <<0x11, 5, 0, 0, 0, 1, 0xff>>,
    // Chosen Version is the reserved zero value.
    <<0x11, 4, 0, 0, 0, 0>>,
    // One Available Version is the reserved zero value.
    <<0x11, 8, 0, 0, 0, 1, 0, 0, 0, 0>>,
    // A client omitted its Chosen Version from Available Versions.
    <<0x11, 8, 0, 0, 0, 1, 0x6b, 0x33, 0x43, 0xcf>>,
  ]
  ...
  // Server lists describe deployment state: Chosen need not be listed and
  // the list may be empty while a version is being retired.
  assert transport_parameter.decode_all(
      <<0x11, 8, 0, 0, 0, 1, 0x6b, 0x33, 0x43, 0xcf>>,
      transport_parameter.Server,
      limits,
  ...
  assert transport_parameter.decode_all(
    ...
    == Ok([transport_parameter.VersionInformation(version.Version1, [])])
}
