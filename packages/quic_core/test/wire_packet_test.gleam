...
// nolint: unused_exports -- gleeunit discovers public test functions by suffix.
pub fn unnegotiated_packets_never_clear_the_quic_bit_test() -> Nil {
  let assert Ok(keys) =
    traffic_keys.from_secret(version.Version1, hello.Aes128GcmSha256, <<10:256>>)
  integers(0, 63)
  |> list.each(fn(packet_number) {
    let assert Ok(bytes) =
      wire_packet.protect_short(
        destination_connection_id,
        packet_number,
    ...
    let assert <<first, _:bits>> = bytes
    assert int.bitwise_and(first, 0x40) == 0x40
