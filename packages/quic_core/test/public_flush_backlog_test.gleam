...
// nolint: unused_exports -- gleeunit discovers public test functions by suffix.
pub fn client_ack_reopens_congestion_without_followup_command_test() -> Nil {
  let certificate = fixture("server.pem") |> should.be_ok
  let private_key = fixture("server-key.pem") |> should.be_ok
  let ca_certificate = fixture("ca.pem") |> should.be_ok
  let deadlines = config.uniform_deadlines(bound_milliseconds) |> should.be_ok
  ...
  let payload = repeated_bytes(backlog_bytes)
  let stream = client.open_bidirectional(connection) |> should.be_ok
  client.send_and_finish(stream, payload) |> should.be_ok
  let assert server.IncomingStream(peer_stream, server.Bidirectional) =
    server.accept_stream(peer) |> should.be_ok

  // No client API call follows the write. ACK reception itself must drive the
  // queued remainder after the first congestion window is acknowledged.
  let received = drain_server(peer_stream, <<>>)
  assert bit_array.byte_size(received) == backlog_bytes
  assert received == payload
...
fn drain_server(stream: server.Stream, collected: BitArray) -> BitArray {
  case server.receive(stream, 65_536) |> should.be_ok {
    server.Reset(_) -> collected
    server.Finished -> collected
    server.Data(bytes, True) -> bit_array.append(collected, bytes)
    server.Data(bytes, False) ->
      drain_server(stream, bit_array.append(collected, bytes))
  }
}
