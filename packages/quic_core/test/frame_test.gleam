...
// nolint: unused_exports -- gleeunit discovers public test functions by suffix.
pub fn accepts_empty_datagrams_in_both_rfc9221_wire_forms_test() -> Nil {
  assert frame.decode_all(<<0x30>>, frame.default_limits())
    == Ok([frame.Datagram(<<>>)])
  assert frame.decode_all(<<0x31, 0>>, frame.default_limits())
    == Ok([frame.Datagram(<<>>)])
  assert frame.encode(frame.Datagram(<<>>)) == Ok(<<0x31, 0>>)
}
