...
// RFC 7301 section 3.1: protocol names are non-empty, untruncated byte
// strings, and the enclosing vector length is exact.
// nolint: unused_exports -- gleeunit discovers public test functions by suffix.
pub fn rfc7301_alpn_wire_vector_and_bounds_test() -> Nil {
  let protocols = [<<"h2">>, <<"http/1.1">>]
  assert extension_value.encode_alpn(protocols)
    == Ok(<<0, 12, 2, "h2", 8, "http/1.1">>)
  assert extension_value.decode_alpn(<<0, 12, 2, "h2", 8, "http/1.1">>)
    == Ok(protocols)

  assert extension_value.encode_alpn([]) == Error(extension_value.InvalidLength)
  assert extension_value.encode_alpn([<<>>])
    == Error(extension_value.EmptyProtocol)
  assert extension_value.encode_alpn([<<0:2048>>])
    == Error(extension_value.InvalidLength)
  assert extension_value.encode_alpn([<<0:size(1)>>])
    == Error(extension_value.NonByteAligned)
  assert extension_value.encode_alpn([<<"h3">>, <<"h3">>])
    == Error(extension_value.DuplicateProtocol)
  assert extension_value.decode_alpn(<<0, 4, 3, "h2">>)
    == Error(extension_value.Truncated)
  assert extension_value.decode_alpn(<<0, 3, 2, "h2", 0>>)
    == Error(extension_value.Truncated)
}
