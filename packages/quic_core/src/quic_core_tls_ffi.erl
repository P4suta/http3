...
trust_store_from_pem(Pem) when is_binary(Pem) ->
    case decode_pem_certificates(Pem) of
        {ok, Certificates} -> {ok, {quic_core_trust_store, Certificates}};
        Error -> Error
    end;
    ...
    case ensure_public_key() of
        ok ->
            try
                ...
                {ok, {quic_core_trust_store, Certificates}}
            catch
...
validate_server_certificate(
    CertificateChain,
    {quic_core_trust_store, TrustAnchors},
    Hostname
...
verified_peer_fingerprint(
    {quic_core_verified_peer, LeafDer, _LeafCertificate, _PublicKeyInfo}
) when is_binary(LeafDer) ->
    ...
    case ensure_public_key() of
        ok ->
            try private_key_entries(public_key:pem_decode(Pem)) of
                [Entry] -> {ok, {quic_core_signing_key, public_key:pem_entry_decode(Entry)}};
                _Entries -> {error, 3}
            catch
                _Class:_Reason -> {error, 3}
            end;
...
signing_key_scheme({quic_core_signing_key, Key}) ->
...
signing_key_matches_certificate(
    ...
    {quic_core_signing_key, _Key} = SigningKey,
    Scheme
    ...
    case ensure_public_key() of
        ok ->
            case validate_chain([LeafDer], [LeafDer]) of
                {ok, Peer} ->
                    Content = <<"quic_core credential validation">>,
...
sign({quic_core_signing_key, Key}, Scheme, Content)
...
verify(
    {quic_core_verified_peer, _LeafDer, _LeafCertificate, PublicKeyInfo},
    Scheme,
    Content,
    Signature
...
validate_chain_and_identity(CertificateChain, TrustAnchors, Hostname) ->
    case validate_chain(CertificateChain, TrustAnchors) of
        {ok, {quic_core_verified_peer, _LeafDer, Leaf, _PublicKeyInfo} = Peer} ->
...
validate_chain(CertificateChain, TrustAnchors) ->
    try
        ...
        case find_valid_path(TrustAnchors, CertificateChain) of
            {ok, PublicKeyInfo} ->
                {ok, {quic_core_verified_peer, LeafDer, Leaf, PublicKeyInfo}};
            error ->
                {error, 5}
        end
    catch
