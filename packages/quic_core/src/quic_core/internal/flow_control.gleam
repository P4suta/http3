...
pub opaque type Sender {
  Sender(limit: Int, sent: Int, blocked_reported_at: Option(Int))
}
...
pub fn new_sender(initial_limit: Int) -> Result(Sender, Error) {
  case initial_limit >= 0 && initial_limit <= varint.maximum {
    True -> Ok(Sender(initial_limit, 0, None))
    False -> Error(InvalidInput)
  }
}
...
pub fn update_sender_limit(sender: Sender, advertised_limit: Int) -> Sender {
  case advertised_limit > sender.limit && advertised_limit <= varint.maximum {
    True -> Sender(..sender, limit: advertised_limit, blocked_reported_at: None)
    False -> sender
  }
}

/// Take one advisory BLOCKED-frame signal for the current limit.
///
/// A caller invokes this only after discovering that an application write
/// cannot cross the sender's current limit. The first observation returns the
/// limit and records it; immediate re-polls at that same limit return `None`.
/// Raising the limit rearms the signal. A BLOCKED frame already handed to
/// packet recovery remains retransmittable there if its packet is lost.
pub fn take_blocked_limit(sender: Sender) -> #(Sender, Option(Int)) {
  case sender.blocked_reported_at == Some(sender.limit) {
    True -> #(sender, None)
    False -> #(
      Sender(..sender, blocked_reported_at: Some(sender.limit)),
      Some(sender.limit),
    )
  }
}
