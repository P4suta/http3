...
/// or persistent congestion reduces to - from `max_datagram_size`. A
/// controller left at the pre-validation size would floor the window below a
/// single datagram once DPLPMTUD raises the path, stalling path-sized sends
/// until the window regrew. The window itself is never changed here: it is
/// the reductions that follow the new size.
...
pub fn on_packet_acked(
  ...
  // RFC 9002 section 7.8 makes the controller the final guard against
  // application-limited growth. A caller hint improves intent reporting, but
  // sparse bytes in flight are independently observable here and cannot be
  // allowed to inflate cwnd.
  let congestion_window_fully_utilized =
    state.bytes_in_flight >= state.congestion_window
...
/// Apply one packet from an ACK batch using the flight snapshot taken before
/// any packet in that batch was removed.
///
/// Without this entry point, the first ACK of a full flight lowers
/// bytes_in_flight and incorrectly makes every remaining ACK look
/// application-limited. The ordinary single-packet API above retains its own
/// fail-safe utilization check.
