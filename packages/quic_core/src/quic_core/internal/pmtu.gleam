...
/// Reject an unsent probe the local non-fragmenting socket reported as too
/// large. Unlike a black-hole response this keeps the already confirmed path
/// size and makes the rejected size an exclusive search ceiling, so discovery
/// cannot retry the same impossible range forever.
pub fn probe_too_large(state: State, size: Int) -> Result(State, Error) {
  case state.probe {
    Some(expected) if expected == size ->
      Ok(State(..state, upper_bound: size - 1, probe: None))
    _ -> Error(UnexpectedProbe)
  }
}
...
/// Whether discovery can reserve a larger probe under the caller's current
/// send allowance. A false result lets an event-driven runtime suspend
/// discovery until real traffic changes congestion credit, without polling.
pub fn probe_available(state: State, maximum_probe_size: Int) -> Bool {
  state.probe == None
  && state.upper_bound > state.current
  && maximum_probe_size > state.current
}
