...
/// Payload-free send-side state for bounded development diagnostics.
///
/// This deliberately reports byte counts and lifecycle flags only. It is
/// useful when a live stream stops making progress without retaining or
/// exposing any application bytes.
...
/// Snapshot send progress without copying the buffered payload.
pub fn send_progress(state: State) -> SendProgress {
  SendProgress(
    identifier: state.identifier,
    peer_limit: flow_control.sender_limit(state.sender),
    sent_bytes: flow_control.sent_bytes(state.sender),
    pending_bytes: bit_array.byte_size(state.pending_send_data),
    unacknowledged_bytes: state.unacknowledged_send_bytes,
    retransmission_frames: list.length(state.retransmit),
    fin_requested: state.fin_requested,
    fin_emitted: state.fin_emitted,
    fin_acknowledged: state.fin_acknowledged,
    send_reset: state.send_reset,
    pending_output: has_pending_output(state),
  )
}

/// Return whether one transport poll can produce stream output.
///
/// This deliberately mirrors `poll_send` without polling or copying stream
/// state. Retransmissions, fresh bytes (including flow-control-blocked bytes),
/// and an unsent FIN keep the stream active; bytes only awaiting an ACK do not.
pub fn has_pending_output(state: State) -> Bool {
  stream_id.can_send(state.identifier, state.local_endpoint)
  && !state.send_reset
  && {
    state.retransmit != []
    || state.pending_send_data != <<>>
    || { state.fin_requested && !state.fin_emitted }
  }
}
...
fn emit_new_data(
  ...
  case credit <= 0 {
    True -> {
      let #(sender, blocked) = flow_control.take_blocked_limit(state.sender)
      let state = State(..state, sender: sender)
      case blocked {
        Some(limit) -> Ok(SendBlocked(state, limit))
        None -> Ok(SendDeferred(state))
      }
    }
