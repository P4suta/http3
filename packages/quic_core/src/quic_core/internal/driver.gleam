...
/// Return whether a receive failure must be silently discarded.
///
/// Unauthenticated wire failures cannot affect connection state. A datagram
/// which races a connection already in Closing, Draining, or Closed likewise
/// has no live connection to fail; `ConnectionUnavailable` is the explicit
/// terminal-state signal. Authenticated protocol violations remain fatal.
pub fn discardable_receive_error(error: Error) -> Bool {
  case error {
    InvalidInput | DestinationConnectionIdMismatch | PacketFailure(_) -> True
    ConnectionFailure(connection_state.ConnectionUnavailable)
...
/// Reject one prepared but unsent DPLPMTUD probe after the local
/// non-fragmenting socket reports it as too large. Ordinary prepared
/// datagrams cannot enter this transition.
pub fn reject_pmtu_probe(prepared: PreparedDatagram) -> Result(State, Error) {
  let PreparedDatagram(state, _, _, frames, bytes) = prepared
  case frames {
    [frame.Ping, frame.Padding(_)] ->
      connection_state.reject_pmtu_probe(
        state.connection,
...
pub fn pacing_wake_progress(
  state: State,
  now_ms: Int,
) -> connection_state.PacingWakeProgress {
  connection_state.pacing_wake_progress(state.connection, now_ms)
}
        ...
        Ok(connection_state.PacketPrepared(connection, _, packet_number, frames)) ->
            ...
            )
          {
            Error(ConnectionFailure(connection_state.MissingWriteKeys(_))) ->
              prepare_levels(state, rest, maximum_frame_data_bytes, now_ms)
            // An unvalidated server can retain positive credit that is still
            // narrower than this level's complete protected datagram. Try a
            // later packet space, whose Handshake packet may fit where a
            // padded Initial does not. If none fits, `Ok(None)` tells the
            // actor to wait for peer bytes without consuming the prepared
            // frame or treating ordinary anti-amplification pressure as a
            // fatal transport inconsistency.
            Error(ConnectionFailure(connection_state.AmplificationLimited)) ->
              prepare_levels(state, rest, maximum_frame_data_bytes, now_ms)
            outcome -> outcome
          }
      }
...
fn receive_short_packet(
  ...
  // RFC 9001 section 5.7: neither endpoint may decrypt or process incoming
  // 1-RTT packets before its TLS handshake completes. A server can already
  // possess 1-RTT read keys while it is still waiting for the client's
  // Finished, so checking only MissingReadKeys below is insufficient. Do not
  // retain the peer-controlled ciphertext: silently dropping it is bounded,
  // does not acknowledge unprocessed frames, and lets QUIC retransmission
  // deliver the STREAM data after the Finished has established this state.
  case connection_state.phase(state.connection) {
    connection_state.Handshaking -> Ok(state)
    connection_state.Established
    | connection_state.Closing
    | connection_state.Draining
    | connection_state.Closed ->
      receive_short_packet_after_handshake(state, datagram, codepoint, now_ms)
  }
}

fn receive_short_packet_after_handshake(
  state: State,
  datagram: BitArray,
  codepoint: packet_space.ReceivedCodepoint,
  now_ms: Int,
