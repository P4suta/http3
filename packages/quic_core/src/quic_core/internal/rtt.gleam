...
/// Return whether the path has observed a real RTT sample.
///
/// Recovery records this bit with each sent packet. Reading the estimator at
/// loss time is too late: RFC 9002 persistent congestion requires the sample
/// to have existed when both boundary packets were sent.
pub fn has_sample(estimator: Estimator) -> Bool {
  estimator.has_sample
}

/// Compute RFC 9002 section 7.6.1's persistent-congestion duration.
///
/// Unlike PTO, every packet-number space includes the peer's maximum ACK
/// delay, and the recommended threshold is fixed at three.
...
/// Refresh min_rtt after persistent congestion without rewriting the sample.
///
/// RFC 9002 section 5.2 recommends using the newest sample so a path whose RTT
/// permanently increased does not repeatedly trigger persistent congestion.
pub fn on_persistent_congestion(estimator: Estimator) -> Estimator {
  case estimator.has_sample && estimator.latest_rtt > 0 {
    True -> Estimator(..estimator, minimum_rtt: estimator.latest_rtt)
    False -> estimator
  }
}
