...
/// Discard burst credit when a congestion response lowers the live send rate.
///
/// Keeping tokens accumulated under the old rate would let a one-datagram
/// CUBIC path send immediately after every persistent CE mark, defeating RFC
/// 9438 section 4.6's required rate reduction below one datagram per RTT.
pub fn on_congestion_event(
  state: State,
  now_milliseconds: Int,
) -> Result(State, Error) {
  case now_milliseconds >= state.updated_at {
    True -> Ok(State(..state, available_bytes: 0, updated_at: now_milliseconds))
    False -> Error(InvalidInput)
  }
}
