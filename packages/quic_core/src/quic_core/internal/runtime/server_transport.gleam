...
pub fn reject_pmtu_probe(prepared: PreparedDatagram) -> Result(State, Error) {
  connection.reject_pmtu_probe(prepared.prepared)
  |> result.map(fn(connection) {
    State(..prepared.state, connection: connection)
  })
  |> result.map_error(DriverFailure)
...
pub fn stream_identifiers(state: State) -> List(Int) {
  connection.stream_identifiers(state.connection)
}

/// Payload-free send, ACK, and recovery state for internal diagnostics.
...
fn server_transport_parameters(
  ...
  let parameters = [
    ...
    transport_parameter.VersionInformation(
      protocol_version,
      version_negotiation.fully_deployed_versions(),
    ),
