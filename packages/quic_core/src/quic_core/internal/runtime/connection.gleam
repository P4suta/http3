...
/// Connection-level receive limit already stated to the peer.
pub fn advertised_max_data(state: State) -> Int {
  transport.advertised_max_data(driver.connection(state.quic))
}

/// Advertised connection credit the peer has not yet spent.
pub fn outstanding_receive_credit(state: State) -> Int {
  transport.outstanding_receive_credit(driver.connection(state.quic))
}

/// Whether endpoint pressure is withholding stream credit growth.
pub fn credit_growth_held(state: State) -> Bool {
  transport.credit_growth_held(driver.connection(state.quic))
}
...
pub fn pacing_wake_progress(
  ...
  driver.pacing_wake_progress(state.quic, now)
}
...
/// Retire a prepared but unsent PMTU probe which the local socket rejected as
/// too large. No packet, byte, flush, recovery, or congestion counter grows.
pub fn reject_pmtu_probe(
  prepared: PreparedDatagram,
) -> Result(State, driver.Error) {
  driver.reject_pmtu_probe(prepared.prepared)
  |> result.map(fn(quic) { State(..prepared.state, quic: quic) })
}
...
/// Return whether this stream's send direction is transport-settled.
pub fn send_finished(
  state: State,
  identifier: Int,
) -> Result(Bool, driver.Error) {
  case
    transport.stream_send_finished(driver.connection(state.quic), identifier)
  {
    Ok(finished) -> Ok(finished)
    // A stream is removed from transport state only after every direction
    // available to this endpoint is terminal. An opaque handle reaching this
    // branch therefore denotes a settled stream, not an unknown caller ID.
    Error(transport.UnknownStream(_)) -> Ok(True)
    Error(error) -> Error(driver.ConnectionFailure(error))
  }
}
...
pub fn stop_sending(
  ...
  driver.update_connection(state.quic, fn(connection) {
    transport.stop_sending(connection, identifier, application_error_code)
  })
  |> result.map(fn(quic) { State(..state, quic: quic) })
}
...
pub fn guaranteed_datagram_size(state: State) -> Result(Int, driver.Error) {
  transport.guaranteed_datagram_data_size(driver.connection(state.quic))
  |> result.map_error(driver.ConnectionFailure)
}
...
pub fn pmtu_probe_outstanding(state: State) -> Bool {
  transport.pmtu_probe_outstanding(driver.connection(state.quic))
}

pub fn pmtu_probe_available(state: State) -> Bool {
  transport.pmtu_probe_available(driver.connection(state.quic))
}
...
pub fn active_stream_count(state: State) -> Int {
  transport.active_stream_count(driver.connection(state.quic))
}

/// Current transport stream identifiers for internal diagnostics.
pub fn stream_identifiers(state: State) -> List(Int) {
  transport.stream_identifiers(driver.connection(state.quic))
}

/// Payload-free send and recovery state for internal diagnostics.
pub fn send_progress(state: State) -> transport.SendProgress {
  transport.send_progress(driver.connection(state.quic))
}
...
pub fn stream_send_progress(
  state: State,
  identifier: Int,
) -> Result(stream_state.SendProgress, driver.Error) {
  transport.stream_send_progress(driver.connection(state.quic), identifier)
  |> result.map_error(driver.ConnectionFailure)
}
