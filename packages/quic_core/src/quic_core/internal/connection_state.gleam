...
import quic_core/internal/aead_usage
import quic_core/internal/amplification
import quic_core/internal/connection_id
import quic_core/internal/cubic
import quic_core/internal/ecn
import quic_core/internal/flow_control
import quic_core/internal/initial_crypto
import quic_core/internal/key_phase
import quic_core/internal/new_reno
import quic_core/internal/pacer
import quic_core/internal/packet_space
import quic_core/internal/path_validation
import quic_core/internal/pmtu
import quic_core/internal/reassembler
...
import quic_core/internal/tls/session_ticket
...
// RFC 9000 section 20: TRANSPORT_PARAMETER_ERROR. A transport-parameter
// failure is not attributable to a QUIC frame, so its CONNECTION_CLOSE frame
// type is zero.
const transport_parameter_error_code = 0x08

const version_negotiation_error_code = 0x11

// RFC 9001 section 4.8 carries TLS alert descriptions in QUIC's CRYPTO_ERROR
// range. RFC 7301 assigns no_application_protocol alert description 120.
const no_application_protocol_error_code = 0x178
...
/// Payload-free send and recovery state for developer diagnostics.
///
/// Stream identifiers are ordered exactly as the round-robin scheduler will
/// visit them. The snapshot contains no frame or application payload.
pub type SendProgress {
  SendProgress(
    connection_peer_limit: Int,
    connection_sent_bytes: Int,
    pto_backoff_count: Int,
    application_outstanding_packets: Int,
    application_outstanding: List(OutstandingPacketProgress),
    application_largest_acknowledged: Option(Int),
    application_next_packet_number: Int,
    application_recovery_deadline: Option(Int),
    application_ack_deadline: Option(Int),
    queued_application_frames: Int,
    active_streams: List(Int),
  )
}

/// Payload-free summary of one outstanding packet and its STREAM frames.
pub type OutstandingPacketProgress {
  OutstandingPacketProgress(
    packet_number: Int,
    sent_at_milliseconds: Int,
    sent_bytes: Int,
    stream_frames: List(StreamFrameProgress),
    other_frames: Int,
  )
}

/// Payload-free STREAM frame metadata retained by recovery.
pub type StreamFrameProgress {
  StreamFrameProgress(identifier: Int, offset: Int, bytes: Int, finished: Bool)
}

/// Why an owner did or did not arm a pacing wake for pending output.
///
/// This is deliberately payload-free. The candidate is only a byte count and
/// the remaining fields are the bounded controller inputs needed to reproduce
/// the decision without retaining a packet or frame.
pub type PacingWakeProgress {
  NoPacingOutput
  PacingReady(
    candidate_datagram_bytes: Int,
    path_mtu: Int,
...
/// The sole wire response retained while this endpoint is closing.
///
/// RFC 9000 section 10.2.2 forbids every other frame class in this phase. The
/// encryption level is retained with the already-bounded frame so an
/// authenticated peer retransmission can re-arm exactly the same response
/// without retaining peer-controlled input or reconstructing a reason.
type CloseResponse {
  CloseResponse(level: engine.EncryptionLevel, frame: frame.Frame)
}
...
/// Round-robin queue of streams that can produce transport output.
///
/// Front and back stacks make dequeue and enqueue amortized O(1). The set
/// prevents duplicate queue entries while a stream remains active; the count
/// makes the common pending-output check independent of the number of streams.
type ActiveStreams {
  ActiveStreams(
    front: List(Int),
    back: List(Int),
...
pub opaque type State {
  State(
    config: Config,
    // RFC 9369 section 4.1 keeps 0-RTT on the version that produced its
    // resumption keys even if compatible negotiation changes Initial,
    // Handshake, and 1-RTT packets to another version.
    zero_rtt_version: Version,
    ...
    // The server limit authenticated with the ticket and used to admit
    // client 0-RTT DATAGRAM frames. The first fresh transport-parameter set
    // must preserve or increase it before this guard is cleared.
    remembered_peer_maximum_datagram_frame_size: Option(Int),
...
fn active_streams_new() -> ActiveStreams {
  ActiveStreams([], [], set.new(), 0)
}

fn active_streams_push(queue: ActiveStreams, identifier: Int) -> ActiveStreams {
  case set.contains(queue.members, identifier) {
    True -> queue
    False ->
      ActiveStreams(
        ..queue,
        back: [identifier, ..queue.back],
        members: set.insert(queue.members, identifier),
        count: queue.count + 1,
      )
  }
}

fn active_streams_remove(
  queue: ActiveStreams,
  identifier: Int,
) -> ActiveStreams {
  case set.contains(queue.members, identifier) {
    ...
    True ->
      ActiveStreams(
        front: list.filter(queue.front, fn(value) { value != identifier }),
        back: list.filter(queue.back, fn(value) { value != identifier }),
        members: set.delete(queue.members, identifier),
        count: queue.count - 1,
      )
  }
}

fn active_streams_pop(
  queue: ActiveStreams,
) -> Result(#(Int, ActiveStreams), Nil) {
  case queue.front, queue.back {
    ...
    [], back ->
      active_streams_pop(
        ActiveStreams(..queue, front: list.reverse(back), back: []),
      )
  }
}
...
pub type Error {
  ...
  /// An authenticated frame appeared at an encryption level where RFC 9000
  /// forbids it. Only finite frame and level classes are retained.
  FrameAtWrongEncryptionLevel(engine.EncryptionLevel, FrameKind)
  CongestionLimited
  PacingLimited(Int)
  RecoveryLimited
...
/// Stable, payload-free cause of an authenticated QUIC protocol violation.
///
/// Values identify only the finite state transition which rejected input.
/// They never retain frame contents, connection identifiers, peer-supplied
/// numbers, TLS material, or implementation errors.
pub type ProtocolViolationReason {
  PacketVersionMismatch
  KeyPhaseCandidateRejected
  LongPacketHeaderMismatch
  CryptoSendAtZeroRtt
  EarlyDatagramLimitReduced
  InvalidPeerAckDelay
  InvalidPeerUdpPayloadSize
  NegativePacketTimestamp
  HandshakeFailureCloseAtZeroRtt
  CryptoFrameAtZeroRtt
  CryptoReassemblyInsertFailure
  CryptoReassemblyReadFailure
  StreamFrameDirectionViolation
  ResetStreamDirectionViolation
  StopSendingDirectionViolation
  MaxStreamDataDirectionViolation
  PathResponseQueueLimitExceeded
  HandshakeDoneStateViolation
  ServerEndpointClientConfirmation
  DatagramNotNegotiatedViolation
  DatagramFrameTooLargeViolation
  PeerConnectionIdRegistryMissing
  RemoteStreamIdentifierInvalid
  RemoteUsedLocalStream
  RemoteStreamIndexRegression
  StreamIdentifierEncodeFailure
  AckEcnRangesEmpty
  AckEcnValidationFailure
  ReservedBitsViolation
}

/// Payload-free QUIC frame class used by internal failure diagnostics.
pub type FrameKind {
  PaddingFrame
  PingFrame
  AckFrame
...
pub fn new(config: Config, now_milliseconds: Int) -> Result(State, Error) {
  ...
  Ok(State(
    config: config,
    zero_rtt_version: config.version,
    ...
    application_queue: [],
    streams: dict.new(),
    active_streams: active_streams_new(),
    ...
    remembered_peer_maximum_datagram_frame_size: None,
...
pub fn validate_send_budget(
  ...
  case datagram_bytes > 0 && now_milliseconds >= 0 {
    ...
    True -> {
      ...
      let pto_probe = ack_eliciting && state.pto_probe_allowance > 0
      use _ <- result.try(validate_recovery_capacity(
        state,
        level,
        in_flight,
        pto_probe,
...
/// Retire an unsent DPLPMTUD probe rejected by the local non-fragmenting
/// socket and exclude its size from the remaining search range.
pub fn reject_pmtu_probe(state: State, size: Int) -> Result(State, Error) {
  case pmtu.probe_too_large(state.pmtu, size) {
...
/// Whether one exact DPLPMTUD probe is currently awaiting acknowledgement or
/// loss processing.
pub fn pmtu_probe_outstanding(state: State) -> Bool {
  option.is_some(pmtu.outstanding_probe(state.pmtu))
}

/// Whether congestion credit currently permits a probe larger than the
/// confirmed path size.
pub fn pmtu_probe_available(state: State) -> Bool {
  pmtu.probe_available(state.pmtu, probe_allowance(state))
}
...
pub fn receive_protected_long_packet(
  ...
  case packet_version == state.config.version {
    False -> Error(ProtocolViolation(PacketVersionMismatch))
    True -> {
...
/// Open the next locally initiated bidirectional stream without exporting the
/// transport's direction type across a protocol-package boundary.
pub fn open_bidirectional_stream(state: State) -> Result(#(State, Int), Error) {
  open_stream(state, stream_id.Bidirectional)
}

/// Open the next locally initiated unidirectional stream without exporting the
/// transport's direction type across a protocol-package boundary.
pub fn open_unidirectional_stream(
  state: State,
) -> Result(#(State, Int), Error) {
  open_stream(state, stream_id.Unidirectional)
}
      ...
      case dict.get(state.streams, identifier) {
        ...
        Ok(stream) ->
          case stream_state.queue_send(stream, data, fin) {
            ...
            Ok(updated) -> Ok(put_stream(state, identifier, updated))
          }
      }
    _, _ -> Error(ConnectionUnavailable)
  }
...
/// Store one stream and synchronize its round-robin send activity.
///
/// Every stream mutation flows through this helper. Activation appends once,
/// preserving round-robin order; deactivation removes the member immediately
/// so deadline checks stay O(1) and a later reactivation cannot duplicate it.
fn put_stream(
  state: State,
  identifier: Int,
  stream: stream_state.State,
) -> State {
  let active_streams = case stream_state.has_pending_output(stream) {
    True -> active_streams_push(state.active_streams, identifier)
    False -> active_streams_remove(state.active_streams, identifier)
  }
  State(
    ..state,
...
/// Store a stream whose pending bytes cannot currently produce a frame.
///
/// A stream that already reported STREAM_DATA_BLOCKED returns `SendDeferred`
/// until MAX_STREAM_DATA raises its peer limit. Leaving it in the active queue
/// would make that dormant stream terminate every packet poll before a later
/// ready stream gets its round-robin turn. The MAX_STREAM_DATA path uses
/// `put_stream`, so new credit reactivates it at the queue tail.
fn put_dormant_stream(
  state: State,
  identifier: Int,
  stream: stream_state.State,
) -> State {
  State(
    ..state,
    streams: dict.insert(state.streams, identifier, stream),
    active_streams: active_streams_remove(state.active_streams, identifier),
  )
}
...
///
/// Unlike `abort_stream`, this queues STOP_SENDING without resetting the
/// local send direction, so a bidirectional application protocol can reject
/// an inbound body and still finish its outbound response.
pub fn stop_sending(
  state: State,
  identifier: Int,
  application_error_code: Int,
    ...
    && stream_id.can_receive(identifier, local_initiator(state))
  {
    ...
    True -> {
      ...
      Ok(queue_application_frame(
        state,
        frame.StopSending(identifier, application_error_code),
      ))
    }
  }
}
...
/// Return whether this stream's FIN was acknowledged or its send direction
/// was reset. This is the transport-settled condition applications need before
/// gracefully closing a connection that has just emitted its final response.
pub fn stream_send_finished(
  ...
  case dict.get(state.streams, identifier) {
    Error(_) -> Error(UnknownStream(identifier))
    Ok(stream) -> Ok(stream_state.send_finished(stream))
  }
}
...
/// Return a raw QUIC DATAGRAM payload ceiling that remains valid for this
/// connection after ACK debt grows or the validated path falls back to the
/// mandatory 1200-byte floor.
///
/// This is deliberately smaller than the live maximum on a healthy path. It
/// reserves the largest ACK share queue admission can charge (half a path),
/// the widest 1-RTT protection overhead, and the DATAGRAM frame header. Peer
/// transport parameters are immutable after authentication, so no later
/// connection state can reduce this bound.
pub fn guaranteed_datagram_data_size(state: State) -> Result(Int, Error) {
  case state.peer_maximum_datagram_frame_size {
    0 -> Error(DatagramNotNegotiated)
    _ -> {
      let frame_limit = guaranteed_datagram_frame_limit(state)
      datagram_payload_for_frame_limit(frame_limit, frame_limit - 2)
    }
  }
}
...
/// The QUIC DATAGRAM frame width guaranteed across this connection's path and
/// ACK state transitions.
///
/// Every path carries 1200 bytes. An established packet spends at most the
/// widest short-header protection overhead plus half of its path on a
/// coalesced ACK; a larger ACK takes its own packet. A 0-RTT long header is
/// much smaller than that combined reservation, so the established bound is
/// also safe when queried during an early-data attempt.
fn guaranteed_datagram_frame_limit(state: State) -> Int {
  let floor =
    minimum_datagram_bytes
    - wire_packet.maximum_short_packet_overhead()
    - { minimum_datagram_bytes / 2 }
  minimum(state.peer_maximum_datagram_frame_size, floor)
}
...
fn abort_established_stream(
  ...
  let state = put_stream(state, identifier, stream)
  Ok(
...
pub fn read_stream(
  ...
  case state.phase {
    Closing | Draining | Closed ->
      terminal_read_stream(state, identifier, maximum_bytes)
...
/// A terminal phase stops wire input and output, not delivery of bytes this
/// endpoint had already authenticated and acknowledged. Only an outcome which
/// is already decided is handed over: no further bytes can arrive, so a pull
/// with nothing buffered stays unavailable. The drain advertises no receive
/// credit because RFC 9000 section 10.2.2 permits no frame beside the retained
/// CONNECTION_CLOSE.
fn terminal_read_stream(
  state: State,
  identifier: Int,
  maximum_bytes: Int,
) -> Result(#(State, stream_state.ReadOutcome), Error) {
  case dict.get(state.streams, identifier) {
    ...
    Ok(stream) ->
      case stream_state.read(stream, maximum_bytes) {
        Error(_) -> Error(StreamFailure)
        Ok(stream_state.ReadPending(_)) -> Error(ConnectionUnavailable)
        Ok(outcome) -> apply_terminal_stream_read(state, identifier, outcome)
      }
  }
}
...
/// Return whether a pull needs more authenticated stream bytes.
pub fn read_is_pending(outcome: stream_state.ReadOutcome) -> Bool {
  case outcome {
    stream_state.ReadPending(_) -> True
    _ -> False
  }
}

/// Return whether a stream had already delivered its final byte.
pub fn read_is_finished(outcome: stream_state.ReadOutcome) -> Bool {
  case outcome {
    stream_state.ReadFinished(_) -> True
    _ -> False
  }
}

/// Return a peer application reset code without exposing stream state.
pub fn read_reset_code(outcome: stream_state.ReadOutcome) -> Option(Int) {
  case outcome {
    stream_state.ReadReset(_, application_error_code, _, _) ->
      Some(application_error_code)
    _ -> None
  }
}
...
/// since the last send pushes the wake later. A release that elapsed after the
/// owner's previous flush is returned as an immediate deadline; otherwise the
/// ready packet can be stranded until an unrelated ACK or PTO wakes the actor.
/// A completely occupied congestion window is excluded from that immediate
/// retry, because pacer credit cannot make it sendable and a zero-delay wake
/// would spin until recovery makes room.
///
/// Sizing that datagram re-derives the acknowledgement this connection owes
/// and walks only the active streams the send path would draw from, so one
/// wakeup costs O(active streams + retained ACK ranges). Both are bounded by
/// limits this endpoint sets and the peer cannot raise --
/// `maximum_total_streams`, the peer stream limits it advertises, and
/// `maximum_ack_ranges` -- so the work per wakeup has a fixed ceiling and is
/// not worth memoising against state that changes on every packet.
pub fn next_deadline(
  ...
  case now_milliseconds < state.last_activity_milliseconds, state.phase {
    ...
    False, Handshaking | False, Established -> {
      ...
      use anti_deadlock <- result.try(client_anti_deadlock_deadline(state))
      let deadline =
        Some(
        ...
        |> earlier_deadline(anti_deadlock)
...
/// Current stream identifiers for bounded internal diagnostics.
pub fn stream_identifiers(state: State) -> List(Int) {
  dict.keys(state.streams)
}
...
/// Return the connection-wide exponential PTO backoff exponent.
///
/// This is a bounded, payload-free recovery diagnostic. Packet spaces retain
/// independent loss timers, but consecutive PTOs share this exponent.
pub fn pto_backoff_count(state: State) -> Int {
  connection_probe_timeout_count(state)
}

/// Snapshot the connection-wide state that decides whether stream output can
/// progress, without exposing frames, packet bytes, or cryptographic state.
pub fn send_progress(state: State) -> SendProgress {
  SendProgress(
    connection_peer_limit: flow_control.sender_limit(state.connection_sender),
    connection_sent_bytes: flow_control.sent_bytes(state.connection_sender),
    pto_backoff_count: connection_probe_timeout_count(state),
    application_outstanding_packets: packet_space.outstanding_count(
      state.application_space,
    ),
    application_outstanding: outstanding_packet_progress(
      packet_space.outstanding_packets(state.application_space),
      [],
    ),
    application_largest_acknowledged: packet_space.largest_acknowledged(
      state.application_space,
    ),
    application_next_packet_number: packet_space.next_packet_number(
      state.application_space,
    ),
    application_recovery_deadline: packet_space.timer_deadline(
      state.application_space,
      state.estimator,
      ...
      |> result.unwrap(None),
    application_ack_deadline: packet_space.ack_deadline(state.application_space),
    queued_application_frames: list.length(state.application_queue),
    active_streams: list.append(
      state.active_streams.front,
      list.reverse(state.active_streams.back),
    ),
  )
}

fn outstanding_packet_progress(
  packets: List(packet_space.SentPacket),
  accumulated: List(OutstandingPacketProgress),
) -> List(OutstandingPacketProgress) {
  case packets {
    [] -> list.reverse(accumulated)
    [
      packet_space.SentPacket(number, sent_at, _, _, bytes, frames, _, _),
      ..rest
    ] -> {
      let #(stream_frames, other_frames) = frame_progress(frames, [], 0)
      outstanding_packet_progress(rest, [
        OutstandingPacketProgress(
          packet_number: number,
          sent_at_milliseconds: sent_at,
          sent_bytes: bytes,
...
fn frame_progress(
  frames: List(frame.Frame),
  streams: List(StreamFrameProgress),
  other: Int,
) -> #(List(StreamFrameProgress), Int) {
  case frames {
    [] -> #(list.reverse(streams), other)
    [frame.Stream(identifier, offset, data, finished), ..rest] ->
      frame_progress(
        rest,
        [
          StreamFrameProgress(
            ...
            offset: offset,
            bytes: bit_array.byte_size(data),
            finished: finished,
          ),
          ..streams
        ],
    ...
    [_, ..rest] -> frame_progress(rest, streams, other + 1)
  }
}

/// Snapshot one stream's send-side progress without copying its payload.
pub fn stream_send_progress(
  state: State,
  identifier: Int,
) -> Result(stream_state.SendProgress, Error) {
  case dict.get(state.streams, identifier) {
    Error(Nil) -> Error(UnknownStream(identifier))
    Ok(stream) -> Ok(stream_state.send_progress(stream))
  }
}
...
fn packet_space_deadline(
  ...
  case amplification.can_send(state.amplification, 1) {
    False -> Ok(None)
    True -> {
      let recovery_deadline = case
        level == engine.OneRtt && !state.handshake_confirmed
      {
        True -> Ok(None)
        False ->
      ...
      use recovery_deadline <- result.try(recovery_deadline)
      Ok(earlier_deadline(recovery_deadline, packet_space.ack_deadline(space)))
    }
  }
}

/// RFC 9002 section 6.2.2.1 keeps a client probing before it knows the server
/// validated its address, even when no packet remains in flight.
fn client_anti_deadlock_deadline(state: State) -> Result(Option(Int), Error) {
  let level = anti_deadlock_probe_level(state)
  case
    state.config.role == Client
    && !state.handshake_confirmed
    && packet_space.largest_acknowledged(state.handshake_space) == None,
    level
  {
    True, Some(level) -> {
      ...
      use duration <- result.try(
        rtt.probe_timeout(
          ...
          0,
          False,
          packet_space.probe_timeout_count(space),
          timer_granularity_milliseconds,
        )
        |> result.map_error(fn(_) { PacketSpaceFailure }),
      )
      Ok(Some(state.last_activity_milliseconds + duration))
    }
    _, _ -> Ok(None)
  }
}

fn anti_deadlock_probe_level(state: State) -> Option(engine.EncryptionLevel) {
  case
    keys_available(state, engine.Handshake, Write),
    keys_available(state, engine.Initial, Write)
  {
    True, _ -> Some(engine.Handshake)
    False, True -> Some(engine.Initial)
    False, False -> None
  }
}
...
fn pending_pacer_release(state: State, now_milliseconds: Int) -> Option(Int) {
  case pacing_wake_progress(state, now_milliseconds) {
    PacingWaiting(deadline_milliseconds: deadline, ..) -> Some(deadline)
    PacingReady(..) ->
      case pacing_retry_has_congestion_room(state) {
        True -> Some(now_milliseconds)
        False -> None
      }
    NoPacingOutput | PacingDecisionInvalid(..) -> None
  }
}

/// Whether an already pacing-ready retry can make congestion progress.
///
/// The send path splits STREAM and CRYPTO data down to its fixed pre-path
/// budget, so one 1200-byte unit of remaining window is sufficient regardless
/// of a larger validated PMTU. Smaller queued control frames report their
/// smaller candidate instead. A PTO allowance bypasses congestion in the
/// commit path and therefore remains immediately actionable even above cwnd.
fn pacing_retry_has_congestion_room(state: State) -> Bool {
  let required = minimum(minimum_datagram_bytes, pending_datagram_bytes(state))
  state.pto_probe_allowance > 0
  || bytes_in_flight(state) + required <= congestion_window(state)
}

/// Explain the current pacing-wake decision without exposing queued data.
///
/// The same decision feeds `next_deadline`, so a live actor snapshot describes
/// the timer it actually used rather than a parallel diagnostic approximation.
pub fn pacing_wake_progress(
  state: State,
  now_milliseconds: Int,
) -> PacingWakeProgress {
  case has_pending_output(state) {
    False -> NoPacingOutput
    True -> {
      let candidate = pending_datagram_bytes(state)
      let path = path_mtu(state)
      let window = pacing_window(state)
      let in_flight = bytes_in_flight(state)
      let rtt.Snapshot(_, smoothed, _, _) = rtt.snapshot(state.estimator)
      case ask_pacer(state, candidate, now_milliseconds) {
        Ok(pacer.Decision(_, pacer.WaitUntil(deadline))) ->
          PacingWaiting(candidate, deadline, path, window, in_flight, smoothed)
        Ok(pacer.Decision(_, pacer.SendNow)) ->
          PacingReady(candidate, path, window, in_flight, smoothed)
        // Invalid size, controller input, or monotonic clock. No such outcome
        // is safe to silently confuse with "nothing pending" in diagnostics.
        Error(pacer.InvalidInput) ->
          PacingDecisionInvalid(candidate, path, window, in_flight, smoothed)
      }
    }
  }
}
...
fn pending_stream_bytes(state: State) -> Int {
  let largest =
    list.fold(state.active_streams.front, 0, fn(largest, identifier) {
      case dict.get(state.streams, identifier) {
        Error(Nil) -> largest
        Ok(stream) ->
          maximum(
  ...
  list.fold(state.active_streams.back, largest, fn(largest, identifier) {
    case dict.get(state.streams, identifier) {
      Error(Nil) -> largest
      Ok(stream) ->
        maximum(
...
/// active-stream queue. Stream state transitions keep that queue synchronized,
/// so bytes only awaiting acknowledgment never count as output. Acknowledgments
/// are not paced and carry their own packet-space deadlines, so they are
...
fn apply_authenticated_key_phase(
  ...
  case candidate_kind_is_allowed(allowed, candidate_kind) {
    False -> Error(ProtocolViolation(KeyPhaseCandidateRejected))
    True ->
...
fn protect_long_payload(
  ...
  let protocol_version = case kind {
    wire_packet.ZeroRtt -> state.zero_rtt_version
    wire_packet.Initial(_) | wire_packet.Handshake -> state.config.version
  }
  let protected =
...
fn finish_long_packet(
  ...
  {
    False -> Error(ProtocolViolation(LongPacketHeaderMismatch))
    True ->
...
fn apply_tls_action(
  ...
  case action {
    ...
    engine.DiscardKeys(level) -> discard_level(state, level)
    engine.PeerTransportParameters(parameters) ->
      apply_authenticated_peer_parameters(state, parameters)
...
fn install_traffic_keys(
  ...
  let state = case state.config.role, level, direction {
    // The remembered peer parameters are applied immediately before the
    // client 0-RTT write keys. Snapshot the DATAGRAM limit at that boundary;
    // the authenticated parameters in EncryptedExtensions are a distinct,
    // later PeerTransportParameters action.
    Client, engine.ZeroRtt, Write ->
      State(
        ..state,
        remembered_peer_maximum_datagram_frame_size: Some(
...
fn discard_level(
  ...
  case level {
    engine.Initial -> {
      use state <- result.try(abandon_packet_space_congestion(
        state,
        state.initial_space,
      ))
      Ok(
...
/// Remove every congestion-controlled byte whose recovery metadata becomes
/// unusable with discarded packet-protection keys.
///
/// Packet-space discard and congestion accounting are one transition. Doing
/// only the former leaves an irrecoverable flight permanently occupying cwnd,
/// even though no future ACK can name those packets.
fn abandon_packet_space_congestion(
  state: State,
  space: packet_space.State,
) -> Result(State, Error) {
  let bytes =
    packet_space.outstanding_packets(space)
    |> in_flight_packet_bytes(0)
  use congestion <- result.try(abandon_early_congestion(state.congestion, bytes))
...
fn queue_aligned_crypto(
  ...
  case level {
    ...
    engine.ZeroRtt -> Error(ProtocolViolation(CryptoSendAtZeroRtt))
  }
}
...
/// Apply one authenticated peer transport-parameter set, enforcing the
/// RFC 9221 0-RTT monotonicity rule before any remembered value can be
/// overwritten. Absence is the protocol-defined zero and therefore also a
/// downgrade when the ticket remembered DATAGRAM support.
fn apply_authenticated_peer_parameters(
  state: State,
  parameters: List(transport_parameter.Parameter),
) -> Result(State, Error) {
  let current_datagram_limit = peer_datagram_limit(parameters)
  case state.remembered_peer_maximum_datagram_frame_size {
    Some(remembered) if current_datagram_limit < remembered ->
      Error(ProtocolViolation(EarlyDatagramLimitReduced))
    remembered -> {
      use applied <- result.try(apply_peer_parameters(state, parameters))
      let applied = case remembered {
        Some(_) ->
          State(..applied, remembered_peer_maximum_datagram_frame_size: None)
        None -> applied
      }
      Ok(applied)
    }
...
fn peer_datagram_limit(parameters: List(transport_parameter.Parameter)) -> Int {
  case parameters {
    [] -> 0
    [transport_parameter.MaxDatagramFrameSize(value), ..] -> value
    [_, ..rest] -> peer_datagram_limit(rest)
  }
}
...
fn apply_peer_parameter(
  ...
  case parameter {
    ...
    transport_parameter.MaxAckDelay(delay) ->
      ...
      {
        Error(_) -> Error(ProtocolViolation(InvalidPeerAckDelay))
    ...
    transport_parameter.MaxUdpPayloadSize(size) ->
      case pmtu.set_peer_maximum(state.pmtu, size) {
        Error(_) -> Error(ProtocolViolation(InvalidPeerUdpPayloadSize))
        Ok(path_mtu) ->
          Ok(put_path_mtu(
...
fn prepare_sendable_packet(
  ...
  case discarded, writable {
    ...
    False, True -> {
      // RFC 9000 section 10.2.2 permits only CONNECTION_CLOSE while this
      // endpoint is Closing. Packet-space acknowledgement state remains
      // bounded and is discarded at the fixed close deadline, but it must
      // never escape beside or instead of the retained close response.
      use #(acked, acknowledgement) <- result.try(case state.phase {
        Closing -> Ok(#(state, None))
        _ -> take_due_ack(state, level, now_milliseconds)
      })
...
fn take_outgoing_frame(
  ...
  case state.phase {
    Closing -> take_closing_frame(state, level, maximum_data_bytes)
    Handshaking | Established ->
      take_live_outgoing_frame(state, level, maximum_data_bytes)
    Draining | Closed -> Ok(#(state, None))
  }
}

fn take_live_outgoing_frame(
  state: State,
  level: engine.EncryptionLevel,
  maximum_data_bytes: Int,
) -> Result(#(State, Option(frame.Frame)), Error) {
...
fn take_closing_frame(
  state: State,
  level: engine.EncryptionLevel,
  maximum_data_bytes: Int,
) -> Result(#(State, Option(frame.Frame)), Error) {
  let queue = queue_for_level(state, level)
  case queue {
    [frame.ConnectionCloseTransport(_, _, _), ..]
    | [frame.ConnectionCloseApplication(_, _), ..] ->
      take_queue_head(state, level, queue, maximum_data_bytes)
    _ -> Ok(#(state, None))
  }
}
...
fn queue_for_level(
  ...
  case level {
    engine.Initial -> state.initial_queue
    engine.Handshake -> state.handshake_queue
    engine.ZeroRtt -> state.zero_rtt_queue
    engine.OneRtt -> state.application_queue
  }
}
...
fn poll_stream_frame(
  ...
  case active_streams_pop(state.active_streams) {
    Error(Nil) -> Ok(#(state, None))
    Ok(#(identifier, active_streams)) -> {
      let state = State(..state, active_streams: active_streams)
      case dict.get(state.streams, identifier) {
        Error(Nil) -> poll_stream_frame(state, maximum_data_bytes)
        Ok(stream) ->
          handle_stream_poll(state, stream, identifier, maximum_data_bytes)
      }
    }
  }
}
...
fn handle_stream_poll(
  ...
  case stream_state.poll_send(original_stream, maximum_data_bytes) {
    Ok(stream_state.Emit(stream, emitted)) ->
      complete_stream_emit(state, identifier, original_stream, stream, emitted)
    Ok(stream_state.SendBlocked(stream, limit)) ->
      Ok(#(
        put_stream(state, identifier, stream),
        Some(frame.StreamDataBlocked(identifier, limit)),
      ))
    Ok(stream_state.SendDeferred(stream)) ->
      poll_stream_frame(
        put_dormant_stream(state, identifier, stream),
        maximum_data_bytes,
      )
...
fn complete_stream_emit(
  ...
  case flow_control.reserve(state.connection_sender, newly_sent) {
    Ok(sender) ->
      Ok(#(
        put_stream(
          State(..state, connection_sender: sender),
          identifier,
          stream,
        ),
        Some(emitted),
      ))
    Error(flow_control.FlowControlBlocked(_)) -> {
      let #(sender, blocked) =
        flow_control.take_blocked_limit(state.connection_sender)
      let state =
        put_stream(
          State(..state, connection_sender: sender),
          identifier,
      ...
      case blocked {
        Some(limit) -> Ok(#(state, Some(frame.DataBlocked(limit))))
        None -> Ok(#(state, None))
      }
    }
  ...
  let recorded = case pto_probe {
    True ->
      packet_space.record_probe_sent(
        ...
        frames,
        codepoint,
        rtt.has_sample(state.estimator),
      )
    False ->
    ...
    Ok(#(space, _)) -> {
      ...
      let sent =
        State(
          ...
          amplification: amplification,
          ecn: ecn_state,
          pto_probe_allowance: consume_probe_allowance(
            state.pto_probe_allowance,
            pto_probe,
          ),
...
fn check_and_record_congestion_send(
  ...
  case congestion {
    RenoState(state) ->
      case !in_flight || pto_probe || new_reno.can_send(state, datagram_bytes) {
    ...
    CubicState(state) ->
      case !in_flight || pto_probe || cubic.can_send(state, datagram_bytes) {
        False -> Error(CongestionLimited)
        True ->
...
fn consume_probe_allowance(allowance: Int, pto_probe: Bool) -> Int {
  case pto_probe && allowance > 0 {
    True -> allowance - 1
    False -> allowance
  }
}

fn grant_probe_allowance(state: State, probes: Int) -> State {
  State(
    ..state,
    pto_probe_allowance: minimum(2, state.pto_probe_allowance + probes),
  )
}
...
/// The congestion-controller rate input supplied to the live pacer. CUBIC can
/// reduce this below cwnd after persistent CE at its one-datagram floor.
fn pacing_window(state: State) -> Int {
  case state.congestion {
    RenoState(reno) -> new_reno.congestion_window(reno)
    CubicState(cubic_state) -> cubic.pacing_window(cubic_state)
  }
}
...
fn receive_live_packet(
  ...
  case discarded, readable, now_milliseconds >= 0 {
    True, _, _ -> Error(SpaceUnavailable)
    False, False, _ -> Error(MissingReadKeys(level))
    False, True, False -> Error(ProtocolViolation(NegativePacketTimestamp))
    False, True, True ->
      case first_invalid_frame_at_level(frames, level) {
        Some(kind) -> Error(FrameAtWrongEncryptionLevel(level, kind))
        None ->
...
fn receive_valid_packet(
    ...
    )
  {
    ...
    Ok(packet_space.Duplicate(space)) -> {
      let state = put_packet_space(state, level, space)
      case state.phase {
        Closing -> receive_closing_packet(state, frames, now_milliseconds)
        _ -> Ok(state)
      }
    }
    Ok(packet_space.Accepted(space, _)) -> {
      ...
      case state.phase {
        Closing -> receive_closing_packet(state, frames, now_milliseconds)
        _ -> process_frames(state, level, frames, now_milliseconds)
      }
    }
  }
}

/// Ignore ordinary peer frames after a local close and re-arm only the
/// retained close response. A peer CONNECTION_CLOSE instead moves this side
/// to Draining, where no packets at all may be sent. The original close
/// deadline is deliberately left unchanged, so inbound traffic cannot retain
/// actor, socket, keys, or qlog state past the fixed bound.
fn receive_closing_packet(
  state: State,
  frames: List(frame.Frame),
  now_milliseconds: Int,
) -> Result(State, Error) {
  case peer_connection_close(frames) {
    Some(#(error_code, reason)) ->
      Ok(enter_draining(state, error_code, reason, now_milliseconds))
    None -> Ok(rearm_close_response(state))
  }
}

fn peer_connection_close(frames: List(frame.Frame)) -> Option(#(Int, String)) {
  case frames {
    [] -> None
    ...
    [frame.ConnectionCloseApplication(error_code, reason), ..] ->
      Some(#(error_code, reason))
    [_, ..rest] -> peer_connection_close(rest)
  }
}

/// At most one close response may wait in the bounded output queue. This
/// naturally rate-limits responses to authenticated peer packets: another
/// packet cannot add another response until the prior one was drawn.
fn rearm_close_response(state: State) -> State {
  case state.close_response {
    None -> state
    Some(CloseResponse(level, response)) ->
      case queue_for_level(state, level) {
        [] -> put_queue(state, level, [response])
        _ -> state
      }
  }
}
...
fn first_invalid_frame_at_level(
  frames: List(frame.Frame),
  level: engine.EncryptionLevel,
) -> Option(FrameKind) {
  case frames {
    [] -> None
    [value, ..rest] ->
      case frame_valid_at_level(value, level) {
        True -> first_invalid_frame_at_level(rest, level)
        False -> Some(frame_kind(value))
      }
  }
}
...
fn frame_kind(value: frame.Frame) -> FrameKind {
  case value {
    frame.Padding(_) -> PaddingFrame
    frame.Ping -> PingFrame
    frame.Ack(_) -> AckFrame
    frame.ResetStream(_, _, _) -> ResetStreamFrame
    frame.StopSending(_, _) -> StopSendingFrame
    frame.Crypto(_, _) -> CryptoFrame
    frame.NewToken(_) -> NewTokenFrame
    frame.Stream(_, _, _, _) -> StreamFrame
    frame.MaxData(_) -> MaxDataFrame
    frame.MaxStreamData(_, _) -> MaxStreamDataFrame
    frame.MaxStreams(_, _) -> MaxStreamsFrame
    frame.DataBlocked(_) -> DataBlockedFrame
    frame.StreamDataBlocked(_, _) -> StreamDataBlockedFrame
    frame.StreamsBlocked(_, _) -> StreamsBlockedFrame
    frame.NewConnectionId(_, _, _, _) -> NewConnectionIdFrame
    frame.RetireConnectionId(_) -> RetireConnectionIdFrame
    frame.PathChallenge(_) -> PathChallengeFrame
    frame.PathResponse(_) -> PathResponseFrame
    frame.ConnectionCloseTransport(_, _, _) -> ConnectionCloseTransportFrame
    frame.ConnectionCloseApplication(_, _) -> ConnectionCloseApplicationFrame
    frame.HandshakeDone -> HandshakeDoneFrame
    frame.Datagram(_) -> DatagramFrame
  }
}
...
fn process_frames(
  ...
  case peer_connection_close(frames) {
    Some(#(error_code, reason)) ->
      Ok(enter_draining(state, error_code, reason, now_milliseconds))
    None -> process_non_close_frames(state, level, frames, now_milliseconds)
  }
}

fn process_non_close_frames(
  state: State,
  level: engine.EncryptionLevel,
  frames: List(frame.Frame),
  now_milliseconds: Int,
  ...
  case frames {
    ...
    [value, ..rest] -> {
      ...
      process_non_close_frames(state, level, rest, now_milliseconds)
    }
  }
}
...
fn receive_crypto(
  ...
  case level {
    engine.ZeroRtt -> Error(ProtocolViolation(CryptoFrameAtZeroRtt))
    engine.Initial ->
...
fn insert_and_read_crypto(
  ...
  case reassembler.insert(byte_stream, offset, data, False) {
    Error(_) -> Error(ProtocolViolation(CryptoReassemblyInsertFailure))
    Ok(byte_stream) ->
      case reassembler.read(byte_stream, maximum_crypto_buffer_bytes) {
        Error(_) -> Error(ProtocolViolation(CryptoReassemblyReadFailure))
...
fn deliver_crypto(
  ...
  case state.tls_endpoint {
    ...
    ClientTlsEndpoint(client) ->
      ...
      {
        Error(error) ->
          handle_tls_engine_failure(state, level, error, now_milliseconds)
...
fn handle_tls_engine_failure(
  ...
  case error {
    engine.NoApplicationProtocol ->
      close_handshake_transport_failure(
        state,
        level,
        no_application_protocol_error_code,
        now_milliseconds,
      )
    engine.TransportParameterFailure(_) ->
      close_transport_parameter_failure(state, level, now_milliseconds)
    engine.VersionNegotiationFailure ->
      close_version_negotiation_failure(state, level, now_milliseconds)
    _ -> Error(TlsFailure(error))
  }
}

/// Convert a peer transport-parameter failure into the wire-visible QUIC
/// connection error required by RFC 9000 and extension specifications.
///
/// A fatal handshake failure abandons every queued CRYPTO/application frame
/// and every active stream before placing one close at the encryption level
/// that authenticated the bad parameter. This prevents either role from
/// leaking later handshake or application output after it has rejected the
/// peer. ACK state is retained, so the close can still acknowledge the packet
/// that caused it without retaining any peer-controlled diagnostic payload.
fn close_transport_parameter_failure(
  state: State,
  level: engine.EncryptionLevel,
  now_milliseconds: Int,
) -> Result(State, Error) {
  close_handshake_transport_failure(
    state,
    level,
    transport_parameter_error_code,
    now_milliseconds,
  )
}

/// Authenticate a version mismatch but disclose no peer-controlled details.
fn close_version_negotiation_failure(
  state: State,
  level: engine.EncryptionLevel,
  now_milliseconds: Int,
) -> Result(State, Error) {
...
/// Atomically abandon pending work and queue one redacted handshake failure.
fn close_handshake_transport_failure(
  state: State,
  level: engine.EncryptionLevel,
  error_code: Int,
  now_milliseconds: Int,
) -> Result(State, Error) {
  case state.phase, level {
    Closed, _ | Draining, _ -> Error(ConnectionUnavailable)
    Closing, _ -> Ok(state)
    _, engine.ZeroRtt ->
      Error(ProtocolViolation(HandshakeFailureCloseAtZeroRtt))
    Handshaking, _ | Established, _ ->
      case keys_available(state, level, Write) {
        False -> Error(MissingWriteKeys(level))
        True -> {
          let response = frame.ConnectionCloseTransport(error_code, 0, "")
          let closing =
            State(
              ..state,
              phase: Closing,
              ...
              close_response: Some(CloseResponse(level, response)),
            )
          Ok(put_queue(closing, level, [response]))
        }
      }
  }
}
...
fn receive_stream_frame(
  ...
  {
    False -> Error(ProtocolViolation(StreamFrameDirectionViolation))
    True -> {
    ...
    Ok(#(stream, newly_received)) ->
      case flow_control.receive(state.connection_receiver, newly_received) {
        ...
        Ok(receiver) ->
          Ok(
            put_stream(
              State(..state, connection_receiver: receiver),
              identifier,
              stream,
            )
...
fn receive_stream_reset(
  ...
  case stream_id.can_receive(identifier, local_endpoint(state.config.role)) {
    False -> Error(ProtocolViolation(ResetStreamDirectionViolation))
    True -> {
      use state <- result.try(ensure_remote_stream(state, identifier))
      case dict.get(state.streams, identifier) {
        Error(Nil) ->
...
fn receive_stop_sending(
  ...
  case stream_id.can_send(identifier, local_endpoint(state.config.role)) {
    False -> Error(ProtocolViolation(StopSendingDirectionViolation))
    True -> {
      use state <- result.try(ensure_remote_stream(state, identifier))
      case dict.get(state.streams, identifier) {
        Error(Nil) ->
...
fn update_stream_send_limit(
  ...
  case stream_id.can_send(identifier, local_endpoint(state.config.role)) {
    False -> Error(ProtocolViolation(MaxStreamDataDirectionViolation))
    True -> {
      use state <- result.try(ensure_remote_stream(state, identifier))
      case dict.get(state.streams, identifier) {
        Error(Nil) ->
...
fn queue_path_response(
  ...
  case already_pending, pending_count >= maximum_pending_path_responses {
    True, _ -> Ok(state)
    False, True -> Error(ProtocolViolation(PathResponseQueueLimitExceeded))
    False, False ->
...
fn enter_draining(
  ...
  let close_deadline_milliseconds = case
    state.phase,
    state.close_deadline_milliseconds
  {
    Closing, Some(deadline) | Draining, Some(deadline) -> Some(deadline)
    _, _ -> Some(now_milliseconds + state.config.draining_timeout_milliseconds)
  }
  State(
    ..state,
    phase: Draining,
...
fn receive_handshake_done(state: State) -> Result(State, Error) {
  case state.config.role, state.phase {
    ...
    _, _ -> Error(ProtocolViolation(HandshakeDoneStateViolation))
  }
}
...
fn confirm_client_tls(state: State) -> Result(State, Error) {
  case state.tls_endpoint {
    NoTlsEndpoint -> {
      use state <- result.try(discard_level(state, engine.Handshake))
      Ok(
    ...
    ServerTlsEndpoint(_) ->
      Error(ProtocolViolation(ServerEndpointClientConfirmation))
  }
}
...
fn receive_datagram(state: State, data: BitArray) -> Result(State, Error) {
  ...
  case negotiated, frame_size <= state.config.maximum_datagram_frame_size {
    // These are authenticated peer violations. DatagramNotNegotiated and
    // DatagramTooLarge remain reserved for local send API refusals, where a
    // caller can recover by negotiating or resizing without terminating.
    False, _ -> Error(ProtocolViolation(DatagramNotNegotiatedViolation))
    True, False -> Error(ProtocolViolation(DatagramFrameTooLargeViolation))
...
fn receive_new_connection_id(
  ...
  case state.peer_connection_ids {
    None -> Error(ProtocolViolation(PeerConnectionIdRegistryMissing))
    Some(registry) -> {
...
fn create_missing_remote_streams(
  ...
  case stream_id.decode(identifier) {
    Error(_) -> Error(ProtocolViolation(RemoteStreamIdentifierInvalid))
    Ok(stream_id.StreamId(index, initiator, direction)) ->
      case initiator == local_endpoint(state.config.role) {
        True -> Error(ProtocolViolation(RemoteUsedLocalStream))
...
fn open_remote_through(
  ...
  case next_index > target_index {
    True -> Error(ProtocolViolation(RemoteStreamIndexRegression))
    False ->
...
fn encode_stream_identifier(
  ...
  case stream_id.encode(index, initiator, direction) {
    ...
    Error(_) -> Error(ProtocolViolation(StreamIdentifierEncodeFailure))
  }
}
      ...
      case stream_state.is_terminal(stream) {
        ...
        True ->
          State(
            ...
            active_streams: active_streams_remove(
...
/// Account a terminal drain without advertising receive credit. The application
/// queue is never prepared again in a terminal phase, so a MAX_DATA or
/// MAX_STREAM_DATA queued here could only become dead state, and RFC 9000
/// section 10.2.2 permits no frame beside the retained CONNECTION_CLOSE. This
/// is a separate path rather than a flag so the terminal drain cannot advertise
/// credit at all.
fn apply_terminal_stream_read(
  state: State,
  identifier: Int,
  outcome: stream_state.ReadOutcome,
) -> Result(#(State, stream_state.ReadOutcome), Error) {
  let #(stream, consumed, _) = read_effects(outcome)
  case flow_control.consume(state.connection_receiver, consumed) {
    Error(_) -> Error(FlowControlFailure)
    Ok(#(receiver, _)) -> {
      let state =
...
fn process_ack(
  ...
  let space = packet_space_for_level(state, level)
  let previous_pto_count = connection_probe_timeout_count(state)
  case
    packet_space.on_ack(
      space,
      acknowledgement,
    ...
    )
  {
    ...
    Ok(packet_space.AckOutcome(
      ...
      use ecn_result <- result.try(validate_ack_ecn(
        state.ecn,
        acknowledgement,
        acknowledged,
      ))
      let ecn.AckResult(ecn_state, newly_congestion_experienced) = ecn_result
      // RFC 9002 section 7.8 and RFC 9438 section 4.6 define application-
      // limited growth for the flight observed by this ACK. Snapshot it once:
      // sequentially removing the first acknowledged packet must not suppress
      // growth for the rest of the same full-flight ACK batch.
      let congestion_window_fully_utilized =
        bytes_in_flight(state) >= congestion_window(state)
      let state =
        put_packet_space(state, level, space)
        |> set_estimator(estimator)
      use state <- result.try(synchronize_pto_after_ack(
        state,
        level,
        acknowledged,
        previous_pto_count,
      ...
      use state <- result.try(apply_acknowledged_packets(
        ...
        acknowledged,
        now_milliseconds,
        newly_congestion_experienced > 0,
      ...
      use state <- result.try(finish_ecn_ack(
        state,
        newly_congestion_experienced,
        now_milliseconds,
      ))
      apply_persistent_congestion(
        state,
        persistent_congestion,
        now_milliseconds,
      )
...
fn apply_persistent_congestion(
  state: State,
  persistent_congestion: Bool,
  now_milliseconds: Int,
) -> Result(State, Error) {
  case persistent_congestion {
    False -> Ok(state)
    True -> {
      let congestion = case state.congestion {
        RenoState(reno) -> RenoState(new_reno.on_persistent_congestion(reno))
        CubicState(cubic_state) ->
          CubicState(cubic.on_persistent_congestion(cubic_state))
      }
      use pacing <- result.try(
        pacer.on_congestion_event(state.pacer, now_milliseconds)
      ...
      Ok(add_event(
        State(
          ..state,
          congestion: congestion,
          estimator: rtt.on_persistent_congestion(state.estimator),
          pacer: pacing,
        ),
        PersistentCongestionDetected,
      ))
...
fn apply_acknowledged_packets(
  ...
  packets: List(packet_space.SentPacket),
  now_milliseconds: Int,
  congestion_growth_limited: Bool,
...
/// a PING padded out to the size under test and nothing else. A PTO probe with
/// any delivery work has another shape, regardless of whether a PING was also
/// coalesced, so it keeps its ordinary congestion response even when its size
/// happens to match.
...
fn acknowledge_reno(
  ...
  case
    new_reno.on_packet_acked_in_batch(
      state,
      packet.sent_bytes,
...
fn acknowledge_cubic(
  ...
  case
    cubic.on_packet_acked_in_batch(
      state,
      packet.sent_bytes,
      packet.time_sent_milliseconds,
      now_milliseconds,
          ...
          case dict.get(state.streams, identifier) {
            ...
            Ok(stream) ->
              case stream_state.on_frame_lost(stream, value) {
                Error(_) -> Error(StreamFailure)
                Ok(stream) -> Ok(put_stream(state, identifier, stream))
              }
          }
      }
    frame.Padding(_)
...
) -> Result(ecn.AckResult, Error) {
  let frame.Acknowledgement(_, ranges, feedback) = acknowledgement
  case ranges {
    [] -> Error(ProtocolViolation(AckEcnRangesEmpty))
    [frame.AckRange(_, largest), ..] -> {
      let markings = acknowledged_markings(acknowledged, 0, 0)
      let feedback = map_ecn_feedback(feedback)
      case ecn.on_ack(ecn_state, largest, markings, feedback) {
        Error(_) -> Error(ProtocolViolation(AckEcnValidationFailure))
        Ok(result) -> Ok(result)
      }
    }
  }
...
fn congestion_experienced(
  ...
  let response = case state.congestion {
    RenoState(reno) ->
      case
        new_reno.on_packet_lost(reno, 0, now_milliseconds, now_milliseconds)
      {
        Ok(updated) ->
          Ok(#(
            RenoState(updated),
            new_reno.congestion_window(updated)
              < new_reno.congestion_window(reno),
          ))
        Error(_) -> Error(InvalidInput)
      }
    CubicState(cubic_state) ->
      case cubic.on_congestion_experienced(cubic_state, now_milliseconds) {
        Ok(updated) ->
          Ok(#(
            CubicState(updated),
            cubic.pacing_window(updated) < cubic.pacing_window(cubic_state),
          ))
        Error(_) -> Error(InvalidInput)
      }
  }
  use #(congestion, pacing_reduced) <- result.try(response)
  let state = State(..state, congestion: congestion)
  case pacing_reduced {
    False -> Ok(state)
    True ->
      case pacer.on_congestion_event(state.pacer, now_milliseconds) {
        Ok(pacing) -> Ok(State(..state, pacer: pacing))
        Error(_) -> Error(InvalidInput)
      }
  }
}
    ...
    Handshaking, True | Established, _ -> {
      ...
      Ok(
        State(
          ...
          handshake_queue: [],
          zero_rtt_queue: [],
          application_queue: [response],
          active_streams: active_streams_new(),
          close_deadline_milliseconds: Some(
            now_milliseconds + state.config.draining_timeout_milliseconds,
          ),
          close_response: Some(CloseResponse(engine.OneRtt, response)),
        ),
      )
    }
  }
...
fn tick_monotonic(state: State, now_milliseconds: Int) -> Result(State, Error) {
  case state.phase, state.close_deadline_milliseconds {
    ...
    Closing, Some(deadline) if now_milliseconds >= deadline ->
      Ok(State(..state, phase: Closed, close_response: None))
...
fn tick_recovery(state: State, now_milliseconds: Int) -> Result(State, Error) {
  ...
  tick_client_anti_deadlock(state, now_milliseconds)
}
...
fn tick_packet_space(
  ...
  case
    packet_space_discarded(state, level)
    || { level == engine.OneRtt && !state.handshake_confirmed }
    || !amplification.can_send(state.amplification, 1)
  {
    True -> Ok(state)
    False -> {
      let space = packet_space_for_level(state, level)
        ...
        )
      {
        ...
        Ok(packet_space.ProbeTimeout(space, probes)) -> {
          let count = packet_space.probe_timeout_count(space)
          let state = put_packet_space(state, level, space)
          use state <- result.try(synchronize_pto_backoff(state, count))
          Ok(
            state
            |> queue_pto_probe_frames(level)
            |> grant_probe_allowance(probes)
            |> update_ecn_timeout,
          )
        }
...
fn tick_client_anti_deadlock(
  state: State,
  now_milliseconds: Int,
) -> Result(State, Error) {
  use deadline <- result.try(client_anti_deadlock_deadline(state))
  case deadline, anti_deadlock_probe_level(state) {
    Some(deadline), Some(level) if now_milliseconds >= deadline -> {
      let space = packet_space_for_level(state, level)
      case packet_space.force_probe_timeout(space) {
        Ok(packet_space.ProbeTimeout(space, probes)) -> {
          let count = packet_space.probe_timeout_count(space)
          let state = put_packet_space(state, level, space)
          use state <- result.try(synchronize_pto_backoff(state, count))
          Ok(
            state
            |> queue_pto_probe_frame(level)
            |> grant_probe_allowance(probes)
            |> update_ecn_timeout,
          )
        }
        _ -> Error(PacketSpaceFailure)
      }
    }
    _, _ -> Ok(state)
  }
...
/// Queue the expired-space probe and, when possible, one probe in another
/// packet space with in-flight data. The fixed PTO allowance is two, so this
/// search can never create more recovery work than the reserved ledger holds.
fn queue_pto_probe_frames(
  state: State,
  expired_level: engine.EncryptionLevel,
) -> State {
  let state = queue_pto_probe_frame(state, expired_level)
  case
    other_pto_probe_level(state, expired_level, [
      engine.Initial,
      engine.Handshake,
      engine.OneRtt,
      engine.ZeroRtt,
  ...
  {
    None -> state
    Some(level) -> queue_pto_probe_frame(state, level)
  }
}

/// Make one packet space ready to spend a PTO allowance. New queued work is
/// already the best probe payload. Otherwise transfer the oldest reliable
/// outstanding frame to the send queue; only a space with no delivery work at
/// all falls back to PING.
fn queue_pto_probe_frame(state: State, level: engine.EncryptionLevel) -> State {
  case level_has_ack_eliciting_output(state, level) {
    True -> state
    False -> {
      let space = packet_space_for_level(state, level)
      let #(space, selected) = packet_space.take_probe_frame(space)
      let state = put_packet_space(state, level, space)
      case selected {
        Some(value) -> prepend_queue(state, level, value)
        None -> prepend_queue(state, level, frame.Ping)
      }
    }
  }
}

fn level_has_ack_eliciting_output(
  state: State,
  level: engine.EncryptionLevel,
) -> Bool {
  let queue = case level {
    engine.Initial -> state.initial_queue
    engine.Handshake -> state.handshake_queue
    engine.ZeroRtt -> state.zero_rtt_queue
    engine.OneRtt -> state.application_queue
  }
  list.any(queue, frame_ack_eliciting)
  || case level {
    engine.ZeroRtt | engine.OneRtt -> state.active_streams.count > 0
    engine.Initial | engine.Handshake -> False
  }
}

fn other_pto_probe_level(
  state: State,
  expired_level: engine.EncryptionLevel,
  candidates: List(engine.EncryptionLevel),
) -> Option(engine.EncryptionLevel) {
  case candidates {
    [] -> None
    [candidate, ..rest] ->
      case
        !same_packet_space(candidate, expired_level)
        && !packet_space_discarded(state, candidate)
        && keys_available(state, candidate, Write)
        && packet_space.has_in_flight_ack_eliciting(packet_space_for_level(
          state,
          candidate,
        ))
      {
        True -> Some(candidate)
        False -> other_pto_probe_level(state, expired_level, rest)
      }
  }
}

fn same_packet_space(
  left: engine.EncryptionLevel,
  right: engine.EncryptionLevel,
) -> Bool {
  case left, right {
    engine.Initial, engine.Initial | engine.Handshake, engine.Handshake -> True
    engine.ZeroRtt, engine.ZeroRtt
    | engine.ZeroRtt, engine.OneRtt
    | engine.OneRtt, engine.ZeroRtt
    | engine.OneRtt, engine.OneRtt
    -> True
    _, _ -> False
  }
}

/// Project one connection-level PTO exponent into all packet spaces.
fn synchronize_pto_backoff(state: State, count: Int) -> Result(State, Error) {
  use initial <- result.try(
    packet_space.synchronize_probe_timeout_count(state.initial_space, count)
    |> result.replace_error(PacketSpaceFailure),
  )
  use handshake <- result.try(
    packet_space.synchronize_probe_timeout_count(state.handshake_space, count)
    |> result.replace_error(PacketSpaceFailure),
  )
  use application <- result.try(
    packet_space.synchronize_probe_timeout_count(state.application_space, count)
    |> result.replace_error(PacketSpaceFailure),
  )
  Ok(
    State(
      ...
      application_space: application,
    ),
  )
}

fn connection_probe_timeout_count(state: State) -> Int {
  maximum(
    packet_space.probe_timeout_count(state.initial_space),
    maximum(
      packet_space.probe_timeout_count(state.handshake_space),
      packet_space.probe_timeout_count(state.application_space),
    ),
  )
}

/// A newly acknowledged ack-eliciting packet resets the shared exponent,
/// except for a client's Initial ACK before handshake confirmation. Such an
/// ACK does not prove the server finished validating the client's address.
fn synchronize_pto_after_ack(
  state: State,
  level: engine.EncryptionLevel,
  acknowledged: List(packet_space.SentPacket),
  previous_count: Int,
) -> Result(State, Error) {
  let acknowledged_ack_eliciting =
    list.any(acknowledged, fn(packet) { packet.ack_eliciting })
  case acknowledged_ack_eliciting {
    False -> Ok(state)
    True
      if state.config.role == Client
      && level == engine.Initial
      && !state.handshake_confirmed
    -> synchronize_pto_backoff(state, previous_count)
    True -> synchronize_pto_backoff(state, 0)
  }
}
...
fn map_wire_result(
  ...
  case value {
    ...
    Error(wire_packet.InvalidReservedBits) ->
      Error(ProtocolViolation(ReservedBitsViolation))
