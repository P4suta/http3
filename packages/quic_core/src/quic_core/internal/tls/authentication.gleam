...
@external(erlang, "quic_core_tls_ffi", "is_ip_address")
fn raw_is_ip_address(hostname: String) -> Bool

import quic_core/internal/tls/extension_value.{type SignatureScheme}
...
@external(erlang, "quic_core_tls_ffi", "trust_store_from_pem")
fn raw_trust_store_from_pem(pem: BitArray) -> Result(TrustStore, Int)

@external(erlang, "quic_core_tls_ffi", "trust_store_from_der")
fn raw_trust_store_from_der(
  certificates: List(BitArray),
) -> Result(TrustStore, Int)

@external(erlang, "quic_core_tls_ffi", "system_trust_store")
fn raw_system_trust_store() -> Result(TrustStore, Int)

@external(erlang, "quic_core_tls_ffi", "certificate_chain_from_pem")
fn raw_certificate_chain_from_pem(pem: BitArray) -> Result(List(BitArray), Int)

@external(erlang, "quic_core_tls_ffi", "validate_server_certificate")
...
@external(erlang, "quic_core_tls_ffi", "validate_client_certificate")
fn raw_validate_client_certificate(
  certificate_chain: List(BitArray),
  trust_store: TrustStore,
) -> Result(VerifiedPeer, Int)

@external(erlang, "quic_core_tls_ffi", "validate_client_certificate_purpose")
fn raw_validate_client_certificate_purpose(
  certificate_chain: List(BitArray),
) -> Result(Nil, Int)

@external(erlang, "quic_core_tls_ffi", "verified_peer_fingerprint")
fn raw_verified_peer_fingerprint(peer: VerifiedPeer) -> Result(BitArray, Int)

@external(erlang, "quic_core_tls_ffi", "signing_key_from_pem")
fn raw_signing_key_from_pem(pem: BitArray) -> Result(SigningKey, Int)

@external(erlang, "quic_core_tls_ffi", "signing_key_scheme")
fn raw_signing_key_scheme(signing_key: SigningKey) -> Result(Int, Int)

@external(erlang, "quic_core_tls_ffi", "signing_key_matches_certificate")
...
@external(erlang, "quic_core_tls_ffi", "sign")
fn raw_sign(
  signing_key: SigningKey,
  signature_scheme: Int,
  content: BitArray,
...
@external(erlang, "quic_core_tls_ffi", "verify")
fn raw_verify(
  peer: VerifiedPeer,
  signature_scheme: Int,
  content: BitArray,
...
@external(erlang, "quic_core_tls_ffi", "constant_time_equal")
