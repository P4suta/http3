...
/// A validated caller-managed guard retained without exposing its TLS term.
pub opaque type ManagedReplayGuard {
  ManagedReplayGuard(replay_guard.Guard)
}
...
/// Validate the finite execution deadline of a caller-managed replay guard.
pub fn managed_replay_guard(
  timeout_milliseconds: Int,
  check: fn(BitArray, Int) -> Result(Bool, Nil),
) -> Result(ManagedReplayGuard, Error) {
  replay_guard.new(timeout_milliseconds, check)
  |> result.map(ManagedReplayGuard)
  |> result.replace_error(InvalidPolicy)
}

/// Require one validated managed guard before accepting early data.
pub fn with_managed_replay_guard(
  policy: ServerPolicy,
  guard: ManagedReplayGuard,
) -> ServerPolicy {
  let ManagedReplayGuard(guard) = guard
  with_external_replay_guard(policy: policy, guard: guard)
}
