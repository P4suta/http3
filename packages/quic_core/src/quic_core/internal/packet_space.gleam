...
import quic_core/internal/recovery
...
/// One previously acknowledged send time retained only while an older packet
/// remains outstanding. The list is bounded by the sent ledger and packet-
/// threshold loss detection, and prevents a false persistent-congestion
/// declaration across an ACK that arrived in an earlier ACK frame.
type AcknowledgedPacket {
  AcknowledgedPacket(packet_number: Int, time_sent_milliseconds: Int)
}
...
pub opaque type State {
  State(
    ...
    acknowledged_history: List(AcknowledgedPacket),
...
/// Retain a PTO probe in one of exactly two recovery-reserve slots.
///
/// RFC 9002 requires probes to remain in flight even when the ordinary ledger
/// is full. A fixed reserve preserves that progress guarantee without turning
/// recovery metadata into an unbounded peer-controlled allocation.
pub fn record_probe_sent(
  state: State,
  now_milliseconds: Int,
  ack_eliciting: Bool,
  in_flight: Bool,
...
/// Advance PTO backoff and request two probes when the connection-level
/// client anti-deadlock timer fires without any packet in flight.
pub fn force_probe_timeout(state: State) -> Result(TimeoutOutcome, Error) {
  case state.discarded, state.probe_timeout_count >= 62 {
    True, _ -> Error(SpaceDiscarded)
    False, True -> Error(InvalidInput)
    False, False ->
      Ok(ProbeTimeout(
...
/// Set the connection-wide PTO backoff projected into this packet space.
///
/// Packet number spaces retain independent loss times and sent ledgers, but
/// RFC 9002 applies consecutive-PTO exponential backoff across all spaces.
/// The connection owner synchronizes this bounded counter after every PTO and
/// every ACK that resets recovery.
pub fn synchronize_probe_timeout_count(
  state: State,
  count: Int,
) -> Result(State, Error) {
  case count >= 0 && count <= 62 {
    True -> Ok(State(..state, probe_timeout_count: count))
    False -> Error(InvalidInput)
  }
}
...
/// Transfer the oldest reliable frame's retransmission responsibility to a
/// PTO probe without declaring its packet lost.
///
/// The packet remains in the sent ledger with its congestion and RTT metadata,
/// but a later loss of that original packet cannot enqueue a second copy of
/// the transferred frame. ACK, PADDING, PING, CONNECTION_CLOSE, PATH_RESPONSE,
/// and unreliable DATAGRAM frames carry no reliable delivery obligation and
/// are skipped. The sent ledger bounds this scan.
pub fn take_probe_frame(state: State) -> #(State, Option(frame.Frame)) {
  let #(sent_packets, selected) =
    take_probe_frame_from_packets(state.sent_packets, [])
  #(State(..state, sent_packets: sent_packets), selected)
}

fn take_probe_frame_from_packets(
  packets: List(SentPacket),
  reversed: List(SentPacket),
) -> #(List(SentPacket), Option(frame.Frame)) {
  case packets {
    [] -> #(list.reverse(reversed), None)
    [packet, ..rest] -> {
      let #(frames, selected) = take_probe_frame_from_frames(packet.frames, [])
      case selected {
        None -> take_probe_frame_from_packets(rest, [packet, ..reversed])
        Some(_) -> #(
          list.append(list.reverse(reversed), [
            SentPacket(..packet, frames: frames),
            ..rest
          ]),
          selected,
        )
...
fn take_probe_frame_from_frames(
  frames: List(frame.Frame),
  reversed: List(frame.Frame),
) -> #(List(frame.Frame), Option(frame.Frame)) {
  case frames {
    [] -> #(list.reverse(reversed), None)
    [value, ..rest] ->
      case probe_frame_reliable(value) {
        True -> #(list.append(list.reverse(reversed), rest), Some(value))
        False -> take_probe_frame_from_frames(rest, [value, ..reversed])
      }
  }
}

fn probe_frame_reliable(value: frame.Frame) -> Bool {
  case value {
    frame.Padding(_)
    | frame.Ping
    | frame.Ack(_)
...
/// Return whether this packet space has data that can justify a PTO probe.
pub fn has_in_flight_ack_eliciting(state: State) -> Bool {
  list.any(state.sent_packets, fn(packet) {
    packet.in_flight && packet.ack_eliciting
  })
}
...
fn record_valid_packet(
  ...
  let packet =
    SentPacket(
      ...
      frames: frames,
      ecn: codepoint,
      rtt_sample_available_when_sent: rtt_sample_available_when_sent,
    )
  let retained = ack_eliciting || in_flight
  case
    retained
    && list.length(state.sent_packets)
    >= state.maximum_outstanding_packets + recovery_reserve
  {
    True ->
      Error(SentLedgerFull(state.maximum_outstanding_packets + recovery_reserve))
    False -> {
...
fn process_acknowledgement(
  ...
  case largest >= state.next_packet_number {
    ...
    False -> {
      ...
      let acknowledged_history =
        remember_acknowledged(state.acknowledged_history, acknowledged)
      ...
      let state =
        State(
          ...
          acknowledged_history: acknowledged_history,
      ...
      use persistent_congestion <- result.try(establishes_persistent_congestion(
        lost,
        acknowledged_history,
        estimator,
        state.maximum_ack_delay_milliseconds,
      ...
      let state =
        State(
          ..state,
          acknowledged_history: retain_relevant_acknowledgements(
            acknowledged_history,
            state.sent_packets,
          ),
        )
...
fn remember_acknowledged(
  history: List(AcknowledgedPacket),
  packets: List(SentPacket),
) -> List(AcknowledgedPacket) {
  case packets {
    [] -> history
    [packet, ..rest] ->
...
fn establishes_persistent_congestion(
  lost: List(SentPacket),
  acknowledged: List(AcknowledgedPacket),
  estimator: rtt.Estimator,
  maximum_ack_delay_milliseconds: Int,
  timer_granularity_milliseconds: Int,
) -> Result(Bool, Error) {
  ...
  {
    Error(rtt.InvalidInput) -> Error(InvalidInput)
    Ok(duration) -> Ok(persistent_interval(lost, acknowledged, duration, None))
  }
}

/// Walk newly lost packets in send order. Any previously or newly
/// acknowledged send time is a hard boundary; after one, a new lost
/// ack-eliciting packet starts the next candidate interval.
fn persistent_interval(
  lost: List(SentPacket),
  acknowledged: List(AcknowledgedPacket),
  duration_milliseconds: Int,
  start: Option(SentPacket),
) -> Bool {
  case lost {
    [] -> False
    [packet, ..rest] ->
      case
        packet.ack_eliciting && packet.rtt_sample_available_when_sent,
        start
      {
        False, _ ->
          persistent_interval(rest, acknowledged, duration_milliseconds, start)
        True, None ->
          persistent_interval(
            rest,
            acknowledged,
        ...
        True, Some(first) ->
          case
            acknowledged_between(
              acknowledged,
              first.time_sent_milliseconds,
              packet.time_sent_milliseconds,
            )
          {
            True ->
            ...
            False
              if packet.time_sent_milliseconds - first.time_sent_milliseconds
              > duration_milliseconds
            -> True
            False ->
              persistent_interval(
...
fn acknowledged_between(
  acknowledged: List(AcknowledgedPacket),
  oldest_time_milliseconds: Int,
  newest_time_milliseconds: Int,
) -> Bool {
  list.any(acknowledged, fn(packet) {
    packet.time_sent_milliseconds > oldest_time_milliseconds
    && packet.time_sent_milliseconds < newest_time_milliseconds
  })
}

/// Once no older packet remains outstanding, older ACK evidence can no longer
/// split a future loss interval. Pruning here makes the history finite without
/// weakening any future persistent-congestion decision.
fn retain_relevant_acknowledgements(
  acknowledged: List(AcknowledgedPacket),
  outstanding: List(SentPacket),
) -> List(AcknowledgedPacket) {
  case earliest_send_time(outstanding, None) {
    None -> []
    Some(earliest) ->
      list.filter(acknowledged, fn(packet) {
        packet.time_sent_milliseconds > earliest
      })
  }
}

fn earliest_send_time(
  packets: List(SentPacket),
  earliest: Option(Int),
) -> Option(Int) {
  case packets {
    [] -> earliest
    [packet, ..rest] -> {
      let next = case earliest {
        None -> Some(packet.time_sent_milliseconds)
        Some(value) if packet.time_sent_milliseconds < value ->
          Some(packet.time_sent_milliseconds)
        Some(_) -> earliest
      }
      earliest_send_time(rest, next)
    }
  }
}
...
fn update_rtt(
  ...
  // ACK Delay describes the frame's largest acknowledged packet. RFC 9002
  // section 5.1 therefore permits a sample only when that exact packet is
  // newly acknowledged and at least one newly acknowledged packet is
  // ack-eliciting. The largest packet need not itself be ack-eliciting.
  case
    newly_acknowledged_packet(acknowledged, largest_acknowledged),
    has_ack_eliciting(acknowledged)
  {
    Some(packet), True ->
...
fn newly_acknowledged_packet(
  packets: List(SentPacket),
  packet_number: Int,
) -> Option(SentPacket) {
  case packets {
    [] -> None
    [packet, ..] if packet.packet_number == packet_number -> Some(packet)
    [_, ..rest] -> newly_acknowledged_packet(rest, packet_number)
  }
}
