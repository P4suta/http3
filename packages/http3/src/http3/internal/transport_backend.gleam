...
/// Primitive redacted handshake diagnostics.
pub type RawHandshakeAttempt =
...
pub fn client_guaranteed_datagram_size(
  stream: client_stream_backend.StreamHandle,
) -> Result(Int, Failure) {
  native_client.guaranteed_datagram_size(stream) |> map_native_result
}
...
pub fn server_guaranteed_datagram_size(
  request: server_backend.RequestHandle,
) -> Result(Int, Failure) {
  native_server.guaranteed_datagram_size(request)
  |> map_native_server_result
}

pub fn server_prospective_guaranteed_datagram_size(
  request: server_backend.RequestHandle,
) -> Result(Int, Failure) {
  native_server.prospective_guaranteed_datagram_size(request)
  |> map_native_server_result
}
...
pub fn client_handshake_attempt(
  connection: client_stream_backend.ConnectionHandle,
) -> Result(RawHandshakeAttempt, Failure) {
  native_client.handshake_attempt(connection)
  |> result.map(fn(attempt) {
    let native_client.HandshakeAttempt(
      ticket_supplied,
      zero_rtt_enabled,
    ...
    #(
      ...
      zero_rtt_enabled,
      ticket_allows_zero_rtt,
      early_data_code(early),
      resumption_code(resumption),
    )
  })
  |> map_native_result
}
