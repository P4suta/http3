...
const maximum_encoded_origin_entries = 256
  ...
  case bit_array.bit_size(bytes) % 8 {
    ...
    _ ->
      case
        ...
        && limits.maximum_origin_entries > 0
      {
        True -> Ok(Nil)
...
fn decode_payload(
  ...
  case frame_type {
    ...
    12 ->
      origin.decode(payload, maximum_entries: limits.maximum_origin_entries)
      |> result.map(Origin)
  ...
  case value {
    ...
    Origin(origins) -> {
      use payload <- result.try(
        origin.encode(origins, maximum_entries: maximum_encoded_origin_entries)
        |> result.map_error(OriginFailure),
      )
      Ok(#(12, payload))
    }
