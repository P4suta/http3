...
/// A structurally valid HTTP/3 Datagram before request semantics are bound.
pub type Decoded {
  Decoded(stream_id: Int, payload: BitArray)
}
...
pub fn decode_unreliable(
  ...
  use decoded <- result.try(decode_wire(state, encoded))
  bind_received(state, decoded)
}

/// Decode the connection-level wire prefix without assuming the associated
/// request already exists. The HTTP/3 owner uses this seam to distinguish a
/// future stream (silently droppable) from an existing request with no
/// Datagram semantics (request error).
pub fn decode_wire(state: State, encoded: BitArray) -> Result(Decoded, Error) {
  ...
  case quarter_stream_id > varint.maximum / 4 {
    ...
    False -> {
      ...
      use _ <- result.try(validate_payload(state, payload))
      Ok(Decoded(stream_id, payload))
    }
  }
}

/// Bind a decoded Datagram to the explicitly negotiated extension request.
pub fn bind_received(
  state: State,
  decoded: Decoded,
) -> Result(Received, Error) {
  let Decoded(stream_id, payload) = decoded
