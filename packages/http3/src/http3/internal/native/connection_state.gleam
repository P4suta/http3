...
pub type Event {
  ...
  OriginsReceived(List(origin.Origin))
  PriorityChanged(priority.Update)
  ExtensionFrameIgnored(frame_type: Int)
}

/// Application disposition of one structurally valid HTTP/3 Datagram.
pub type DatagramReceipt {
  DatagramDelivered(datagram.Received)
  DatagramDropped
  DatagramRequestRejected(stream_id: Int)
}
...
// Frames which arrive after a QPACK-blocked field section remain ordered on
// their QUIC stream, but independent reader actors can deliver them before the
// encoder-stream instruction that unblocks the section. Keep only a finite,
// per-stream FIFO. The two-list representation makes enqueue and dequeue
// amortised O(1), while `bytes` makes the admission check allocation-free.
type BlockedFrame {
  BlockedFrame(value: frame.Frame, bytes: Int)
}

type BlockedFrames {
  BlockedFrames(front: List(BlockedFrame), back: List(BlockedFrame), bytes: Int)
}
...
pub opaque type State {
  State(
    ...
    largest_peer_request_stream_id: Int,
    transactions: Dict(Int, Transaction),
    push_transactions: Dict(Int, PushTransaction),
    blocked_frames: Dict(Int, BlockedFrames),
    blocked_frame_bytes: Int,
...
pub type Error {
  ...
  StreamBlocked(Int)
  BlockedStreamBufferExceeded(maximum: Int)
  BlockedConnectionBufferExceeded(maximum: Int)
  FrameUnexpected
  RequestRejected(Int)
  PushRejected(Int)
  ...
  QpackDecompressionFailure(decoder.Error)
  QpackEncoderStreamFailure(decoder.Error)
  QpackDecoderStreamFailure(encoder.Error)
...
pub fn receive_push_stream_frame(
  ...
  case transaction.blocked {
    Some(_) -> queue_blocked_frame(state, stream_id, incoming)
...
pub fn receive_control_frame(
  ...
  case event {
    ...
    control.OriginsReceived(origins) -> Ok(#(state, [OriginsReceived(origins)]))
...
/// Bytes retained for frames which followed a blocked field section. This is
/// an internal observability seam used by resource-bound tests and diagnostics.
pub fn blocked_stream_bytes(state: State, stream_id: Int) -> Int {
  dict.get(state.blocked_frames, stream_id)
  |> result.map(fn(queue) { queue.bytes })
  |> result.unwrap(0)
}

/// Total encoded frame bytes retained behind QPACK-blocked field sections.
pub fn blocked_connection_bytes(state: State) -> Int {
  state.blocked_frame_bytes
}

/// Number of future request priorities retained before their streams open.
/// This intentionally excludes priorities already attached to the scheduler.
pub fn pending_priority_count(state: State) -> Int {
  dict.size(state.pending_priorities)
}

/// Payload-free cardinalities for retained per-stream protocol state.
///
/// These instantaneous counts let live resource tests distinguish parser,
/// transaction, push, and QPACK-blocked retention without exposing headers or
/// stream identifiers.
pub fn resource_counts(state: State) -> #(Int, Int, Int) {
  #(
    dict.size(state.transactions),
    dict.size(state.push_transactions),
    dict.size(state.blocked_frames),
  )
}

/// Largest future request stream ID whose priority can currently be retained.
/// The window slides as request streams are observed and is bounded by the
/// same finite transaction admission policy used by the connection.
pub fn maximum_retained_priority_stream_id(state: State) -> Int {
  state.largest_peer_request_stream_id + state.config.maximum_transactions * 4
}

/// Release all receive-side HTTP/3 and QPACK state after RESET_STREAM.
///
/// The role adapter retains the peer's application error code. This state
/// transition only performs bounded protocol cleanup and queues QPACK stream
/// cancellation feedback when a field section still references peer entries.
pub fn receive_stream_reset(
  state: State,
  stream_id: Int,
) -> Result(State, Error) {
  let state =
    State(
      ..state,
      qpack_decoder: decoder.cancel_stream(state.qpack_decoder, stream_id),
    )
    |> drop_blocked_frames(stream_id)
  case optional_transaction(state, stream_id) {
    Some(transaction) ->
      Ok(reset_request_transaction(state, stream_id, transaction))
    None ->
      case optional_push_transaction(state, stream_id) {
        Some(transaction) ->
          reset_push_transaction(state, stream_id, transaction)
        None -> Ok(state)
      }
  }
}
...
pub fn receive_qpack_encoder_instruction(
  ...
  use qpack_decoder <- result.try(
    ...
    |> map_qpack_encoder_stream_result,
  )
  retry_blocked(
...
pub fn receive_qpack_decoder_instruction(
  ...
  use qpack_encoder <- result.try(
    ...
    |> map_qpack_decoder_stream_result,
  )
...
pub fn send_datagram(
  ...
  // Resolve semantic association first so an ordinary request remains a
  // stable DatagramNotAssociated failure even if its HTTP transaction has
  // already reached terminal cleanup in a concurrent worker.
  use encoded <- result.try(
    datagram.encode_unreliable(state.datagrams, stream_id, payload)
    |> map_datagram_result,
  )
  use transaction <- result.try(get_transaction(state, stream_id))
  case transaction.outbound_finished {
    True -> Error(DatagramFailure(datagram.SendSideClosed(stream_id)))
    False -> Ok(encoded)
  }
}

/// Decode an associated QUIC DATAGRAM payload and apply request lifecycle.
pub fn receive_datagram(
  state: State,
  payload: BitArray,
) -> Result(DatagramReceipt, Error) {
  use decoded <- result.try(
    datagram.decode_wire(state.datagrams, payload) |> map_datagram_result,
  )
  let datagram.Decoded(stream_id, _) = decoded
  case optional_transaction(state, stream_id) {
    None -> Ok(DatagramDropped)
    Some(Transaction(inbound_finished: True, ..)) -> Ok(DatagramDropped)
    Some(_) ->
      case datagram.bind_received(state.datagrams, decoded) {
        Ok(received) -> Ok(DatagramDelivered(received))
        Error(datagram.UnknownAssociation(_)) ->
          Ok(DatagramRequestRejected(stream_id))
    ...
    Ok(update) ->
      case state.config.role, update {
        ...
        Server, priority.RequestUpdate(stream_id, value) -> {
          use _ <- result.try(validate_priority_request_target(state, stream_id))
        ...
        Server, priority.PushUpdate(push_id, _) ->
          case push.get(state.pushes, push_id) {
            Some(_) -> Ok(#(state, [PriorityChanged(update)]))
            None -> Error(PriorityFailure(priority.InvalidElementId(push_id)))
          }
      }
  }
}

fn validate_priority_request_target(
  state: State,
  stream_id: Int,
) -> Result(Nil, Error) {
  case stream_id <= maximum_retained_priority_stream_id(state) {
    True -> Ok(Nil)
    False -> Error(PriorityFailure(priority.InvalidElementId(stream_id)))
  }
}
...
fn ensure_inbound_transaction(
    ...
    incoming
  {
    ...
    False, Server, frame.Headers(_) -> {
      let state = observe_peer_request_stream(state, stream_id)
      use _ <- result.try(ensure_transaction_capacity(state, stream_id))
...
fn observe_peer_request_stream(state: State, stream_id: Int) -> State {
  case stream_id > state.largest_peer_request_stream_id {
    True -> State(..state, largest_peer_request_stream_id: stream_id)
    False -> state
  }
}
...
fn receive_transaction_frame(
  ...
  case incoming {
    ...
    frame.Unknown(frame_type, _) ->
      case priority.is_update_frame_type(frame_type) {
        True -> Error(FrameUnexpected)
        False -> Ok(#(state, [ExtensionFrameIgnored(frame_type)]))
      }
    _ -> Error(FrameUnexpected)
  }
}

fn queue_blocked_frame(
  state: State,
  stream_id: Int,
  incoming: frame.Frame,
) -> Result(#(State, List(Event)), Error) {
  use encoded <- result.try(frame.encode(incoming) |> map_frame_result)
  let bytes = bit_array.byte_size(encoded)
  let maximum = state.config.maximum_frame_payload_bytes
  let queue =
    dict.get(state.blocked_frames, stream_id)
    |> result.unwrap(BlockedFrames([], [], 0))
  case
    bytes > maximum - queue.bytes,
    bytes > maximum - state.blocked_frame_bytes
  {
    True, _ -> Error(BlockedStreamBufferExceeded(maximum))
    False, True -> Error(BlockedConnectionBufferExceeded(maximum))
    False, False -> {
      let queue =
        BlockedFrames(
          ..queue,
          back: [BlockedFrame(incoming, bytes), ..queue.back],
      ...
      Ok(
        #(
          State(
            ..state,
            blocked_frames: dict.insert(state.blocked_frames, stream_id, queue),
            blocked_frame_bytes: state.blocked_frame_bytes + bytes,
          ),
          [],
        ),
      )
...
fn pop_blocked_frame(
  state: State,
  stream_id: Int,
) -> #(State, Option(frame.Frame)) {
  case optional_blocked_frames(state, stream_id) {
    None -> #(state, None)
    Some(queue) -> pop_existing_blocked_frame(state, stream_id, queue)
  }
}

fn pop_existing_blocked_frame(
  state: State,
  stream_id: Int,
  queue: BlockedFrames,
) -> #(State, Option(frame.Frame)) {
  let BlockedFrames(front, back, retained) = normalise_blocked_frames(queue)
  case front {
    [] -> #(drop_blocked_frames(state, stream_id), None)
    [BlockedFrame(value, bytes), ..rest] -> {
      let blocked_frames =
        retain_blocked_frame_tail(
          state.blocked_frames,
          stream_id,
          rest,
          back,
          retained - bytes,
...
fn normalise_blocked_frames(queue: BlockedFrames) -> BlockedFrames {
  case queue.front {
    [] -> BlockedFrames(list.reverse(queue.back), [], queue.bytes)
    _ -> queue
  }
}

fn retain_blocked_frame_tail(
  queues: Dict(Int, BlockedFrames),
  stream_id: Int,
  front: List(BlockedFrame),
  back: List(BlockedFrame),
  bytes: Int,
) -> Dict(Int, BlockedFrames) {
  case front, back {
    [], [] -> dict.delete(queues, stream_id)
    _, _ -> dict.insert(queues, stream_id, BlockedFrames(front, back, bytes))
  }
}

fn replay_request_frames(
  state: State,
  stream_id: Int,
) -> Result(#(State, List(Event)), Error) {
  let #(state, next) = pop_blocked_frame(state, stream_id)
  case next {
    None -> Ok(#(state, []))
    Some(incoming) -> replay_request_frame(state, stream_id, incoming)
  }
}

fn replay_request_frame(
  state: State,
  stream_id: Int,
  incoming: frame.Frame,
) -> Result(#(State, List(Event)), Error) {
  case optional_transaction(state, stream_id) {
    None -> Ok(#(drop_blocked_frames(state, stream_id), []))
    Some(transaction) -> {
      use #(state, events) <- result.try(receive_transaction_frame(
        state,
        stream_id,
        transaction,
        incoming,
      ))
      continue_request_replay(state, stream_id, events)
    }
  }
}

fn continue_request_replay(
  state: State,
  stream_id: Int,
  events: List(Event),
) -> Result(#(State, List(Event)), Error) {
  case optional_transaction(state, stream_id) {
    None -> Ok(#(drop_blocked_frames(state, stream_id), events))
    Some(transaction) ->
      case transaction.blocked {
        Some(_) -> Ok(#(state, events))
        None -> {
          use #(state, remaining) <- result.try(replay_request_frames(
            state,
            stream_id,
          ))
          Ok(#(state, list.append(events, remaining)))
        }
      }
  }
}

fn replay_push_frames(
  state: State,
  stream_id: Int,
) -> Result(#(State, List(Event)), Error) {
  let #(state, next) = pop_blocked_frame(state, stream_id)
  case next {
    None -> Ok(#(state, []))
    Some(incoming) -> replay_push_frame(state, stream_id, incoming)
  }
}

fn replay_push_frame(
  state: State,
  stream_id: Int,
  incoming: frame.Frame,
) -> Result(#(State, List(Event)), Error) {
  case optional_push_transaction(state, stream_id) {
    None -> Ok(#(drop_blocked_frames(state, stream_id), []))
    Some(transaction) -> {
      use #(state, events) <- result.try(receive_push_frame(
        state,
        stream_id,
        transaction,
        incoming,
      ))
      continue_push_replay(state, stream_id, events)
    }
  }
}

fn continue_push_replay(
  state: State,
  stream_id: Int,
  events: List(Event),
) -> Result(#(State, List(Event)), Error) {
  case optional_push_transaction(state, stream_id) {
    ...
    Some(transaction) ->
      case transaction.blocked {
        Some(_) -> Ok(#(state, events))
        None -> {
          use #(state, remaining) <- result.try(replay_push_frames(
            state,
            stream_id,
          ))
          Ok(#(state, list.append(events, remaining)))
...
fn drop_blocked_frames(state: State, stream_id: Int) -> State {
  let released = blocked_stream_bytes(state, stream_id)
  State(
    ..state,
    blocked_frames: dict.delete(state.blocked_frames, stream_id),
    blocked_frame_bytes: int.max(state.blocked_frame_bytes - released, 0),
  )
}
...
fn decode_headers(
  ...
  use outcome <- result.try(
    ...
    |> map_qpack_decompression_result,
  )
  case outcome {
...
fn finish_push_after_unblock(
  state: State,
  stream_id: Int,
  _transaction: PushTransaction,
  events: List(Event),
) -> Result(#(State, List(Event)), Error) {
  use #(state, replayed) <- result.try(replay_push_frames(state, stream_id))
  let events = list.append(events, replayed)
  case optional_push_transaction(state, stream_id) {
    None -> Ok(#(drop_blocked_frames(state, stream_id), events))
    Some(transaction) ->
      case transaction.blocked, transaction.fin_pending {
        Some(_), _ | None, False -> Ok(#(state, events))
        None, True -> {
...
fn finish_after_unblock(
  ...
  use #(state, replayed) <- result.try(replay_request_frames(state, stream_id))
  let events = list.append(events, replayed)
  case optional_transaction(state, stream_id) {
    None -> Ok(#(drop_blocked_frames(state, stream_id), events))
    Some(transaction) ->
      case transaction.blocked, transaction.inbound_fin_pending {
        Some(_), _ | None, False -> Ok(#(state, events))
        None, True -> {
...
fn reset_request_transaction(
  ...
  cleanup_if_complete(state, stream_id)
}

fn reset_push_transaction(
  state: State,
  stream_id: Int,
  transaction: PushTransaction,
) -> Result(State, Error) {
  use pushes <- result.try(
    push.cancel(state.pushes, transaction.push_id) |> map_push_result,
  )
  use pushes <- result.try(
    push.release(pushes, transaction.push_id) |> map_push_result,
  )
  use drain_state <- result.try(
    drain.complete_push(state.drain, transaction.push_id) |> map_drain_result,
  )
  use peer_streams <- result.try(
    stream_registry.close(state.peer_streams, stream_id)
    |> map_registry_result,
  )
  Ok(drop_blocked_frames(
    State(
      ..state,
      pushes: pushes,
      drain: drain_state,
...
fn optional_transaction(state: State, stream_id: Int) -> Option(Transaction) {
  dict.get(state.transactions, stream_id)
  |> result.map(Some)
  |> result.unwrap(None)
}
...
fn optional_push_transaction(
  state: State,
  stream_id: Int,
) -> Option(PushTransaction) {
  dict.get(state.push_transactions, stream_id)
  |> result.map(Some)
  |> result.unwrap(None)
}

fn optional_blocked_frames(
  state: State,
  stream_id: Int,
) -> Option(BlockedFrames) {
  dict.get(state.blocked_frames, stream_id)
  |> result.map(Some)
  |> result.unwrap(None)
}
...
fn validate_config(
  ...
  case
    ...
    && config.maximum_frame_payload_bytes > 0
...
fn map_qpack_decompression_result(
  value: Result(value, decoder.Error),
) -> Result(value, Error) {
  case value {
    Ok(updated) -> Ok(updated)
    Error(error) -> Error(QpackDecompressionFailure(error))
  }
}

fn map_qpack_encoder_stream_result(
  value: Result(value, decoder.Error),
) -> Result(value, Error) {
  case value {
    Ok(updated) -> Ok(updated)
    Error(error) -> Error(QpackEncoderStreamFailure(error))
  }
}

fn map_qpack_decoder_stream_result(
  value: Result(value, encoder.Error),
) -> Result(value, Error) {
  case value {
    Ok(updated) -> Ok(updated)
    Error(error) -> Error(QpackDecoderStreamFailure(error))
  }
}
