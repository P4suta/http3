...
import quic_core.{type AddressFamily, Ipv4, Ipv6}
...
pub opaque type Server {
  Server(
    default_credential: server_worker.Credential,
    alternative_credentials: List(server_worker.Credential),
    port: Int,
    ...
    allow_zero_rtt: Bool,
    replay_guard: Option(server_worker.ReplayGuard),
...
pub opaque type ReplayGuard {
  ReplayGuard(handle: server_worker.ReplayGuard)
}
...
pub fn new(
  ...
  use default_credential <- result.try(
    server_worker.default_credential(certificate_pem, private_key_pem)
...
pub fn replay_guard(
  ...
  server_worker.new_replay_guard(timeout_milliseconds, check)
...
pub fn is_valid_server_name(server_name: String) -> Bool {
  server_worker.valid_server_name(server_name)
}
...
pub fn with_certificate(
  ...
  use Nil <- result.try(case server_worker.valid_server_name(server_name) {
    True -> Ok(Nil)
  ...
  use Nil <- result.try(
    case
      list.any(server.alternative_credentials, fn(credential) {
        server_worker.credential_server_name(credential) == server_name
      })
    {
  ...
  use credential <- result.try(
    server_worker.named_credential(
      server_name,
      certificate_pem,
      private_key_pem,
    )
...
pub fn is_valid_certificate(certificate_pem: BitArray) -> Bool {
  server_worker.valid_certificate(certificate_pem)
}
...
pub fn is_valid_private_key(private_key_pem: BitArray) -> Bool {
  server_worker.valid_private_key(private_key_pem)
}
...
/// Set the aggregate bytes retained across every connection on this endpoint.
pub fn with_endpoint_memory_limit(
  server server: Server,
  bytes bytes: Int,
) -> Result(Server, ConfigurationError) {
  case bytes > 0 {
    True -> Ok(Server(..server, endpoint_memory_limit: bytes))
...
pub fn reload_certificates(
  ...
  server_worker.reload_certificates(
    handle,
    replacement.default_credential,
...
/// Cancel both QUIC directions of one accepted request stream idempotently.
pub fn cancel(request: Request) -> Result(Cancellation, Error) {
  let Request(handle) = request
  case server_worker.cancel(handle) {
    Ok(server_worker.Cancelled) -> Ok(Cancelled)
    Ok(server_worker.AlreadyCancelled) -> Ok(AlreadyCancelled)
    Ok(server_worker.AlreadyCompleted) -> Ok(AlreadyCompleted)
    Error(error) -> Error(map_error(error))
  }
}
...
pub fn respond(
  ...
  let Request(handle) = request
  server_worker.respond(handle, status, headers, body)
  |> result.map_error(map_error)
}
...
pub fn send_capsule(request: Request, bytes: BitArray) -> Result(Nil, Error) {
  let Request(handle) = request
  server_worker.send_capsule(handle, bytes) |> result.map_error(map_error)
}
...
pub fn path_stats(request: Request) -> Result(PathStats, Error) {
  ...
  |> result.map(fn(snapshot) {
    let diagnostics.PathStats(a, b, c, d, e, f, g, h) = snapshot
...
pub fn request_state_stats(
  ...
  let Request(handle) = request
  server_worker.request_state_stats(handle) |> result.map_error(map_error)
}
...
pub fn guaranteed_datagram_size(request: Request) -> Result(Int, Error) {
  let Request(handle) = request
  server_worker.guaranteed_datagram_size(handle) |> result.map_error(map_error)
}

/// Return the prospective HTTP Datagram payload ceiling before response 2xx.
pub fn prospective_guaranteed_datagram_size(
  request: Request,
) -> Result(Int, Error) {
  let Request(handle) = request
  server_worker.prospective_guaranteed_datagram_size(handle)
  |> result.map_error(map_error)
}
...
fn map_credential_error(
  error: server_worker.CredentialError,
) -> ConfigurationError {
  case error {
    server_worker.InvalidCredentialCertificate -> InvalidCertificate
    server_worker.InvalidCredentialPrivateKey -> InvalidPrivateKey
    server_worker.IncompatibleCredentialPrivateKey -> IncompatiblePrivateKey
    server_worker.InvalidCredentialServerName -> InvalidServerName
  }
}
...
fn map_error(error: server_worker.Error) -> Error {
  case error {
    ...
    server_worker.StreamCancelled(code) ->
      Failure(runtime_failure.Closed(runtime_failure.Local, Some(code)))
    ...
    server_worker.CapsuleProtocolUnavailable -> CapsuleProtocolUnavailable
