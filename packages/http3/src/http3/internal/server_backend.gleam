...
pub fn start(
  ...
  endpoint_memory_limit endpoint_memory_limit: Int,
  ...
  use configuration <- result.try(
    native_server.with_endpoint_memory_limit(
      configuration,
  ...
  let configuration =
    native_server.with_address_family(configuration, case address_family {
      config.Ipv4 -> quic_core.Ipv4
      config.Ipv6 -> quic_core.Ipv6
      config.DualStack -> quic_core.DualStack
    })
...
pub fn cancel(request: RequestHandle) -> Result(Int, Failure) {
  case native_server.cancel(request) {
    Ok(native_server.Cancelled) -> Ok(1)
    Ok(native_server.AlreadyCancelled) -> Ok(2)
    Ok(native_server.AlreadyCompleted) -> Ok(3)
    Error(error) -> Error(map_native_error(error))
  }
}
...
pub fn request_state_stats(
  request: RequestHandle,
) -> Result(#(Int, Int, Int, Int, Int, Int, Int, Int, Int), Failure) {
  native_server.request_state_stats(request)
  |> result.map_error(map_native_error)
}
...
pub fn send_capsule(
  ...
  native_server.send_capsule(request, bytes)
  |> result.map_error(map_native_error)
}
...
fn map_native_error(error: native_server.Error) -> Failure {
  case error {
    ...
    native_server.CapsuleProtocolUnavailable -> CapsuleProtocolUnavailable
