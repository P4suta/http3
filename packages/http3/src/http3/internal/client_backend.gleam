...
pub fn send(
  ...
  use client <- result.try(
    native_client.with_endpoint_memory_limit(client, endpoint_memory_limit)
    |> result.map_error(from_native_configuration_error),
  )
  use client <- result.try(
    native_client.with_telemetry_limit(client, telemetry_limit)
    |> result.map_error(from_native_configuration_error),
  )
  ...
  use client <- result.try(case qlog_directory {
    "" -> Ok(client)
    directory ->
      native_client.with_qlog(client, directory)
      |> result.map_error(from_native_configuration_error)
  })
...
fn from_native_error(error: native_client.Error) -> Failure {
  case error {
    ...
    native_client.CapsuleProtocolUnavailable -> CapsuleProtocolUnavailable
