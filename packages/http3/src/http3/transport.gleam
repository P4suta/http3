...
/// Return an HTTP Datagram payload ceiling stable for the connection's life.
///
/// This conservative value does not grow with path-MTU discovery and cannot
/// shrink because of ACK debt, migration, or black-hole fallback. It is useful
/// when a downstream resource must be bounded once before payload admission.
pub fn guaranteed_datagram_size(stream: Stream) -> Result(Int, Error) {
  case stream {
    ClientStream(handle) ->
      transport_backend.client_guaranteed_datagram_size(handle) |> map_failure
    ServerStream(handle) ->
      transport_backend.server_guaranteed_datagram_size(handle) |> map_failure
  }
}

/// Return a server request's guaranteed payload ceiling before its successful
/// Extended CONNECT response activates the HTTP Datagram association.
@internal
pub fn prospective_guaranteed_datagram_size(
  stream: Stream,
) -> Result(Int, Error) {
  case stream {
    ClientStream(_) -> Error(DatagramNotAssociated)
    ServerStream(handle) ->
      transport_backend.server_prospective_guaranteed_datagram_size(handle)
      |> map_failure
  }
}
...
pub fn handshake_attempt(
  ...
  let Connection(handle) = connection
  case transport_backend.client_handshake_attempt(handle) {
    Ok(#(ticket_supplied, enabled, permitted, early, resumption)) -> {
      use early <- result.try(map_early_data_code(early))
      use resumption <- result.try(map_resumption_code(resumption))
      Ok(HandshakeAttempt(
        ticket_supplied:,
        zero_rtt_enabled: enabled,
        ticket_allows_zero_rtt: permitted,
        early_data: early,
        resumption:,
      ))
    }
...
fn map_early_data_status(
  ...
  case result {
    Ok(code) -> map_early_data_code(code)
    Error(error) -> Error(from_backend_failure(error))
  }
}

fn map_early_data_code(code: Int) -> Result(EarlyDataStatus, Error) {
  case code {
    0 -> Ok(NotAttempted)
    1 -> Ok(Pending)
    2 -> Ok(Accepted)
    3 -> Ok(Rejected)
    _ -> Error(Failure(runtime_failure.Http3(runtime_failure.Local, None)))
  }
}
...
fn map_resumption_status(
  ...
  case result {
    Ok(code) -> map_resumption_code(code)
    Error(error) -> Error(from_backend_failure(error))
  }
}

fn map_resumption_code(code: Int) -> Result(ResumptionStatus, Error) {
  case code {
    0 -> Ok(ResumptionNotAttempted)
    1 -> Ok(ResumptionPending)
    2 -> Ok(Resumed)
    3 -> Ok(FullHandshake)
    _ -> Error(Failure(runtime_failure.Http3(runtime_failure.Local, None)))
  }
