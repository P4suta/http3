...
current implementation uses the sibling `quic_core` package as its only
...
HTTP/3 package workers off package-private QUIC/TLS primitives remains a release
blocker.
...
http3-owned HTTP/3 workers, session, Capsules, and QPACK
    |
package-private quic_core QUIC driver, TLS, and recovery
    |
...
`http3/internal`. Their role adapters use only public opaque `quic_core`
connections and streams plus the restricted application diagnostic sink.
QUIC listener routing, UDP, TLS, Retry, tokens, recovery, migration, and
connection actors remain core-owned. The HTTP/3-owned Gleam workers own
reusable client coordination, request registries, pull waiters, bounded queues,
and operation deadlines. The HTTP/3 session owns:
...
`packages/quic_core` contains the protocol implementation. Its driver joins
...
Two capacity queries keep those meanings explicit. `maximum_datagram_size` is
the point-in-time packing bound and can change with validated PMTU and ACK debt.
`guaranteed_datagram_size` reserves the 1200-byte floor, widest short-header
protection, and the maximum half-path ACK share, then applies the authenticated
peer frame limit. It is therefore a connection-lifetime bound. HTTP/3 subtracts
the quarter-stream ID from either value; the pre-response server query uses the
guaranteed value so downstream sockets can be bounded before an optimistic 2xx.
...
switch. Typed client credentials and server-side `Disabled`, `Optional`, and
`Required` mTLS policies are implemented; verified client identity is exposed
only through the redacted public identity value.
...
Production Erlang FFI is restricted to 23 source modules and audited as one
closed set:

| Package | Modules and responsibility |
| --- | --- |
| `http` | Fourteen `http_*_ffi` modules for Body/resource ownership, TCP/TLS, client/server lifecycle and HTTP/1 plus HTTP/2 listener diagnostics, OTP compression, and cryptographic primitives used by the published extensions |
| `http3` | `http3_address_ffi`, `http3_internal_transport_ffi`, `http3_process_label_ffi`, and `http3_websocket_ffi` for address conversion, opaque-handle adaptation, labels, and WebSocket validation helpers |
| `quic_core` | Five `quic_core_*_ffi` modules for crypto/TLS, UDP ownership, process labels, and bounded qlog output |

The FFI audit reads the canonical inventory from `qualification.json`, proves
that source discovery matches it exactly, then runs Dialyzer and compiled xref
over that same set. It rejects undeclared cross-boundary calls. Every module
validates term and binary shapes, catches runtime failures, and returns closed
numeric or typed errors. Protocol state machines and HTTP/QUIC wire codecs
remain Gleam; there is no NIF, native library, or runtime downloader.

## Dependency and replacement boundary

The root `http` package depends on `http3`, `gleam_http`, and `gleam_stdlib`.
This `http3` package depends on `quic_core`, `gleam_http`, `gleam_erlang`, and
`gleam_stdlib`; `quic_core` depends only on `gleam_erlang` and
`gleam_stdlib`. Development uses path dependencies, while the release
simulation rewrites them to exact temporary-registry `0.1.0` dependencies.
No external QUIC library, NIF, C/Rust library, or alternative production
...
The intended `quic_core` public API is application-protocol independent. Raw
...
APIs are implemented and directly tested. HTTP/3 uses only the six
compiler-exported core modules and opaque connections/streams. A Semgrep rule,
source import scan, compiler-interface
audit, and compiled xref all enforce the boundary; the former 41-entry
allowlist is now comments-only with zero accepted imports.
...
enforced listener-wide on the server role. The client validates its configured
endpoint budget before creating an actor or socket and rejects an insufficient
fixed admission reservation through the typed public failure path.
