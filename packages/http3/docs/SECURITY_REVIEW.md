...
| Item | Value |
| --- | --- |
| Review date | Reopened and refreshed 2026-08-30 |
...
| Scope | The `http`, `http3`, and `quic_core` public APIs and adapters, repository-owned protocol code, production Erlang FFI, manifests, tests, and CI |
| Review type | Internal source and behavior review |
| Outcome | In progress; unresolved release findings are tracked in the conformance matrix |
...
The root package normalizes requests, responses, policies, and errors before
selecting H1, H2, or H3. HTTP/3 owns its application-protocol workers and uses
only opaque public-core connections and streams. Core Gleam workers own QUIC
processes, UDP sockets, TLS and transport state, admission, and routed mailbox
credit. The full data flow is documented in
[Architecture](ARCHITECTURE.md).

The production FFI audit checks the 23 modules in the canonical qualification
inventory: fourteen root `http_*_ffi` modules, four `http3_*_ffi` modules, and
five `quic_core_*_ffi` modules. Source discovery must match that inventory
exactly. They cover bounded resource/socket ownership and the narrow OTP
crypto, compression, X.509, address, process-label, and qlog primitives
described in [Architecture](ARCHITECTURE.md). Dialyzer and compiled xref run
over the whole set and reject undeclared cross-package calls.
...
covered without crashing the transport. The live validator requires the
configured transport, TLS, recovery, HTTP/3, and QPACK families in every client
and server trace, plus schema, redaction, and qvis-compatible lifecycle checks.
Completing the standards-level event inventory remains separate release work.

## Dependencies and source audit

The production closure is deliberately small:

- `http`: `http3`, `gleam_http`, and `gleam_stdlib`;
- `http3`: `quic_core`, `gleam_http`, `gleam_erlang`, and `gleam_stdlib`;
- `quic_core`: `gleam_erlang` and `gleam_stdlib`; and
...
from package manifests. Development uses path dependencies, but the OTP 29
release simulation builds all three archives twice, rewrites the chain to
exact temporary-registry `0.1.0` dependencies, and runs empty consumers.

The current source gate runs gitleaks with nine documented public fixture-key
fingerprints and two exact public/test-marker false positives, five repository
Semgrep boundary/FFI rules, REUSE, and Dialyzer/xref over all 23 production
Erlang FFI modules. The online OSV gate
...
generates CycloneDX evidence. Canonical-source reruns remain an audit-bundle
condition.
...
exports must normalize to the same bytes. The three-package release simulation
adds exact-semver root packaging, archive-wide content checks, SBOM,
provenance, and ephemeral signature verification on OTP 29; OTP 28 and the
canonical-source rerun remain open.

On 2026-08-30, the current worktree passed exact-key gitleaks, five repository
Semgrep rules, the configured OSV scan, REUSE, Dialyzer, production-FFI xref,
and CycloneDX generation. This is
...
gate additionally checks canonical compiler API snapshots for all three packages,
Dialyzer/xref, gitleaks, and local Semgrep rules; the separate security gate
adds OSV and CycloneDX generation. Coverage thresholds, expanded peers and
platforms, cross-protocol performance, requirements completion, and the
canonical-source release rerun remain open. Exact requirements are in
...
- incomplete standards/errata, coverage, expanded interop,
