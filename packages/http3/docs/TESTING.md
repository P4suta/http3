...
two runners: `test/native/quic_core_fuzz.gleam` and
`test/native/quic_core_property.gleam` here, and
`packages/quic_core/test/quic_core_fuzz.gleam` and
`packages/quic_core/test/quic_core_property.gleam` in the core package.
...
Current pass/fail state, discovered test counts, and finding status are
generated from [`qualification.json`](../../../qualification.json) into the
[product conformance status](../../../docs/CONFORMANCE.md). They are not
maintained in this package guide.

The generated campaigns have two modes. `mise run property` and
`mise run fuzz` execute 10,000 cases per affected wire family for a normal
change. The nightly workflow passes explicit seeds to eight independent
shards, totalling 1,000,000 generated cases per family; fuzz shards also replay
the retained corpus. `mise run model` similarly runs a PR-scale CUBIC model and
accepts nightly shard arguments for one million transitions in aggregate.

`mise run hostile-peer` executes isolated BEAM modules for stalled peers,
UDP/mailbox floods, actor crashes, aggregate memory pressure, admission
exhaustion, endpoint memory, parser bombs, and slow consumers. `mise run
credential-matrix-local` runs live RSA-PSS, ECDSA P-256/P-384, Ed25519, mTLS,
HRR, rotation, resumption, 0-RTT fallback, expiry, revocation, and mismatch
rows. It records a report even when the current host lacks a qualified
platform label. Nightly collects and aggregates Ubuntu 24.04, macOS 15, and
Windows 2025 on OTP 28 and 29.

`mise run release-sim` creates all three Hex v3 archives twice, requires exact
`0.1.0` archive dependencies, compiles and executes empty consumers, and emits
SBOM, provenance, content, checksum, unwanted-file, and ephemeral-signature
evidence. That local simulation does not replace a passing canonical-source
candidate, cross-platform reports, or the independent audit.
...
three-package release simulation, fixed performance thresholds, and
...
`packages/quic_core/test/qlog_test_ffi.erl` create their scratch directories
...
Outstanding qualification gates and their implementation states are declared
once in the repository [`qualification.json`](../../../qualification.json)
manifest. `mise run status-audit` verifies that every manifest entry agrees
with its executable task, and the missing gates exit non-zero naming the task.
They are not treated as successful evidence by `mise run check`.
