...
policy in `packages/quic_core/src/quic_core_udp_ffi.erl`. Every earlier
...
option being accepted; neither had been observed on a Windows host.

The run was taken on commit `48a8b3b`, before the three-package split, when
the same FFI was `packages/gleam_quic/src/gleam_quic_udp_ffi.erl`. The module
moved; its Windows option constants, its Don't-Fragment policy, and the send
classifier this measures did not, so the measurement is carried here under its
original date and host rather than re-taken.

The measurements below were taken on the live network path, not on loopback, whose
...
Windows, so `quic_core/internal/udp.classify_send` receives the `EMSGSIZE` it classifies as a path
