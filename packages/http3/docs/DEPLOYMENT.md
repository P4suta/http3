...
- Set finite connection, handshake, stream, body, buffer, queue, Datagram,
  QPACK, accept-waiter, and telemetry budgets for the host. Size the enforced
  server `EndpointMemory` aggregate budget for the admission reservation and
  expected connection growth; retain BEAM/host controls for memory outside
  library-owned connection and routed-mailbox state.
