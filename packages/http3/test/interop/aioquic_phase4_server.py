...
def install_shutdown_handlers(shutdown_event):
    """Turn TERM/INT into an async cleanup request so qlog is always flushed."""
    ...
    for signal_number in (signal.SIGTERM, signal.SIGINT):
        try:
            ...
            # add_signal_handler is unavailable on some event-loop/platform
            # combinations. KeyboardInterrupt remains the bounded fallback.
            pass


async def close_server(server, proxy_transport):
    """Close every connection before process exit, retaining peer-side qlog."""
    ...
    if protocols:
        try:
            await asyncio.wait_for(
                asyncio.gather(
                    *(protocol.wait_closed() for protocol in protocols),
                    return_exceptions=True,
                ),
                timeout=SHUTDOWN_TIMEOUT_SECONDS,
            )
        except asyncio.TimeoutError:
            print("GRACEFUL_SHUTDOWN_TIMEOUT", flush=True)
    ...
    try:
        if os.environ.get("HTTP3_INTEROP_EXIT_ON_COMPLETE") == "1":
            ...
            shutdown_waiter = asyncio.create_task(shutdown_event.wait())
                ...
                asyncio.wait(
                    {completion_waiter, shutdown_waiter},
            ...
            if completion_waiter in done and completion_waiter.result():
                await asyncio.sleep(0.25)
            else:
                print("SHUTDOWN_SIGNAL_RECEIVED", flush=True)
        else:
            await shutdown_event.wait()
            print("SHUTDOWN_SIGNAL_RECEIVED", flush=True)
    finally:
        await close_server(server, proxy_transport)
