...
run(Port, ResumptionPort, QlogDirectory) ->
    ...
    {ok, HandshakeAttempt} =
        http3@transport:handshake_attempt(ResumedTransport),
    io:format("aioquic handshake-attempt=~p~n", [HandshakeAttempt]),
    {handshake_attempt, true, true, true, AttemptedEarlyData, AttemptedResumption} =
        HandshakeAttempt,
    true = AttemptedEarlyData =/= not_attempted,
    true = AttemptedResumption =/= resumption_not_attempted,
    {ok, ResumptionHandshakeState} =
        http3@transport:resumption_status(ResumedTransport),
    true = ResumptionHandshakeState =/= resumption_not_attempted,
