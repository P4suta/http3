...
/// Fail a test while preserving one deliberately bounded structured reason.
@external(erlang, "http3_test_ffi", "fail")
pub fn fail(reason: reason) -> value
...
/// Route one caller-owned server through a proxy whose loss window is armed
/// synchronously after the handshake. `arm` drops exactly the next two client
/// datagrams, which can deterministically cover a delayed response ACK and the
/// first CONNECTION_CLOSE without making handshake recovery part of the test.
pub fn with_armed_client_loss_proxy(
  server_port: Int,
  ca_certificate: BitArray,
  run: fn(Int, BitArray, fn() -> Nil) -> result,
) -> result {
  with_armed_client_loss_proxy_ffi(server_port, ca_certificate, run)
}
...
@external(erlang, "http3_test_ffi", "with_armed_client_loss_proxy")
fn with_armed_client_loss_proxy_ffi(
  server_port: Int,
  ca_certificate: BitArray,
  run: fn(Int, BitArray, fn() -> Nil) -> result,
) -> result
...
/// Run a qlog fixture and retain its traces if exact cardinality is violated.
@external(erlang, "http3_test_ffi", "with_qlog_directory_expect")
pub fn with_qlog_directory_expect(
...
/// Require every qlog trace to contain every exact event name.
@external(erlang, "http3_test_ffi", "qlog_each_trace_contains")
pub fn qlog_each_trace_contains(
  directory: String,
