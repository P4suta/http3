...
-export([
    ...
    start_task/1,
    with_armed_client_loss_proxy/3,
...
fail(Reason) ->
    erlang:error({http3_test_failed, Reason}).

-spec pause_milliseconds(non_neg_integer()) -> nil.
pause_milliseconds(Milliseconds) ->
    timer:sleep(Milliseconds).

%% Runs Fun with the path of a uniquely named scratch directory and reports how
%% many qlog traces were written there. Successful fixtures are deleted. A
%% failing fixture retains its traces and prints only their count and path
%% before re-raising, so a rare live-path failure remains diagnosable without
%% dumping trace contents into CI output.
-spec with_qlog_directory(fun((binary()) -> term())) -> term().
with_qlog_directory(Fun) when is_function(Fun, 1) ->
    Directory = unique_scratch_directory("http3-qlog-test-"),
    try
        Result = Fun(list_to_binary(Directory)),
        SuccessFiles = filelib:wildcard(filename:join(Directory, "*.qlog")),
        Outcome = {Result, length(SuccessFiles)},
        _ = file:del_dir_r(Directory),
        Outcome
    catch
        Class:Reason:Stacktrace ->
            FailureFiles = filelib:wildcard(filename:join(Directory, "*.qlog")),
            io:format(standard_error,
                "qlog fixture failed; retained ~B trace(s) at ~ts~n",
                [length(FailureFiles), Directory]),
            erlang:raise(Class, Reason, Stacktrace)
    end.

%% Run a qlog fixture whose trace cardinality is itself part of the success
%% contract. The check happens before cleanup, so an unexpected but otherwise
%% successful retry/connection pattern retains the exact same evidence as an
%% exception raised by Fun.
-spec with_qlog_directory_expect(
    fun((binary()) -> term()), non_neg_integer()
) -> term().
with_qlog_directory_expect(Fun, Expected)
        when is_function(Fun, 1), is_integer(Expected), Expected >= 0 ->
    Directory = unique_scratch_directory("http3-qlog-test-"),
    try
        Result = Fun(list_to_binary(Directory)),
        SuccessFiles = filelib:wildcard(filename:join(Directory, "*.qlog")),
        case length(SuccessFiles) of
            Expected ->
                ...
                Result;
            Actual ->
                erlang:error({qlog_trace_count_mismatch, Expected, Actual})
        end
    catch
        Class:Reason:Stacktrace ->
            FailureFiles = filelib:wildcard(filename:join(Directory, "*.qlog")),
...
with_environment_override(Name, Prefix, Fun) ->
    Root = unique_scratch_directory(Prefix),
    Previous = [
...
%% Atomically reserves a unique directory under the OS temporary directory.
%% unique_integer/1 alone restarts with each VM and can collide with retained
%% failure evidence from an earlier process. The timestamp narrows collision
%% probability, while make_dir/1 is the actual cross-process exclusion and an
%% existing directory is retried rather than reused.
-spec unique_scratch_directory(string()) -> string().
unique_scratch_directory(Prefix) ->
    Root = temporary_root(),
    ok = filelib:ensure_dir(filename:join(Root, "placeholder")),
    reserve_scratch_directory(Root, Prefix, 0).

-spec reserve_scratch_directory(string(), string(), non_neg_integer()) -> string().
reserve_scratch_directory(Root, Prefix, Attempt) when Attempt < 1024 ->
    Suffix = lists:flatten(io_lib:format(
        "~B-~B",
        [erlang:system_time(nanosecond),
         erlang:unique_integer([positive, monotonic])]
    )),
    Directory = filename:join(Root, Prefix ++ Suffix),
    case file:make_dir(Directory) of
        ok -> Directory;
        {error, eexist} -> reserve_scratch_directory(Root, Prefix, Attempt + 1);
        {error, Reason} -> erlang:error({cannot_reserve_scratch_directory, Reason})
    end;
reserve_scratch_directory(_Root, _Prefix, 1024) ->
    erlang:error(scratch_directory_collision_limit).
...
%% Require every retained trace in a fixture to contain every exact event
%% name. This catches an apparently complete aggregate formed by one rich
%% trace and one transport-only trace without exposing trace contents in test
%% output.
-spec qlog_each_trace_contains(binary(), [binary()]) -> boolean().
qlog_each_trace_contains(Directory, EventNames)
        when is_binary(Directory), is_list(EventNames) ->
    Files = filelib:wildcard(filename:join(binary_to_list(Directory), "*.qlog")),
    Files =/= [] andalso lists:all(fun(File) ->
        case file:read_file(File) of
            {ok, Contents} ->
                lists:all(fun(EventName) ->
                    Pattern = <<"\"name\":\"", EventName/binary, "\"">>,
                    binary:match(Contents, Pattern) =/= nomatch
                end, EventNames);
            {error, _Reason} -> false
        end
    end, Files).
...
%% Arm a deterministic post-handshake loss window from the test process.
%% The synchronous arm acknowledgement makes the ordering independent of the
%% proxy mailbox scheduler: packets sent after Arm() returns see the new
%% window, and exactly two client datagrams must exercise it before cleanup.
-spec with_armed_client_loss_proxy(
    inet:port_number(),
    binary(),
    fun((inet:port_number(), binary(), fun(() -> nil)) -> Result)
) -> Result.
with_armed_client_loss_proxy(ServerPort, CaCert, Fun) when is_function(Fun, 3) ->
    {Proxy, Monitor, ProxyPort} = start_proxy(ServerPort, armed_client_loss),
    Arm = fun() -> arm_client_loss(Proxy, 2) end,
    try
        Fun(ProxyPort, CaCert, Arm)
    after
        stop_proxy(Proxy, Monitor)
    end.

arm_client_loss(Proxy, Count) ->
    Ref = make_ref(),
    Proxy ! {arm_client_loss, Count, self(), Ref},
    receive
        {Ref, armed} -> nil
    after ?FIXTURE_TIMEOUT ->
        erlang:error(arm_client_loss_timeout)
    end.
...
initial_proxy_state(packet_loss) ->
    #{drop_client => true};
initial_proxy_state(armed_client_loss) ->
    #{
        await_server_trigger => false,
        armed_client_losses => 0,
        remaining_client_losses => 0,
        dropped_client => 0,
        server_triggers => 0
    };
...
proxy_loop(Socket, ServerPort, Mode, Client, State) ->
    receive
        {arm_client_loss, Count, From, Ref}
            when Mode =:= armed_client_loss, Count > 0, Count =< 8 ->
            From ! {Ref, armed},
            proxy_loop(
                Socket,
                ServerPort,
                ...
                Client,
                State#{
                    await_server_trigger => true,
                    armed_client_losses => Count
                }
            );
...
proxy_client_packet(
    ...
    armed_client_loss,
    _Packet,
    #{remaining_client_losses := Remaining, dropped_client := Dropped} = State
) when Remaining > 0 ->
    State#{
        remaining_client_losses => Remaining - 1,
        dropped_client => Dropped + 1
    };
proxy_client_packet(
    Socket,
    ServerPort,
...
proxy_server_packet(
    ...
    State#{
        await_server_trigger => false,
        remaining_client_losses => Count,
        server_triggers => Triggers + 1
    };
proxy_server_packet(
    Socket,
    corruption,
...
ensure_fault_was_exercised(
    ...
    ok;
ensure_fault_was_exercised(armed_client_loss, State) ->
    erlang:error({armed_client_loss_fault_not_exercised, State});
