...
// nolint: unused_exports -- gleeunit discovers public test functions by suffix.
pub fn rejects_truncated_and_out_of_range_quarter_stream_ids_test() -> Nil {
  let assert Ok(state) = datagram.new(True, True, 1, 8)
  assert datagram.decode_wire(state, <<>>) == Error(datagram.Truncated)
  assert datagram.decode_wire(state, <<
      0xff,
      0xff,
      0xff,
      0xff,
      ...
      0xff,
    >>)
    == Error(datagram.InvalidQuarterStreamId(4_611_686_018_427_387_903))
}
