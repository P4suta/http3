...
// nolint: unused_exports -- gleeunit discovers public test functions by suffix.
pub fn rejects_invalid_dynamic_references_and_declared_insert_counts_test() -> Nil {
  let assert Ok(state) = decoder.new(128, 1, 16, 4096)
  assert decoder.apply_encoder_instruction(
      state,
      instruction.InsertWithNameReference(False, 0, <<"value">>),
    )
    == Error(decoder.DynamicTableFailure(dynamic_table.MissingRelativeIndex(0)))
  let assert Ok(state) =
    decoder.apply_encoder_instruction(
      state,
      instruction.SetDynamicTableCapacity(128),
  ...
  let too_small =
    field_section.Section(field_section.Prefix(0, False, 1), [
      field_section.Indexed(False, 0),
    ])
    |> field_section.encode(False)
    |> result.unwrap(<<>>)
  assert decoder.decode(state, 0, too_small)
    == Error(decoder.InvalidRequiredInsertCount)
  let missing =
    field_section.Section(field_section.Prefix(2, False, 1), [
      field_section.IndexedPostBase(0),
    ])
    |> field_section.encode(False)
    |> result.unwrap(<<>>)
  assert decoder.decode(state, 0, missing)
    == Error(decoder.InvalidDynamicReference(2))
}
