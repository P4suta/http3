...
// RFC 9204 verified erratum 8410: Required Insert Count is one greater than
// the largest zero-based dynamic index. The first entry therefore encodes a
// non-zero count (wire value 2 at this capacity), never the static-only zero.
// nolint: unused_exports -- gleeunit discovers public test functions by suffix.
pub fn first_dynamic_index_uses_count_one_per_verified_erratum_8410_test() -> Nil {
  let assert Ok(state) = encoder.new(128, 128, 1, 16, 4096)
  let field = header.Header(<<"first">>, <<"value">>, False)
  let assert Ok(state) = encoder.insert(state, field)
  let assert Ok(#(_, encoded)) = encoder.encode(state, 0, [field], True, False)
  let assert Ok(field_section.Section(
    field_section.Prefix(2, False, 0),
