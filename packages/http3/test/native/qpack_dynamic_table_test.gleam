...
// nolint: unused_exports -- gleeunit discovers public test functions by suffix.
pub fn rejects_capacity_and_entry_growth_beyond_the_negotiated_ceiling_test() -> Nil {
  let assert Ok(table) = dynamic_table.new(64)
  assert dynamic_table.set_capacity(table, 65)
    == Error(dynamic_table.CapacityExceeded(64))
  assert dynamic_table.insert(table, dynamic_table.Field(<<"a">>, <<"b">>))
    == Error(dynamic_table.EntryTooLarge(0))
  let assert Ok(table) = dynamic_table.set_capacity(table, 32)
  assert dynamic_table.insert(table, dynamic_table.Field(<<"a">>, <<>>))
    == Error(dynamic_table.EntryTooLarge(32))
}
