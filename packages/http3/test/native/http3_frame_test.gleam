...
pub fn rejects_http2_duplicates_truncation_and_resource_exhaustion_test() -> Nil {
  let limits = frame.Limits(8, 4, 2, 2)
...
// nolint: unused_exports -- gleeunit discovers public test functions by suffix.
pub fn origin_frames_are_typed_empty_bounded_and_strictly_framed_test() -> Nil {
  let limits = frame.Limits(64, 64, 4, 1)
  assert frame.decode(<<0x0c, 0>>, limits) == Ok(#(frame.Origin([]), <<>>))
  assert frame.decode(
      <<0x0c, 21, 19:size(16), "https://EXAMPLE.com":utf8>>,
      limits,
    )
    == Ok(#(frame.Origin([origin.Origin("https", "example.com", None)]), <<>>))
  assert frame.decode(<<0x0c, 4, 3:size(16), "ab":utf8>>, limits)
    == Error(frame.OriginFailure(origin.Truncated))
  assert frame.decode(<<0x0c, 4, 0:size(16), 0:size(16)>>, limits)
    == Error(frame.OriginFailure(origin.TooManyOrigins(maximum: 1)))
}
