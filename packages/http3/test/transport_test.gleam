...
pub fn http_datagrams_require_explicit_negotiation_test() -> Nil {
  ...
  assert server.request_datagram_capacity(incoming)
    == Error(transport.DatagramsNotNegotiated)
  assert client.send_capsule(stream, capsule.Datagram(<<"too-early":utf8>>))
    == Error(client.CapsuleProtocolUnavailable)
  assert server.send_capsule(incoming, capsule.Datagram(<<"too-early":utf8>>))
    == Error(server.CapsuleProtocolUnavailable)
  ...
  assert server.request_datagram_capacity(ordinary_request)
    == Error(transport.DatagramNotAssociated)
  assert client.send_capsule(ordinary, capsule.Datagram(<<"forbidden":utf8>>))
    == Error(client.CapsuleProtocolUnavailable)
  assert server.send_capsule(
      ordinary_request,
      capsule.Datagram(<<"forbidden":utf8>>),
    )
    == Error(server.CapsuleProtocolUnavailable)
...
// nolint: unused_exports -- gleeunit discovers public test functions by suffix.
pub fn rfc9220_websocket_handshake_uses_no_http1_nonce_fields_test() -> Nil {
  let #(certificate, private_key, ca_certificate) =
    http3_test_support.server_credentials()
  let listener =
    server.new(certificate, private_key)
  ...
  let incoming = server.accept(listener) |> should.be_ok
  let request_headers = server.headers(incoming)
  server.send_response(incoming, 200, []) |> should.be_ok
  let connected = http3_test_support.await_task(connecting)

  assert !contains_header(request_headers, "sec-websocket-key")
  // nolint: assert_ok_pattern -- Keep the asynchronous handshake Result typed.
  let assert Ok(socket) = connected
  websocket.cancel(socket) |> should.be_ok
  assert client.close(connection) == Ok(client.Closed)
  assert server.stop(listener) == Ok(server.Stopped)
}

// nolint: unused_exports -- gleeunit discovers public test functions by suffix.
pub fn rfc9220_unknown_extended_protocol_gets_501_test() -> Nil {
  let #(certificate, private_key, ca_certificate) =
    http3_test_support.server_credentials()
  let listener =
    server.new(certificate, private_key)
  ...
  let stream =
    client.open_extended_connect(
      connection,
      streaming_request(port, "/unsupported"),
      "not-a-supported-protocol",
    )
    |> should.be_ok
  let incoming = server.accept(listener) |> should.be_ok

  assert websocket.accept(websocket.new(), incoming)
    == Error(websocket.InvalidHandshake)
  assert client.next_event(stream) == Ok(client.Response(501, []))
  assert receive_response(stream) == <<>>
  assert client.close(connection) == Ok(client.Closed)
  assert server.stop(listener) == Ok(server.Stopped)
}
...
// nolint: unused_exports -- gleeunit discovers public test functions by suffix.
pub fn rfc9220_failed_client_handshake_cancels_the_stream_test() -> Nil {
  let #(certificate, private_key, ca_certificate) =
    http3_test_support.server_credentials()
  let listener =
    server.new(certificate, private_key)
    |> should.be_ok
    |> server.with_timeout(500)
    |> should.be_ok
    |> server.start
    |> should.be_ok
  let port = server.port(listener) |> should.be_ok
  ...
  let connecting =
    http3_test_support.start_task(fn() {
      websocket.connect(
        websocket.new(),
        connection,
        streaming_request(port, "/legacy-response"),
      )
    })
  let incoming = server.accept(listener) |> should.be_ok
  server.send_response(incoming, 200, [
    #("sec-websocket-accept", "legacy-http1-nonce"),
  ])
  |> should.be_ok

  assert http3_test_support.await_task(connecting)
    == Error(websocket.InvalidHandshake)
  assert server.next_event(incoming)
    == Error(
      server.Failure(runtime_failure.Closed(runtime_failure.Peer, Some(0x10c))),
    )
  assert client.close(connection) == Ok(client.Closed)
  assert server.stop(listener) == Ok(server.Stopped)
}
...
// nolint: unused_exports -- gleeunit discovers public test functions by suffix.
pub fn capsule_protocol_rejects_forbidden_message_semantics_test() -> Nil {
  let #(certificate, private_key, ca_certificate) =
    http3_test_support.server_credentials()
  let listener =
    server.new(certificate, private_key)
  ...
  let no_content =
    client.open_extended_connect(
      connection,
      streaming_request(port, "/capsule-204"),
      "test-capsule",
    )
    |> should.be_ok
  let no_content_request = server.accept(listener) |> should.be_ok
  server.send_response(no_content_request, 204, []) |> should.be_ok
  assert client.next_event(no_content) == Ok(client.Response(204, []))
  assert client.send_capsule(no_content, capsule.Datagram(<<>>))
    == Error(client.CapsuleProtocolUnavailable)
  assert server.send_capsule(no_content_request, capsule.Datagram(<<>>))
    == Error(server.CapsuleProtocolUnavailable)
  client.finish(no_content) |> should.be_ok
  assert server.read_body(no_content_request) |> should.be_ok == <<>>
  server.finish_response(no_content_request) |> should.be_ok
  assert receive_response(no_content) == <<>>

  let response_content =
    client.open_extended_connect(
      connection,
      streaming_request(port, "/capsule-response-content"),
      "test-capsule",
    )
    |> should.be_ok
  let response_content_request = server.accept(listener) |> should.be_ok
  server.send_response(response_content_request, 200, [
    #("content-type", "application/capsule"),
  ])
  |> should.be_ok
  assert client.next_event(response_content)
    == Ok(client.Response(200, [#("content-type", "application/capsule")]))
  assert client.send_capsule(response_content, capsule.Datagram(<<>>))
    == Error(client.CapsuleProtocolUnavailable)
  assert server.send_capsule(response_content_request, capsule.Datagram(<<>>))
    == Error(server.CapsuleProtocolUnavailable)
  client.finish(response_content) |> should.be_ok
  assert server.read_body(response_content_request) |> should.be_ok == <<>>
  server.finish_response(response_content_request) |> should.be_ok
  assert receive_response(response_content) == <<>>

  let request_content =
    client.open_extended_connect(
      connection,
      streaming_request(port, "/capsule-request-content")
        |> request.set_header("content-type", "application/capsule"),
      "test-capsule",
    )
    |> should.be_ok
  let request_content_request = server.accept(listener) |> should.be_ok
  server.send_response(request_content_request, 200, []) |> should.be_ok
  assert client.next_event(request_content) == Ok(client.Response(200, []))
  assert client.send_capsule(request_content, capsule.Datagram(<<>>))
    == Error(client.CapsuleProtocolUnavailable)
  assert server.send_capsule(request_content_request, capsule.Datagram(<<>>))
    == Error(server.CapsuleProtocolUnavailable)
  client.finish(request_content) |> should.be_ok
  assert server.read_body(request_content_request) |> should.be_ok == <<>>
  server.finish_response(request_content_request) |> should.be_ok
  assert receive_response(request_content) == <<>>
...
pub fn http_datagram_limits_and_concurrent_receive_are_typed_test() -> Nil {
  ...
  assert server.protocol(incoming) == Some("test-datagram")
  let server_transport = server.request_transport(incoming)
  let prospective = server.request_datagram_capacity(incoming) |> should.be_ok
  assert prospective > 0
  assert prospective <= 32
  assert transport.maximum_datagram_size(server_transport)
    == Error(transport.DatagramNotAssociated)
  assert transport.guaranteed_datagram_size(server_transport)
    == Error(transport.DatagramNotAssociated)
  server.send_response(incoming, 200, []) |> should.be_ok
  assert client.next_event(stream) == Ok(client.Response(200, []))
  let client_transport = client.stream_transport(stream)
  ...
  assert maximum <= 32
  let server_maximum =
    transport.maximum_datagram_size(server_transport) |> should.be_ok
  let server_guaranteed =
    transport.guaranteed_datagram_size(server_transport) |> should.be_ok
  assert server_maximum > 0
  assert server_guaranteed == prospective
  assert server_guaranteed <= server_maximum
...
// nolint: unused_exports -- gleeunit discovers public test functions by suffix.
pub fn server_cancel_releases_body_and_datagram_waiters_with_local_provenance_test() -> Nil {
  let #(certificate, private_key, ca_certificate) =
    http3_test_support.server_credentials()
  let listener =
    server.new(certificate, private_key)
  ...
  let stream =
    client.open_extended_connect(
      connection,
      streaming_request(port, "/cancel-waiters"),
      "connect-udp",
    )
    |> should.be_ok
  let incoming = server.accept(listener) |> should.be_ok
  ...
  let datagram_waiter =
    http3_test_support.start_task(fn() {
      transport.next_datagram(server_transport)
    })
  http3_test_support.pause_milliseconds(20)

  assert server.cancel(incoming) == Ok(server.Cancelled)
  let local_cancellation =
    runtime_failure.Closed(runtime_failure.Local, Some(0x10c))
  assert http3_test_support.await_task(body_waiter)
    == Error(server.Failure(local_cancellation))
  assert http3_test_support.await_task(datagram_waiter)
    == Error(transport.Failure(local_cancellation))
  assert client.next_event(stream)
    == Error(
      client.Failure(runtime_failure.Closed(runtime_failure.Peer, Some(0x10c))),
    )
  ...
  let #(_, qlog_files) =
    http3_test_support.with_qlog_directory(fn(qlog_directory) {
      ...
      assert http3_test_support.qlog_event_count(
        ...
        == 2
      assert http3_test_support.qlog_each_trace_contains(qlog_directory, [
        "quic:connection_started",
        "quic:packet_sent",
  ...
  let #(_, qlog_files) =
    http3_test_support.with_qlog_directory(fn(qlog_directory) {
      let #(certificate, private_key, ca_certificate) =
        http3_test_support.server_credentials()
      let qlog =
        transport.qlog(qlog_directory)
        |> require_push_setup(PushQlogConfiguration)
      let listener =
        server.new(certificate, private_key)
        |> require_push_setup(PushServerConfiguration)
        |> server.with_qlog(qlog)
        |> server.start
        |> require_server_push(PushListenerStart)
      let port = server.port(listener) |> require_server_push(PushListenerPort)
      let configuration =
        client_configuration(ca_certificate)
        |> client.with_push_limit(2)
        |> require_push_setup(PushClientConfiguration)
        |> client.with_qlog(qlog)
      let connection =
        client.connect(configuration, "localhost", port)
        |> require_client_push(PushConnect)
      let stream =
        client.open_stream(connection, streaming_request(port, "/document"))
        |> require_client_push(PushOpenRequest)
      client.finish(stream) |> require_client_push(PushFinishRequest)

      let incoming =
        server.accept(listener) |> require_server_push(PushAcceptRequest)
      let request_body =
        server.read_body(incoming) |> require_server_push(PushReadRequest)
      assert request_body == <<>>
      let push =
        server.promise_push(incoming, "/style.css?version=1", [
          #("accept", "text/css"),
        ])
        |> require_server_push(PushPromise)
      server.send_push_response(push, 200, [#("content-type", "text/css")])
      |> require_server_push(PushResponseHeaders)
      server.send_push_chunk(push, <<"body{}":utf8>>)
      |> require_server_push(PushResponseData)
      server.send_push_trailers(push, [#("x-push-complete", "yes")])
      |> require_server_push(PushResponseTrailers)
      server.respond(incoming, 200, [], <<"document":utf8>>)
      |> require_server_push(PushParentResponse)

      let promised =
        client.next_push(connection) |> require_client_push(PushAcceptPromise)
      ...
      let response =
        client.next_push_event(promised)
        |> require_client_push(PushReadResponseHeaders)
      assert response == client.Response(200, [#("content-type", "text/css")])
      let data =
        client.next_push_event(promised)
        |> require_client_push(PushReadResponseData)
      assert data == client.Data(<<"body{}":utf8>>)
      let trailers =
        client.next_push_event(promised)
        |> require_client_push(PushReadResponseTrailers)
      assert trailers == client.Trailers([#("x-push-complete", "yes")])
      let end =
        client.next_push_event(promised)
        |> require_client_push(PushReadResponseEnd)
      assert end == client.End
      let cancellation =
        client.cancel_push(promised)
        |> require_client_push(PushCancelCompleted)
      assert cancellation == client.AlreadyCompleted
      let parent_body =
        receive_response_result(stream)
        |> require_client_push(PushReadParentResponse)
      assert parent_body == <<"document":utf8>>
      let closed = client.close(connection) |> require_client_push(PushClose)
      assert closed == client.Closed
      let stopped = server.stop(listener) |> require_server_push(PushStop)
      assert stopped == server.Stopped
    })
...
pub fn external_replay_guard_failure_falls_back_to_one_rtt_test() -> Nil {
  ...
  assert transport.handshake_attempt(controls)
    == Ok(transport.HandshakeAttempt(
      ticket_supplied: True,
      zero_rtt_enabled: True,
      ticket_allows_zero_rtt: True,
      early_data: transport.Rejected,
      resumption: transport.Resumed,
    ))
  assert client.close(connection) == Ok(client.Closed)
  assert server.stop(listener) == Ok(server.Stopped)
}
...
pub fn operational_keys_survive_listener_restart_without_enabling_zero_rtt_test() -> Nil {
  ...
  assert transport.handshake_attempt(client.connection_transport(connection))
    == Ok(transport.HandshakeAttempt(
      ticket_supplied: True,
      zero_rtt_enabled: False,
      ticket_allows_zero_rtt: False,
      early_data: transport.NotAttempted,
      resumption: transport.Resumed,
    ))
  let post_request =
...
// A payload-free phase taxonomy for the live server-push regression. Keeping
// this separate from source line numbers makes failures comparable across
// formatting, compiler output, coverage instrumentation, and fresh-BEAM runs.
type ServerPushPhase {
  PushQlogConfiguration
  ...
  PushResponseHeaders
  PushResponseData
  PushResponseTrailers
  PushParentResponse
  PushAcceptPromise
  PushReadResponseHeaders
  PushReadResponseData
  PushReadResponseTrailers
  PushReadResponseEnd
  PushCancelCompleted
  PushReadParentResponse
  PushClose
  PushStop
}
...
type ServerPushCause {
  PushResolutionFailure
  PushSocketFailure(runtime_failure.SocketOperation)
  PushTlsFailure(runtime_failure.Origin)
  PushQuicFailure(runtime_failure.Origin, Option(Int))
  PushHttp3Failure(runtime_failure.Origin, Option(Int))
  PushTimeoutFailure(runtime_failure.TimeoutPhase)
  PushCancellationFailure
  PushClosedFailure(runtime_failure.Origin, Option(Int))
  PushLimitFailure(runtime_failure.Resource, Int)
  PushOverloadFailure(runtime_failure.Resource)
  PushPublicContractFailure
}

type ServerPushTraceFailure {
  ServerPushTraceFailure(ServerPushPhase, ServerPushSide, ServerPushCause)
}

fn require_push_setup(
  outcome: Result(value, setup_error),
  phase: ServerPushPhase,
) -> value {
  case outcome {
    Ok(value) -> value
    // nolint: thrown_away_error -- keep a generic setup error out of retained diagnostics.
    Error(_) ->
      http3_test_support.fail(ServerPushTraceFailure(
        phase,
        PushSetupSide,
        PushPublicContractFailure,
      ))
...
fn require_client_push(
  outcome: Result(value, client.Error),
  phase: ServerPushPhase,
) -> value {
  case outcome {
    Ok(value) -> value
    Error(error) ->
      http3_test_support.fail(ServerPushTraceFailure(
        phase,
        PushClientSide,
        server_push_cause(error),
      ))
  }
}

fn require_server_push(
  ...
  case outcome {
    ...
    Error(error) ->
      http3_test_support.fail(ServerPushTraceFailure(
        phase,
        PushServerSide,
        server_push_server_cause(error),
      ))
  }
}

fn server_push_cause(error: client.Error) -> ServerPushCause {
  case error {
    client.Failure(failure) -> server_push_runtime_cause(failure)
    _ -> PushPublicContractFailure
  }
}

fn server_push_server_cause(error: server.Error) -> ServerPushCause {
  case error {
    server.Failure(failure) -> server_push_runtime_cause(failure)
    _ -> PushPublicContractFailure
  }
}

fn server_push_runtime_cause(
  failure: runtime_failure.Failure,
) -> ServerPushCause {
  case failure {
    runtime_failure.Resolution -> PushResolutionFailure
    runtime_failure.Socket(operation) -> PushSocketFailure(operation)
    runtime_failure.Tls(origin) -> PushTlsFailure(origin)
    runtime_failure.Quic(origin, code) -> PushQuicFailure(origin, code)
    runtime_failure.Http3(origin, code) -> PushHttp3Failure(origin, code)
    runtime_failure.Timeout(phase) -> PushTimeoutFailure(phase)
    runtime_failure.Cancelled -> PushCancellationFailure
    runtime_failure.Closed(origin, code) -> PushClosedFailure(origin, code)
    runtime_failure.Limit(resource, maximum) ->
      PushLimitFailure(resource, maximum)
    runtime_failure.Overload(resource) -> PushOverloadFailure(resource)
  }
}
...
fn contains_header(headers: List(#(String, String)), name: String) -> Bool {
  list.any(headers, fn(header) { header.0 == name })
}
...
fn receive_response_result(
  stream: client.Stream,
) -> Result(BitArray, client.Error) {
  receive_response_result_loop(stream, <<>>)
}

fn receive_response_result_loop(
  stream: client.Stream,
  body: BitArray,
) -> Result(BitArray, client.Error) {
  case client.next_event(stream) {
    ...
    Ok(client.Data(chunk)) ->
      receive_response_result_loop(stream, bit_array.append(body, chunk))
    Ok(client.End) -> Ok(body)
    Ok(_) -> receive_response_result_loop(stream, body)
  }
}
