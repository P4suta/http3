...
// A listener which is already bound owns that endpoint exclusively. Allowing a
// second bind would silently redirect the first listener's UDP traffic on
// platforms whose SO_REUSEADDR UDP semantics select the newest socket.
// nolint: unused_exports -- gleeunit discovers public test functions by suffix.
pub fn second_listener_cannot_steal_an_exact_live_endpoint_test() -> Nil {
  let #(certificate, private_key, ca_certificate) =
    http3_test_support.server_credentials()
  let bind_address = address.parse("127.0.0.1") |> should.be_ok
  let configuration = server.new(certificate, private_key) |> should.be_ok
  let configuration = server.with_bind_address(configuration, bind_address)
  let first = server.start(configuration) |> should.be_ok
  let port = server.port(first) |> should.be_ok
  let second_configuration =
    server.with_port(configuration, port) |> should.be_ok

  assert server.start(second_configuration)
    == Error(server.Failure(failure.Socket(failure.BindSocket)))

  let client_task = start_bounded_client(port, ca_certificate, "/exclusive")
  let incoming = server.accept(first) |> should.be_ok
  assert server.path(incoming) == "/exclusive"
  assert server.read_body(incoming) |> should.be_ok == <<>>
  server.respond(incoming, 204, [], <<>>) |> should.be_ok
  let reply = http3_test_support.await_task(client_task) |> should.be_ok
  assert reply.status == 204
  assert server.stop(first) == Ok(server.Stopped)
}
...
// nolint: unused_exports -- gleeunit discovers public test functions by suffix.
pub fn server_request_cancellation_resets_both_directions_idempotently_test() -> Nil {
  let #(listener, port, ca_certificate) = start_server()
  let connection =
    client.connect(client_configuration(ca_certificate), "localhost", port)
    |> should.be_ok
  let stream =
    client.open_stream(connection, streaming_request(port, "/server-cancel"))
    |> should.be_ok
  let incoming = server.accept(listener) |> should.be_ok

  assert server.cancel(incoming) == Ok(server.Cancelled)
  assert server.cancel(incoming) == Ok(server.AlreadyCancelled)
  assert client.next_event(stream)
    == Error(client.Failure(failure.Closed(failure.Peer, Some(0x10c))))
...
// nolint: unused_exports -- gleeunit discovers public test functions by suffix.
pub fn server_request_cancellation_discards_buffered_body_events_test() -> Nil {
  let #(listener, port, ca_certificate) = start_server()
  let connection =
    client.connect(client_configuration(ca_certificate), "localhost", port)
    |> should.be_ok
  let stream =
    client.open_stream(connection, streaming_request(port, "/cancel-buffered"))
    |> should.be_ok
  let incoming = server.accept(listener) |> should.be_ok

  send_small_chunks(stream, 3)
  http3_test_support.pause_milliseconds(50)
  assert server.next_event(incoming) == Ok(server.Data(<<1>>))
  assert server.cancel(incoming) == Ok(server.Cancelled)
  assert server.next_event(incoming)
    == Error(server.Failure(failure.Closed(failure.Local, Some(0x10c))))
  assert client.next_event(stream)
    == Error(client.Failure(failure.Closed(failure.Peer, Some(0x10c))))
...
// nolint: unused_exports -- gleeunit discovers public test functions by suffix.
pub fn server_request_cancellation_preserves_completed_stream_test() -> Nil {
  let #(listener, port, ca_certificate) = start_server()
  let connection =
    client.connect(client_configuration(ca_certificate), "localhost", port)
    |> should.be_ok
  let stream =
    client.open_stream(connection, streaming_request(port, "/cancel-complete"))
    |> should.be_ok
  client.finish(stream) |> should.be_ok
  let incoming = server.accept(listener) |> should.be_ok
  assert server.next_event(incoming) == Ok(server.End)
  server.respond(incoming, 204, [], <<>>) |> should.be_ok
  let #(status, _, body) = receive_client_response(stream, [])
  assert status == 204
  assert body == <<>>

  assert server.cancel(incoming) == Ok(server.AlreadyCompleted)
  assert server.cancel(incoming) == Ok(server.AlreadyCompleted)
  assert client.close(connection) == Ok(client.Closed)
  assert server.stop(listener) == Ok(server.Stopped)
}
...
pub fn graceful_stop_types_rejected_and_new_client_work_test() -> Nil {
  ...
  assert server.next_event(incoming) == Ok(server.End)
  // QUIC does not order separate request streams. Establish the request that
  // must drain at the application boundary before opening the request that
  // must remain queued and be rejected by graceful shutdown.
  let rejected =
...
// nolint: unused_exports -- gleeunit discovers public test functions by suffix.
pub fn complete_response_falls_back_above_the_worker_chunk_limit_test() -> Nil {
  let #(certificate, private_key, ca_certificate) =
    http3_test_support.server_credentials()
  let configuration = server.new(certificate, private_key) |> should.be_ok
  let configuration =
    server.with_stream_buffer_limit(configuration, 65_600) |> should.be_ok
  let configuration = server.with_timeout(configuration, 10_000) |> should.be_ok
  let listener = server.start(configuration) |> should.be_ok
  let port = server.port(listener) |> should.be_ok
  let body = http3_test_support.repeated_bytes(65_537)
  let client_task =
    start_bounded_client(port, ca_certificate, "/large-complete")
  let incoming = server.accept(listener) |> should.be_ok

  server.respond(incoming, 200, [], body) |> should.be_ok
...
// nolint: unused_exports -- gleeunit discovers public test functions by suffix.
pub fn complete_response_retries_atomically_under_send_buffer_pressure_test() -> Nil {
  let #(certificate, private_key, ca_certificate) =
    http3_test_support.server_credentials()
  let configuration = server.new(certificate, private_key) |> should.be_ok
  let configuration =
    server.with_stream_buffer_limit(configuration, 2048) |> should.be_ok
  let configuration = server.with_timeout(configuration, 10_000) |> should.be_ok
  let listener = server.start(configuration) |> should.be_ok
  let port = server.port(listener) |> should.be_ok
  let body = http3_test_support.repeated_bytes(2000)
  let client_task = start_bounded_client(port, ca_certificate, "/pressured")
  let incoming = server.accept(listener) |> should.be_ok

  server.respond(incoming, 200, [#("x-padding", string.repeat("x", 128))], body)
  |> should.be_ok

  let reply = http3_test_support.await_task(client_task) |> should.be_ok
  assert reply.body == body
  assert response.get_header(reply, "x-padding") == Ok(string.repeat("x", 128))
  assert server.stop(listener) == Ok(server.Stopped)
}
