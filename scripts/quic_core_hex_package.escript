...
main(Arguments) ->
    try run(Arguments) of
        ...
        Class:Reason:Stacktrace ->
            io:format(standard_error, "quic_core package gate failed: ~p:~p~n~p~n",
                      [Class, Reason, Stacktrace]),
            halt(1)
    end.
...
normalize(Input, Output) ->
    ...
    io:format("canonical quic_core Hex archive ~s  ~s~n", [Digest, Output]),
    ok.
...
compare(First, Second) ->
    ...
    case FirstDigest =:= SecondDigest of
        true ->
            io:format("reproducible quic_core Hex archive ~s~n",
                      [FirstDigest]),
            ok;
        false ->
...
current_export_path(Project) ->
    ...
    filename:join([Project, "build",
                   "quic_core-" ++ binary_to_list(Version) ++ ".tar"]).
...
audit_metadata(Terms) ->
    ...
    ensure(metadata_value(<<"name">>, Terms) =:= <<"quic_core">>,
           unexpected_package_name),
    ensure(metadata_value(<<"app">>, Terms) =:= <<"quic_core">>,
...
audit_contents(Compressed, MetadataTerms) ->
    ...
    Required = [
        ...
        "gleam.toml",
        "src/quic_core.gleam",
        "src/quic_core/client.gleam",
        "src/quic_core/config.gleam",
        "src/quic_core/diagnostics.gleam",
        "src/quic_core/failure.gleam",
        "src/quic_core/server.gleam"
    ],
    lists:foreach(
