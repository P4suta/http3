...
| RFC 9002 | ACK, RTT, loss, PTO, persistent congestion, ECN interaction, and application-limited behavior | Deterministic recovery, pacing, ECN, and loss/reorder tests exist. The release reference-model and full errata differential are not yet present. | Partial |
...
| PRE-010 | The retained baseline is 258 benchmark, 172 load, and 406 soak requests/second. A 2026-08-25 dirty-connection diagnostic reached a 477 benchmark median and 200–212 load but remained below 516/344; one longer load trial also ended in a bounded peer close. | Meet all three 516/344/812 thresholds on the recorded host, remove the long-load failure, and preserve zero-idle-polling and cleanup bounds. |
...
| PRE-012 | Root-owned HTTP/3 workers still import package-private QUIC/TLS primitives instead of consuming only the generic public transport API. | Move the HTTP/3 runtime onto opaque public core connections/streams and make any root import of `gleam_quic/internal/**` fail the boundary audit. |
