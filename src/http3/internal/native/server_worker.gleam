...
type Command {
  ...
  TelemetryStats(
    request_id: Int,
    reply: Subject(Result(#(Int, Int, Int), Error)),
  )
...
/// Atomically replace credentials used by subsequently accepted connections.
...
/// Atomically replace the current/previous operational key generations.
...
pub fn graceful_stop(listener: Listener) -> Result(DrainResult, Error) {
  case process.is_alive(listener.worker) {
    ...
    True -> {
      ...
      let refine_delay =
        int.max(1, int.min(100, listener.drain_timeout_milliseconds / 4))
      case
        call_with_timeout(
          listener,
...
/// Snapshot bounded diagnostic-writer counters for a request connection.
pub fn telemetry_stats(request: Request) -> Result(#(Int, Int, Int), Error) {
  call(request.listener, fn(reply) { TelemetryStats(request.identifier, reply) })
}
...
fn initialise(
  ...
  let startup = {
    ...
    use stateless_reset_keys <- result.try(
  ...
  case startup {
    ...
    Ok(#(
      ...
      let assert [stateless_reset_key, ..] = stateless_reset_keys
...
fn wait_for_work(worker: Worker, now: Int) -> Nil {
  case next_worker_deadline(worker, now) {
    Error(_) -> shutdown(worker, "connection timer failed")
    Ok(deadline) -> {
...
fn next_worker_deadline(
  ...
  let deadline =
    accept_waiter_deadline(worker.accept_waiters)
    |> earlier_deadline(drain_waiter_deadline(worker.drain_waiter))
    |> request_deadlines(dict.values(worker.requests))
    |> push_deadlines(dict.values(worker.pushes))
  connection_deadlines(deadline, dict.values(worker.connections), now)
}
  ...
  case peers {
    ...
    [peer, ..rest] -> {
      use protocol <- result.try(
        server_connection.next_deadline(peer.connection, now)
        |> result.map_error(map_connection_error),
      )
      connection_deadlines(
        deadline
          |> earlier_deadline(protocol)
          |> earlier_deadline(positive_deadline(
            peer.next_keepalive_milliseconds,
          ))
          |> earlier_deadline(positive_deadline(
...
fn request_deadlines(
  ...
  case requests {
    ...
    [request, ..rest] ->
      request_deadlines(
        deadline
          |> earlier_deadline(event_waiter_deadline(request.event_waiter))
          |> earlier_deadline(datagram_waiter_deadline(request.datagram_waiter))
          |> earlier_deadline(pending_send_deadline(request.pending_send)),
        rest,
      )
  }
}
...
fn push_deadlines(
  ...
  case pushes {
    ...
    [push, ..rest] ->
      push_deadlines(
        earlier_deadline(deadline, pending_send_deadline(push.pending_send)),
        rest,
      )
  }
}

fn accept_waiter_deadline(waiters: Queue(AcceptWaiter)) -> Option(Int) {
  accept_waiter_deadline_loop(waiters, None)
}

fn accept_waiter_deadline_loop(
...
fn drain_waiter_deadline(waiter: Option(DrainWaiter)) -> Option(Int) {
  case waiter {
    Some(DrainWaiter(_, deadline, _, True)) -> Some(deadline)
    Some(DrainWaiter(_, deadline, refine_at, False)) ->
      earlier_deadline(Some(deadline), Some(refine_at))
    None -> None
  }
}

fn event_waiter_deadline(waiter: Option(EventWaiter)) -> Option(Int) {
  case waiter {
    Some(EventWaiter(_, deadline)) -> Some(deadline)
    None -> None
  }
}
...
fn network_step(worker: Worker, message: Dynamic) -> Nil {
  case udp.receive_relay_batch(worker.relay, message) {
    Ok(datagrams) -> {
      // Replenish one batch of credit before protocol work. The relay still
      // permits at most one additional bounded batch in this actor's mailbox.
      case udp.continue_relay(worker.relay) {
        ...
        Error(_) -> shutdown(worker, "socket relay failed")
      }
    }
    Error(udp.InvalidInput) -> loop(worker)
    Error(udp.Closed) -> shutdown(worker, "socket closed")
    Error(_) -> shutdown(worker, "socket receive failed")
  }
}
...
fn handle_command(worker: Worker, command: Command) -> Result(Worker, Nil) {
  case command {
    ...
    ReloadCertificates(
      ...
      let server_config =
        server_connection.Config(
          ...
          alternative_credentials: alternative_credentials,
        )
      process.send(reply, Ok(Nil))
      Ok(Worker(..worker, server_config: server_config))
    }
    ReloadOperationalKeys(
      ticket_keys,
      address_token_keys,
    ...
    TelemetryStats(identifier, reply) -> {
      let outcome =
        with_request_connection(worker, identifier, fn(_, peer, _) {
...
fn handle_accept(
  ...
  case worker.drain_waiter {
    ...
    None ->
      case queue_pop(worker.pending_requests) {
        Ok(#(identifier, pending_requests)) -> {
          process.send(reply, incoming(worker, identifier))
          Ok(Worker(..worker, pending_requests: pending_requests))
        }
        Error(Nil) ->
          case
...
fn reject_pending_requests(worker: Worker, identifiers: Queue(Int)) -> Worker {
  case queue_pop(identifiers) {
    Error(Nil) -> worker
    Ok(#(identifier, rest)) -> {
      let worker =
  ...
  case dict.get(worker.requests, identifier) {
    ...
    Ok(request) -> {
        ...
        exceeds_declared_length(request.declared_content_length, new_total)
      {
        ...
        _, _, Some(_), _, _, _ -> reply_error(worker, reply, ConcurrentSend)
  ...
  case dict.get(worker.requests, identifier) {
    ...
    Ok(request) ->
        ...
        )
      {
        ...
        _, _, Some(_), _, _ -> reply_error(worker, reply, ConcurrentSend)
                    ...
                    )
                  {
                    ...
                    Ok(#(connection, push_id, stream_id)) -> {
                      ...
                      Ok(
                        ...
                        |> mark_connection_dirty(request.connection_id)
...
fn active_handshake_count(worker: Worker) -> Int {
  ...
  |> dict.values
  |> list.filter(fn(peer) {
    !server_connection.handshake_established(peer.connection)
  })
  |> list.length
}
        ...
        }
      {
        ...
        Ok(local_connection_id) -> {
            ...
            )
          {
            Error(_) -> worker
            Ok(connection) ->
              case open_qlog(worker.qlog_directory, worker.telemetry_limit) {
                Error(_) -> worker
                Ok(qlog_writer) -> {
                  case qlog_writer {
                    Some(writer) -> {
                  ...
                  |> mark_connection_dirty(local_connection_id)
                }
              }
          }
        }
...
fn tick_and_flush_all(worker: Worker) -> Worker {
  let worker = Worker(..worker, dirty_connections: dict.new())
...
// Network batches and public commands normally touch only one or a few
// connections. Drive those connections once per actor turn instead of asking
// every idle connection to prepare an empty datagram. Timer expiry still uses
// `tick_and_flush_all`, so loss recovery, idle timeout, keepalive, and PMTU
// progress remain independent of inbound traffic.
...
fn tick_and_flush_identifiers(
  ...
  case identifiers {
    [] -> worker
    [connection_id, ..rest] -> {
      let worker = case dict.get(worker.connections, connection_id) {
        Error(_) -> worker
        Ok(peer) -> {
          let peer =
            maybe_queue_keepalive(peer, worker.keepalive_milliseconds, now)
          case server_connection.tick(peer.connection, now) {
            Error(error) ->
              fail_connection(
                worker,
                connection_id,
          ...
          case current_key(worker.address_token_keys) {
            ...
            Ok(address_token_key) ->
                ...
                )
              {
                ...
                Ok(token) ->
                  case
                    server_connection.queue_new_token(peer.connection, token)
                  {
                    Error(_) -> worker
                    Ok(connection) ->
                      put_peer(
...
fn maybe_probe_connection(
  ...
  case dict.get(worker.connections, connection_id) {
    ...
    Ok(peer) ->
      case server_connection.is_established(peer.connection) {
        False ->
          put_peer(
            worker,
            connection_id,
...
fn send_pmtu_probe(
  ...
  case server_connection.prepare_pmtu_probe(peer.connection, now) {
    ...
    Ok(Some(prepared)) -> {
      ...
      {
        // Don't-Fragment is set, so the kernel refuses a probe the local
        // interface cannot carry whole rather than splitting it. That is the
        // probe's answer: it is dropped uncommitted and the path returns to
        // the 1200-byte floor.
        udp.PathTooSmall ->
          put_peer(
            worker,
            ...
            PeerState(
              ..peer,
              connection: server_connection.report_pmtu_black_hole(
                peer.connection,
              ),
              next_pmtu_probe_milliseconds: now
                + pmtu_probe_interval_milliseconds,
  ...
  case worker.drain_waiter {
    ...
    None ->
      case queue_pop(worker.accept_waiters) {
        Ok(#(AcceptWaiter(reply, _), accept_waiters)) -> {
          process.send(reply, incoming(worker, identifier))
          Worker(..worker, accept_waiters: accept_waiters)
        }
        Error(Nil) ->
          case queue_count(worker.pending_requests) >= worker.queue_limit {
            True ->
...
fn enqueue_request_event(
  ...
  case request.failure, request.event_waiter, queue_is_empty(request.events) {
    Some(_), _, _ -> worker
  ...
  Ok(Incoming(
    Request(
      Listener(
        ...
        worker.drain_timeout_milliseconds,
      ),
      identifier,
    ),
    request.method,
...
fn expire_waiters(worker: Worker, now: Int) -> Worker {
  let worker =
    Worker(
      ..worker,
      accept_waiters: expire_accept_waiters(
        worker.accept_waiters,
        now,
        queue_new(),
      ),
...
fn expire_accept_waiters(
  ...
  case queue_pop(remaining) {
    ...
    Ok(#(AcceptWaiter(reply, deadline) as waiter, rest)) -> {
      let retained = case now >= deadline {
        True -> {
      ...
      expire_accept_waiters(rest, now, retained)
    }
  }
}
...
fn mark_connection_dirty(worker: Worker, identifier: BitArray) -> Worker {
  case dict.has_key(worker.connections, identifier) {
    False -> worker
    True ->
      Worker(
        ..worker,
        dirty_connections: dict.insert(
          worker.dirty_connections,
          identifier,
          Nil,
        ),
      )
  ...
  case request_terminal(request) {
    ...
    Some(terminal) -> {
      ...
      Worker(
        ...
        pending_requests: queue_filter(worker.pending_requests, fn(value) {
          value != identifier
        }),
      )
    }
...
fn request_terminal(request: RequestState) -> Option(TerminalState) {
  ...
  {
    True, Some(error), _ -> Some(TerminalFailed(error))
    True, None, True -> Some(TerminalCompleted)
    _, _, _ -> None
  }
}
...
fn map_connection_error(error: server_connection.Error) -> Error {
  case error {
    ...
    server_connection.StatelessResetFailure(_) -> InvalidConnectionState
