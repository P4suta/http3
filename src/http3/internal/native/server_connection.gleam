...
/// Return the earliest QUIC deadline for the listener's event loop.
pub fn next_deadline(state: State, now_ms: Int) -> Result(Option(Int), Error) {
  case state.protocol {
    Handshaking(quic) ->
      driver.next_deadline(quic, now_ms) |> result.map_error(DriverFailure)
    Established(http3) ->
      session.next_deadline(http3, now_ms) |> result.map_error(SessionFailure)
  }
}
...
/// Return whether TLS completed and HTTP/3 state was installed.
