...
/// Return whether the authenticated client handshake selected its ticket.
...
/// Return the earliest underlying QUIC deadline for an event-driven owner.
pub fn next_deadline(state: State, now_ms: Int) -> Result(Option(Int), Error) {
  driver.next_deadline(state.quic, now_ms) |> map_driver_result
}
...
fn start_established(
  ...
  process_transport_events(
    State(
      driver.put_connection(quic, connection),
      http3,
      dict.new(),
      [],
...
fn new_frame_parser(state: State) -> Result(frame_parser.State, Error) {
  frame_parser.new(
    frame.Limits(
    ...
    // A transport read can contain the tail of the current frame plus bytes
    // from following frames. Leave one complete read of headroom so a valid
    // maximum-sized frame is independent of QUIC delivery boundaries.
