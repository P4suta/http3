//// Bounded RFC 8336 ORIGIN payloads and normalized HTTP origins.
...
/// Canonical authority key used by the authenticated HTTP/2 session pool.
/// The explicit port keeps alternative services and non-default endpoints
/// partitioned even when they share a host name.
pub fn connection_key(host host: String, port port: Int) -> String {
  "https://" <> string.lowercase(host) <> ":" <> int.to_string(port)
}

/// Default-deny pool authority check. An ORIGIN advertisement never bypasses
/// certificate validation or expands reuse by itself.
pub fn same_connection_authority(
...
// nolint: label_possible -- private parser state is always advanced positionally.
fn decode_entries(
  payload: BitArray,
  maximum_entries: Int,
  count: Int,
