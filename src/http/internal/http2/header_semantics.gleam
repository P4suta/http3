//// Strict RFC 9110/RFC 9113 HTTP field and pseudo-header validation.
...
/// Context in which one decoded HPACK field section appears.
...
pub type Error {
  ...
  ContentLengthForbidden(status: Int)
...
fn validate_response(
  ...
  case content_length, status {
    Some(_), status if status >= 100 && status < 200 ->
      Error(ContentLengthForbidden(status: status))
    Some(_), 204 -> Error(ContentLengthForbidden(status: status))
    _, _ -> Ok(Validated(ResponseControlData(status), regular, content_length))
  }
}
